...
function audiostreamer_connected_host_pid_from_log_line() {
  local log_line=$1
...
function audiostreamer_require_same_host_process() {
...
function audiostreamer_require_continuous_log() {
...
function audiostreamer_empty_sha256() {
...
function audiostreamer_capture_log_snapshot() {
  local log_path=$1
...
ready_path = os.environ.get("AUDIOSTREAMER_LOG_SNAPSHOT_TEST_READY")
proceed_path = os.environ.get("AUDIOSTREAMER_LOG_SNAPSHOT_TEST_PROCEED")
hook_used = False
...
PY
  ...
  AUDIOSTREAMER_LOG_SNAPSHOT_ID=${metadata_lines[1]}
  AUDIOSTREAMER_LOG_SNAPSHOT_OFFSET=${metadata_lines[2]}
  AUDIOSTREAMER_LOG_SNAPSHOT_DIGEST=${metadata_lines[3]}
  AUDIOSTREAMER_LOG_SNAPSHOT_ENDS_WITH_NEWLINE=${metadata_lines[4]}
}
...
function audiostreamer_split_completed_log_lines() {
...
function audiostreamer_audit_connected_log_lines() {
  ...
  while IFS= read -r line; do
    ...
    if ! logged_process_id=$(audiostreamer_connected_host_pid_from_log_line \
        "${line}"); then
      return 1
    fi
    if ! audiostreamer_require_same_host_process \
  ...
  AUDIOSTREAMER_AUDITED_CONNECTION_COUNT=${audited_connections}
}

function audiostreamer_exit_on_interrupt() {
  trap - INT TERM
  exit 130
}

function audiostreamer_exit_on_termination() {
  trap - INT TERM
  exit 143
}

function audiostreamer_write_state() {
  local output=$1
  local value=$2
...
function audiostreamer_wait_for_process_exit() {
  local process_pid=$1
...
function audiostreamer_wait_for_final_process_status() {
  local process_pid=$1
  ...
  while true; do
    ...
    if (( observed_status == 127 )); then
      AUDIOSTREAMER_FINAL_PROCESS_STATUS=${final_status}
      return 0
    fi
...
function audiostreamer_run_with_timeout() {
...
function audiostreamer_exec_in_isolated_process_group() {
...
function audiostreamer_start_isolated_validation_process() {
  audiostreamer_exec_in_isolated_process_group "$@" &
  XCODEBUILD_PID=$!
  ...
  audiostreamer_require_isolated_process_group "${XCODEBUILD_PID}" 5
}

function audiostreamer_require_isolated_process_group() {
  local leader_pid=$1
  local wait_started=${SECONDS}
  local timeout_seconds=$2
  local process_group
...
function audiostreamer_process_group_exists() {
  local leader_pid=$1
  kill -0 -- "-${leader_pid}" 2>/dev/null
}

function audiostreamer_wait_for_process_group_exit() {
  local leader_pid=$1
  local timeout_seconds=$2
  local wait_started=${SECONDS}

  while audiostreamer_process_group_exists "${leader_pid}"; do
    if (( SECONDS - wait_started >= timeout_seconds )); then
      return 1
    fi
    sleep 0.1
...
# the host restart. Every successful call must be paired with `audiostreamer_resume_process_group`.
function audiostreamer_suspend_isolated_process_group() {
  local leader_pid=$1
  local timeout_seconds=$2
  local process_group
  local process_state
...
function audiostreamer_resume_process_group() {
  local leader_pid=$1
...
function audiostreamer_terminate_isolated_process_group() {
  local leader_pid=$1
  ...
  if kill -0 "${leader_pid}" 2>/dev/null; then
    ...
    if [[ -n "${process_group}" && "${process_group}" != "${leader_pid}" ]]; then
      ...
      audiostreamer_terminate_process_tree "${leader_pid}" "${grace_seconds}"
      return
    fi
  fi
...
function audiostreamer_device_is_unlocked() {
  local device_udid=$1
...
function audiostreamer_require_device_unlocked() {
  local device_udid=$1
  local command_timeout_seconds=$2
  local output=$3
  local gate_name=$4
...
function audiostreamer_descendant_pids() {
  local parent_pid=$1
  local child_pid
  local children
  ...
  for child_pid in ${(f)children}; do
    audiostreamer_descendant_pids "${child_pid}"
    print -r -- "${child_pid}"
  done
}
...
function audiostreamer_freeze_process_forest() {
  ...
  for scan in {1..20}; do
    ...
    for known_pid in "${known_pids[@]}"; do
      ...
      descendant_text=$(audiostreamer_descendant_pids "${known_pid}")
...
function audiostreamer_terminate_process_tree() {
  local root_pid=$1
  local grace_seconds=$2
  local grace_started
  local descendant_pid
  ...
  descendant_text=$(audiostreamer_descendant_pids "${root_pid}")
  ...
  frozen_process_text=$(audiostreamer_freeze_process_forest \
