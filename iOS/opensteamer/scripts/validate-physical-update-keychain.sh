...
#
# Optional environment: `OPENSTEAMER_UPDATE_PHASE_TIMEOUT_SECONDS`,
# `OPENSTEAMER_DEVICE_COMMAND_TIMEOUT_SECONDS`, and `OPENSTEAMER_DEVICE_LOCK_POLL_SECONDS`
# adjust bounded waits. `OPENSTEAMER_SCRIPT_SELF_TEST` is reserved for deterministic shell tests;
...
# (default `/private/tmp/opensteamer-device-Keychain-Update`). `run-status.txt` ends in `status=passed`
...
PHASE_TIMEOUT_SECONDS=${OPENSTEAMER_UPDATE_PHASE_TIMEOUT_SECONDS:-900}
...
opensteamer_require_positive_integer \
  OPENSTEAMER_UPDATE_PHASE_TIMEOUT_SECONDS \
...
function validate_xcresult() {
  ...
  local expected_identifier_url="test://com.apple.xcode/opensteamer/opensteamerTests/KeychainStoreTests/${expected_test_name}"
  local summary
  local tests
  local build_results
...
function run_phase() {
  ...
  opensteamer_start_isolated_validation_process xcodebuild test \
    ...
    -only-testing:"opensteamerTests/KeychainStoreTests/${test_name}" \
    -resultBundlePath "${result_bundle}" \
    CURRENT_PROJECT_VERSION="${build_number}" \
    "SWIFT_ACTIVE_COMPILATION_CONDITIONS=DEBUG OPENSTEAMER_UPDATE_VALIDATION_HOST ${condition}"
  (
    set -e
