...
final class DiagnosticDriverV11UpdateContractTests: XCTestCase {
    ...
    private let launcherPath = "macOS/scripts/update-opensteamer-diagnostic-driver-v11.sh"
    ...
    func testV11UsesFreshNamespaceAndRetainsExactV8ReleaseBytes() throws {
        ...
        for token in [
            #"const PREFLIGHT_MODE: &str = "--verify-diagnostic-driver-v11-update-preflight";"#,
            #"const EXECUTE_MODE: &str = "--execute-authorized-diagnostic-driver-v11-update";"#,
            #"const ROLLBACK_MODE: &str = "--rollback-authorized-diagnostic-driver-v11-update";"#,
            #"const SELF_TEST_MODE: &str = "--self-test-diagnostic-driver-v11-update";"#,
            #"const ROOT_MODE: &str = "--root-authorized-diagnostic-driver-v11-update";"#,
            #"const ROOT_ROLLBACK_MODE: &str = "--root-rollback-diagnostic-driver-v11-update";"#,
            #"const ROOT_SEALED_ROLLBACK_MODE: &str = "--root-sealed-rollback-diagnostic-driver-v11-update";"#,
            #"const JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_UPDATE_V11";"#,
            #"const ROOT_JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_UPDATE_V11";"#,
            "/diagnostic-driver-updates-v11", "/active-diagnostic-driver-update-v11",
            "/diagnostic-driver-update-v11.lock", "/diagnostic-driver-controllers-v11",
            "/diagnostic-driver-bootstrap-v11.txt", "/diagnostic-driver-probes-v11",
        ] {
            XCTAssertTrue(freshConstants.contains(token), "missing fresh V11 token: \(token)")
        }
    ...
    func testV11RetainsExactV9ReleaseBytesAndVerifiesThemBeforeTheCandidate() throws {
        let controller = try source(controllerPath)
        let launcher = try source(launcherPath)
        let retainedFiles: [(String, Int, String)] = [
            (
    ...
    func testRetainedV8UserGraphPinsAllElevenTerminalNodes() throws {
        ...
        XCTAssertFalse(openGraph.contains(#"b"diagnostic-driver-update-v11.lock""#))
    ...
    func testRetainedV10ReleaseUserAndRootGraphsAreExactAndImmutable() throws {
        ...
        for token in [
            ...
            "RETAINED_V10_OSDS_BEFORE_SHA256", "BOTH_ORDER_PROBE_SHA256",
        ] {
    ...
    func testRetainedV8V9AndV10LocksSpanPreflightForwardAndSealedRecoveryLifetimes() throws {
        let controller = try source(controllerPath)
        ...
        assertOrdered([
            ...
            "&retained_v10_lock",
            "retained_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10=immutable",
        ], in: preflight)
        ...
        assertOrdered([
            ...
            #""retained v8 root boundary changed across the V11 cutover/rollback""#,
            "verify_retained_v9_root_prestop_attempt(&retained_v9_root_lock)? != retained_v9",
            #""retained v9 root boundary changed across the V11 cutover/rollback""#,
            "verify_retained_v10_root_rolled_back(&retained_v10_root_lock)? != retained_v10",
            #""retained v10 root boundary changed across the V11 cutover/rollback""#,
        ], in: rootTransaction)
    ...
    func testDisplayHelpersStayAquaWhileAuthorizationAndLoopbackUseDirectUID501Audio() throws {
        ...
        for forbidden in [
            ...
            XCTAssertFalse(controller.contains(forbidden), "authorization gate can prompt or capture: \(forbidden)")
        }
    ...
    func testRootSupervisorContainsSplitPGIDsDefersCancellationAndRecoversBeforeReporting() throws {
        let controller = try source(controllerPath)
        for token in [
            #"const ROOT_SUDO_SUPERVISOR_MODE: &str = "--root-supervise-diagnostic-driver-v11-worker";"#,
        ...
        assertOrdered([
            ...
            "OPENSTEAMER_DIAGNOSTIC_DRIVER_V11_SUDO_SUPERVISOR_SELF_TEST_RECOVERY",
        ...
        assertOrdered([
            ...
            "output.status.success()", "DIAGNOSTIC_DRIVER_V11_UPDATE_COMMITTED self-test-survivor=true",
    ...
    func testRouteTranscriptIsCanonicalDurableBoundedAndPrivacySafe() throws {
        ...
        for token in [
            ...
            "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROUTE_PROOF_V11",
        ...
        assertOrdered([
            ...
            "!text.ends_with('\\n')", "text.contains('\\r')",
            #"Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROUTE_PROOF_V11")"#,
    ...
    func testDisplayRecoveryRepairsWrongReviewedModeAndAuthenticatesEveryAbortDispatchBoundary() throws {
        ...
        assertOrdered([
            ...
            "DIAGNOSTIC_DRIVER_V11_ROOT_PRESTOP_ABORTED host_pid={} display_mode={}",
            "display_mode_identity_token(&host.display_mode)",
        ], in: pointerless)
    ...
    func testV11PureSelfTestExecutesTheNewRouteAndAquaSemantics() throws {
        let controller = try source(controllerPath)
        let launcher = try source(launcherPath)
        ...
        XCTAssertTrue(launcher.contains("/reviewed/opensteamer-diagnostic-driver-v11"))
        ...
        process.arguments = [
            ...
            "--self-test-diagnostic-driver-v11-update",
        ]
        ...
        XCTAssertNotNil(
            output.range(
                of: #"\ADIAGNOSTIC_DRIVER_V11_SELF_TEST_OK tests=[1-9][0-9]*\n\z"#,
