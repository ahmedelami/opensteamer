...
final class DiagnosticDriverV8UpdateContractTests: XCTestCase {
    ...
    private let launcherPath = "macOS/scripts/update-opensteamer-diagnostic-driver-v8.sh"
    ...
    func testV8NamespacesModesAndDedicatedReleaseCheckoutAreExact() throws {
        ...
        for token in [
            ...
            "/diagnostic-driver-updates-v8", "/active-diagnostic-driver-update-v8",
            "/diagnostic-driver-update-v8.lock", "/diagnostic-driver-controllers-v8",
            "/diagnostic-driver-bootstrap-v8.txt", "/diagnostic-driver-probes-v8",
        ] {
            XCTAssertTrue(freshConstants.contains(token), "missing fresh V8 token: \(token)")
        }
        XCTAssertFalse(freshConstants.contains("diagnostic-driver-v7-host-lock"))
        for version in 1 ... 7 {
            for staleToken in [
                ...
                XCTAssertFalse(
                    ...
                    "fresh V8 constants retain stale namespace: \(staleToken)"
                )
            }
        }
        ...
        for privateMode in ["ROOT_MODE", "ROOT_ROLLBACK_MODE", "ROOT_SEALED_ROLLBACK_MODE",
                            ...
                            "UID501_RETAINED_V7_BOUNDARY_MODE", "UID501_FRESH_ROUTE_MODE",
    ...
    func testRetainedV1ThroughV7SourcesAndEvidenceRemainImmutable() throws {
        let expectedSourceHashes = [
            ...
            "macOS/scripts/opensteamer-diagnostic-driver-v7-update-controller.rs":
                "bb5c2f500b3d1f2458c2af6f7f434a74f0fc9cdc06170ab923ff0c0b0e397508",
            "macOS/scripts/update-opensteamer-diagnostic-driver-v7.sh":
                "889f2672d7fd0530e771c75442ce413ac1e9d61d59689d0435471a45850babe6",
            "macOS/Tests/CaptureServerTests/DiagnosticDriverV7UpdateContractTests.swift":
                "2ae7180a26f4d1ce2648a64f8e2d4d6331d568b1c932acc1e9fc0295db752a8c",
        ]
        ...
        XCTAssertGreaterThanOrEqual(
            rootTransaction.components(separatedBy: "verify_retained_v7_root_rolled_back(&retained_v7_root_lock)?").count - 1,
            2
        )
        assertOrdered([
            ...
            "verify_retained_v7_root_rolled_back(&retained_v7_root_lock)?",
    ...
    func testRetainedV4GuardsAreHeldThroughPreflightAndRootDispatch() throws {
        let controller = try source(controllerPath)
        XCTAssertTrue(controller.contains("retained_v1_v2_v3_v4_v5_v6_v7=immutable"))
        XCTAssertFalse(controller.contains("retained_v1_v2_v3_v4_v5_v6=immutable"))
    ...
    func testRetainedV5LocksSpanForwardAndSealedRecoveryLifetimes() throws {
        ...
        assertOrdered([
            ...
            "DIAGNOSTIC_DRIVER_V8_UPDATE_COMMITTED",
        ], in: rootEntrypoint)
        ...
        assertOrdered([
            ...
            #""retained v5 root boundary changed across the V8 cutover/rollback""#,
            "outcome",
        ], in: rootTransaction)
        XCTAssertGreaterThanOrEqual(
            rootTransaction.components(separatedBy: "verify_retained_v5_root_rolled_back(&retained_v5_root_lock)?").count - 1,
    ...
    func testRetainedV6LocksSpanForwardAndSealedRecoveryLifetimes() throws {
        ...
        assertOrdered([
            ...
            #""retained v6 root boundary changed across the V8 cutover/rollback""#,
            "outcome",
        ], in: rootTransaction)
        XCTAssertGreaterThanOrEqual(
            rootTransaction.components(separatedBy: "verify_retained_v6_root_rolled_back(&retained_v6_root_lock)?").count - 1,
    ...
    func testRetainedV7RolledBackUserGraphIsExactAndDescriptorStable() throws {
        let controller = try source(controllerPath)
        for exactPin in [
            ...
            XCTAssertTrue(controller.contains(exactPin), "missing retained V7 user pin: \(exactPin)")
        }

        let openGraph = try functionBody(
            controller,
            beginningWith: "fn open_retained_v7_descriptor_graph(",
            endingBefore: "fn retained_v7_descriptor_graph_identities("
        )
        XCTAssertTrue(openGraph.contains(#"b"diagnostic-driver-update-v7.lock""#))
        XCTAssertFalse(openGraph.contains(#"b"diagnostic-driver-update-v6.lock""#))

        let identities = try functionBody(
            controller,
            beginningWith: "fn retained_v7_descriptor_graph_identities(",
            endingBefore: "fn verify_retained_v7_descriptor_graph_payload("
        )
        XCTAssertEqual(identities.components(separatedBy: "            &graph.").count - 1, 11)
        assertOrdered([
            "RETAINED_V7_USER_UPDATE_ROOT_INODE", "RETAINED_V7_EVIDENCE_INODE",
            "RETAINED_V7_PROBES_INODE", "RETAINED_V7_USER_ACTIVE_POINTER_INODE",
            "RETAINED_V7_JOURNAL_INODE", "RETAINED_V7_REQUEST_INODE",
            "RETAINED_V7_READER_INODE", "RETAINED_V7_CONTROLLER_PIN_INODE",
            "RETAINED_V7_CONTROLLER_IDENTITY_INODE", "RETAINED_V7_RESULT_INODE",
            "RETAINED_V7_USER_UPDATE_LOCK_INODE", "identity_from_metadata(&held_metadata)",
            "identities.last() != Some(&held_identity)",
        ], in: identities)

        let payload = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v7_descriptor_graph_payload(",
            endingBefore: "fn verify_retained_v7_descriptor_graph("
        )
        assertOrdered([
            "require_retained_descriptor_children(", "&graph.update_root",
            ...
            #"b"result.txt""#, #"b"root-request.txt""#,
            "require_retained_descriptor_children(&graph.probes, &[]",
            "RETAINED_V7_USER_ACTIVE_POINTER_PENDING",
            "RETAINED_V7_USER_ACTIVE_POINTER_SHA256", "RETAINED_V7_JOURNAL_SHA256",
            "RETAINED_V7_REQUEST_SHA256", "DIAGNOSTIC_READER_SHA256",
            "RETAINED_V7_CONTROLLER_PIN_SHA256", "RETAINED_V7_CONTROLLER_IDENTITY_SHA256",
            "RETAINED_V7_RESULT_SHA256",
        ], in: payload)

        let doubleAttempt = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v7_user_rolled_back(",
            endingBefore: "#[derive(Clone, Debug, Eq, PartialEq)]\nstruct RetainedV1RootAttestation"
        )
        assertOrdered([
            "verify_retained_v7_user_rolled_back_once(held_lock)?",
            "thread::sleep(Duration::from_millis(50))",
            "verify_retained_v7_user_rolled_back_once(held_lock)?",
            "verify_retained_v7_descriptor_graph(&first, held_lock)?",
            "verify_retained_v7_descriptor_graph(&second, held_lock)?",
            "first.support_ancestry != second.support_ancestry",
        ], in: doubleAttempt)
        ...
        assertOrdered([
            "mode == UID501_RETAINED_V7_BOUNDARY_MODE",
    ...
    func testRetainedV7RootGraphPinsAll37RolledBackNodes() throws {
        let controller = try source(controllerPath)
        for exactPin in [
            ...
            XCTAssertTrue(controller.contains(exactPin), "missing retained V7 root pin: \(exactPin)")
        }

        let rootSpecs = try functionBody(
            controller,
            beginningWith: "fn retained_v7_root_node_specs(",
            endingBefore: "fn validate_retained_v7_root_identity("
        )
        XCTAssertEqual(rootSpecs.components(separatedBy: "        file(").count - 1, 21)
        XCTAssertEqual(rootSpecs.components(separatedBy: "        directory(").count - 1, 16)
        for inodePin in [
            "29_479_827", "29_479_828", "29_479_829", "29_479_838", "29_479_833",
            "29_479_837", "29_479_836", "29_479_831", "29_479_832", "29_480_080",
            "29_480_081", "29_480_082", "29_480_083", "29_480_058", "29_480_059",
            "29_480_061", "29_480_062", "29_480_069", "29_480_070", "29_480_075",
            "29_480_071", "29_480_076", "29_480_072", "29_480_077", "29_480_073",
            "29_480_078", "29_480_074", "29_480_079", "29_480_830", "29_480_060",
            "29_480_063", "29_480_832", "29_480_084", "29_480_064", "29_480_831",
            "29_480_066", "RETAINED_V7_ROOT_UPDATE_LOCK_INODE",
        ] {
            XCTAssertTrue(rootSpecs.contains(inodePin), "V7 root graph omits \(inodePin)")
        }
        for digestPin in [
            "RETAINED_V7_REQUEST_SHA256", "RETAINED_V7_CONTROLLER_SHA256",
            "RETAINED_V7_CONTROLLER_PIN_SHA256", "RETAINED_V7_CONTROLLER_IDENTITY_SHA256",
            "RETAINED_V7_ROOT_JOURNAL_SHA256", "RETAINED_V7_ROOT_RESULT_SHA256",
            "RETAINED_V7_ROOT_STATE_SHA256", "RETAINED_V7_ROOT_ACTIVE_POINTER_SHA256",
            "35c3a3f222665015ce139c4b2b517fd905d5810fb501e8f042ff56a7ec504a3d",
            "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
        ] {
            XCTAssertTrue(rootSpecs.contains(digestPin), "V7 root graph omits \(digestPin)")
        }

        let rootLock = try functionBody(
            controller,
            beginningWith: "fn acquire_retained_v7_root_update_lock(",
            endingBefore: "fn collect_retained_v7_root_node_identities("
        )
        assertOrdered([
            "spec.path == Path::new(RETAINED_V7_ROOT_UPDATE_LOCK)",
            ".custom_flags(O_NOFOLLOW | O_CLOEXEC)", ".open(&spec.path)?",
            "flock(file.as_raw_fd(), LOCK_EX | LOCK_NB)",
            "retained_v7_root_node_identity(&spec, false)?", "opened != named || named != held",
            "Ok(file)",
        ], in: rootLock)

        let graphOnce = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v7_root_graph_once(",
            endingBefore: "fn uid501_verify_retained_v7_root_rolled_back("
        )
        assertOrdered([
            "let specs = retained_v7_root_node_specs()", "if specs.len() != 37",
            "collect_retained_v7_root_node_identities(restricted_uid501, held_root_lock)?",
            "let child_sets = vec![", "controller parent", "transaction controller support",
            "probe parent", "probe transaction", "root update root", "root transaction",
            "candidate-stage parent", "prior-driver parent", "transaction probes",
            "failed-driver parent", "failed candidate bundle", "failed candidate Contents",
            "failed candidate MacOS", "failed candidate Resources", "failed candidate locale",
            "failed candidate signature", "for (path, expected, label) in child_sets",
            "RETAINED_V7_ROOT_ACTIVE_POINTER_PENDING", "for spec in &specs",
            "hash_retained_v4_root_file(&spec.path, restricted_uid501)?",
            "collect_retained_v7_root_node_identities(restricted_uid501, held_root_lock)?",
            "if before != after",
        ], in: graphOnce)
    ...
    func testRetainedV7LocksSpanForwardAndSealedRecoveryLifetimes() throws {
        let controller = try source(controllerPath)
        let execute = try functionBody(
            controller,
            beginningWith: "fn execute_authorized_update(",
        ...
        assertOrdered([
            "acquire_retained_v5_user_update_lock()?",
            "acquire_retained_v6_user_update_lock()?",
            "acquire_retained_v7_user_update_lock()?", "let _lock = acquire_user_update_lock()?",
            ...
            "run_sudo_helper(&root_controller, ROOT_MODE",
            "verify_retained_v7_user_rolled_back_once(&retained_v7_lock)?",
            #""retained v7 user boundary changed across root dispatch""#,
        ], in: execute)
        ...
        assertOrdered([
            ...
            #""retained v7 root boundary changed across the V8 cutover/rollback""#,
        ], in: rootTransaction)
        ...
        assertOrdered([
            ...
            "run_sudo_helper(", "ROOT_SEALED_ROLLBACK_MODE",
            "verify_retained_v7_user_rolled_back_once(&retained_v7_lock)?",
            #""retained v7 user boundary changed across sealed root recovery""#,
        ], in: publicRollback)
        ...
        assertOrdered([
            #""retained_v7_journal_sha256""#, "RETAINED_V7_JOURNAL_SHA256",
            #""retained_v7_root_journal_sha256""#, "RETAINED_V7_ROOT_JOURNAL_SHA256",
            #""retained_v7_locator_device""#, "retained_v7.nodes[0].device",
            #""retained_v7_locator_inode""#, "retained_v7.nodes[0].inode",
            #""retained_v7_request_sha256""#, "RETAINED_V7_REQUEST_SHA256",
        ], in: journalFields)
    }
    ...
    func testRealUmask077CreationAndFinalPublicationModeMatrix() throws {
        let temporaryRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("opensteamer-v8-mode-matrix-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRootStagingFailureIsTerminalizedWhileTheHostIsStillPreserved() throws {
        ...
        assertOrdered([
            ...
            "outcome.host != *initial", "outcome.route_status != RollbackRouteStatus::Unchanged",
            "verify_live_current_host()? != *initial",
            "stable_coreaudio_generation()? != *baseline_coreaudio",
            "stable_fresh_route_snapshot_for_generation(baseline_coreaudio)? != *baseline_route",
        ...
        assertOrdered([
            ...
            "write_root_state(", "baseline_route.as_ref()",
            "let route_status = rollback_route_status(baseline_route.as_ref())",
            "route_status.as_str()",
        ], in: finalizer)
        XCTAssertFalse(finalizer.contains("None => (host.clone(), Some(stable_route_snapshot()?"))
        ...
        assertOrdered([
            ...
            "routes=unproved pairing=preserved",
        ], in: pointerless)
        XCTAssertFalse(pointerless.contains("stable_route_snapshot"))
        XCTAssertFalse(pointerless.contains("stable_fresh_route_snapshot_for_generation"))
    }

    func testFreshUID501RouteProtocolIsTypedBoundedAndGenerationBracketed() throws {
        let controller = try source(controllerPath)
        for token in [
            ...
            XCTAssertTrue(controller.contains(token), "missing fresh route constant: \(token)")
        }
        ...
        let classifier = try functionBody(
            ...
            endingBefore: "fn classify_default_device_read("
        )
        assertOrdered([
        ...
        let defaultClassifier = try functionBody(
            controller,
            beginningWith: "fn classify_default_device_read(",
            endingBefore: "fn audio_default_device_classified_local("
        )
        assertOrdered([
            "match classify_device_uid_status(status)",
            "DeviceUidStatusClass::TransientBadObject",
            "RouteCaptureFailure::TransientBadObject {",
            "device: AUDIO_SYSTEM_OBJECT", "status",
            "DeviceUidStatusClass::Fatal", "RouteCaptureFailure::Fatal",
            "DeviceUidStatusClass::Success",
            "size != std::mem::size_of::<u32>() as u32", "device == 0",
            "RouteCaptureFailure::Fatal", "Ok(device)",
        ], in: defaultClassifier)

        let classifiedDefaultRead = try functionBody(
            controller,
            beginningWith: "fn audio_default_device_classified_local(",
            endingBefore: "fn audio_default_device_local("
        )
        assertOrdered([
            "AudioObjectGetPropertyData(",
            "classify_default_device_read(selector, status, size, device)",
        ], in: classifiedDefaultRead)

        let ordinaryDefaultRead = try functionBody(
            controller,
            beginningWith: "fn audio_default_device_local(",
            endingBefore: "fn audio_device_uid_classified_local("
        )
        assertOrdered([
            "audio_default_device_classified_local(selector)", ".map_err(route_capture_failure)",
        ], in: ordinaryDefaultRead)

        let uidRead = try functionBody(
            controller,
            beginningWith: "fn audio_device_uid_classified_local(",
        ...
        let classifiedCapture = try functionBody(
            controller,
            beginningWith: "fn capture_route_snapshot_classified_local(",
            endingBefore: "fn stable_route_snapshot_local("
        )
        assertOrdered([
            "audio_default_device_classified_local(SELECTOR_DEFAULT_INPUT)?",
            "audio_default_device_classified_local(SELECTOR_DEFAULT_OUTPUT)?",
            "audio_default_device_classified_local(SELECTOR_DEFAULT_SYSTEM_OUTPUT)?",
        ...
        let uidHexParser = try functionBody(
            controller,
            beginningWith: "fn parse_route_uid_hex(",
            endingBefore: "fn fresh_route_helper_record("
        )
        assertOrdered([
            "value.is_empty()", "value.len() > 2_046", "value.len() % 2 != 0",
            "byte.is_ascii_digit()", #"matches!(byte, b'a'..=b'f')"#,
            "String::from_utf8(decoded)", "character.is_whitespace()",
            "character.is_control()", "character == '\\0'", "route_uid_hex(&uid) != value",
        ], in: uidHexParser)

        let protocolWriter = try functionBody(
            controller,
            beginningWith: "fn fresh_route_helper_record(",
            endingBefore: "fn parse_fresh_route_helper_record("
        )
        for token in [
            "OPENSTEAMER_DIAGNOSTIC_DRIVER_FRESH_ROUTE_V8",
            "status=complete", "input_uid_hex={}", "output_uid_hex={}",
            "system_output_uid_hex={}", "status=transient-bad-object",
            "device={device}", "osstatus_hex={:08x}",
        ] {
            XCTAssertTrue(protocolWriter.contains(token), "fresh route writer omits \(token)")
        }

        let protocolParser = try functionBody(
            controller,
            beginningWith: "fn parse_fresh_route_helper_record(",
            endingBefore: "fn require_uid501_fresh_route_helper_identity("
        )
        assertOrdered([
            "text.len() > FRESH_ROUTE_HELPER_MAX_OUTPUT_BYTES",
            "!text.ends_with('\\n')", "text.contains('\\r')", "strip_suffix('\\n')",
            #"Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_FRESH_ROUTE_V8")"#,
            #"[_, "status=complete", input, output, system_output]"#,
            #"strip_prefix("input_uid_hex=")"#, #"strip_prefix("output_uid_hex=")"#,
            #"strip_prefix("system_output_uid_hex=")"#, "require_route_roles(",
            #"[_, "status=transient-bad-object", device, "osstatus_hex=216f626a"]"#,
            #"strip_prefix("device=")"#, "parse_positive_u32(",
            "status: AUDIO_HARDWARE_BAD_OBJECT_ERROR", "record shape changed",
        ], in: protocolParser)

        let helperIdentity = try functionBody(
            controller,
            beginningWith: "fn require_uid501_fresh_route_helper_identity(",
            endingBefore: "fn uid501_fresh_route_helper("
        )
        assertOrdered([
            "getuid() } != USER_ID", "geteuid() } != USER_ID", "return Err(",
        ], in: helperIdentity)

        let helper = try functionBody(
            controller,
            beginningWith: "fn uid501_fresh_route_helper(",
            endingBefore: "fn run_uid501_fresh_route_helper("
        )
        assertOrdered([
            "require_uid501_fresh_route_helper_identity()?",
            "capture_route_snapshot_classified_local()",
            "PostReloadRouteObservation::Complete(snapshot)",
            "RouteCaptureFailure::TransientBadObject { device, status }",
            "PostReloadRouteObservation::TransientBadObject { device, status }",
            "RouteCaptureFailure::Fatal(error)", "return Err(error)",
            "fresh_route_helper_record(&observation)",
        ], in: helper)

        let helperLaunch = try functionBody(
            controller,
            beginningWith: "fn run_uid501_fresh_route_helper(",
            endingBefore: "fn classify_fresh_route_helper_execution("
        )
        assertOrdered([
            "env::current_exe()?", "path_text(&executable)?", "&[UID501_FRESH_ROUTE_MODE]",
            "timeout", "true",
        ], in: helperLaunch)
        ...
        XCTAssertTrue(privateDispatch.contains(
            "[_, mode] if mode == UID501_FRESH_ROUTE_MODE => uid501_fresh_route_helper()"
        ))

        let boundedExecution = try functionBody(
            controller,
            beginningWith: "fn bounded_output_in_directory(",
            endingBefore: "fn bounded_null_status("
        )
        assertOrdered([
            "Command::new(program)", ".args(arguments)", ".env_clear()",
            ".stdin(Stdio::null())", ".stdout(Stdio::piped())", ".stderr(Stdio::piped())",
            "if run_as_user", "geteuid() } == ROOT_ID", "command.pre_exec(move ||",
            "fchdir(directory_descriptor)", "setgroups(1, groups.as_ptr())",
            "setgid(USER_GROUP)", "setuid(USER_ID)", ".spawn()",
        ], in: boundedExecution)

        let helperExecution = try functionBody(
            controller,
            beginningWith: "fn classify_fresh_route_helper_execution(",
            endingBefore: "fn fresh_route_observation_for_generation("
        )
        assertOrdered([
            "output.stdout.len() > FRESH_ROUTE_HELPER_MAX_OUTPUT_BYTES",
            "output.stderr.len() > FRESH_ROUTE_HELPER_MAX_OUTPUT_BYTES",
            "if output.status.success()", "if !output.stderr.is_empty()", "String::from_utf8(",
            "parse_fresh_route_helper_record(&text)", "!output.stdout.is_empty()",
            "output.status.code() != Some(1)", #"let prefix = "opensteamer diagnostic-driver updater: ""#,
            "strip_suffix('\\n')", "!value.contains(['\\r', '\\n'])",
            #"Err(ControllerError(format!("#,
        ], in: helperExecution)

        let generationBracket = try functionBody(
            controller,
            beginningWith: "fn fresh_route_observation_for_generation(",
            endingBefore: "fn fresh_complete_route_snapshot_for_generation("
        )
        assertOrdered([
            "let before = read_coreaudio_generation()?", "if before != *expected", "return Err(",
            ...
            "return Err(", "classify_fresh_route_helper_execution(capture)",
        ], in: generationBracket)

        let stableFresh = try functionBody(
            controller,
            beginningWith: "fn stable_fresh_route_snapshot_for_generation(",
            endingBefore: "fn stable_route_snapshot("
        )
        assertOrdered([
            "fresh_complete_route_snapshot_for_generation(expected)?",
            "thread::sleep(Duration::from_millis(250))",
            "fresh_complete_route_snapshot_for_generation(expected)?",
            "if first != second", "return Err(", "Ok(first)",
        ], in: stableFresh)

        let rootDispatcher = try functionBody(
            controller,
            beginningWith: "fn stable_route_snapshot(",
            endingBefore: "fn advance_post_reload_route_proof("
        )
        assertOrdered([
            "geteuid() } == USER_ID", "require_uid501_fresh_route_helper_identity()?",
            "stable_route_snapshot_local()", "geteuid() } == ROOT_ID",
            "let generation = stable_coreaudio_generation()?",
            "stable_fresh_route_snapshot_for_generation(&generation)", "else",
            "Err(ControllerError(",
        ], in: rootDispatcher)
        ...
        assertOrdered([
            ...
            "Some(previous) if *previous == current", "PostReloadRouteProofProgress::Proven",
            "None =>", "*previous_complete = Some(current)",
        ], in: reducer)
        ...
        assertOrdered([
            ".checked_add(POST_RELOAD_ROUTE_TIMEOUT)", #""post-reload route deadline overflowed""#,
        ...
        assertOrdered([
            ...
            "stable_fresh_route_snapshot_for_generation(&new_coreaudio)? != baseline_route",
            "restart_exact_current_host(&initial)?",
            "stable_fresh_route_snapshot_for_generation(&new_coreaudio)? != baseline_route",
        ], in: transaction)
        ...
        XCTAssertEqual(
            controller.components(separatedBy: "capture_route_snapshot_classified_local(").count - 1,
            2
        )
        XCTAssertEqual(controller.components(separatedBy: "capture_route_snapshot_local(").count - 1, 3)

        let routeTriState = try functionBody(
            controller,
            beginningWith: "enum RollbackRouteStatus",
            endingBefore: "fn finalize_prestop_preserving_host("
        )
        assertOrdered([
            "Unchanged", "Drifted", "Unproved", #"Self::Unchanged => "unchanged""#,
            #"Self::Drifted => "drifted""#, #"Self::Unproved => "unproved""#,
            "let Some(baseline) = baseline else", "return RollbackRouteStatus::Unproved",
            "match stable_route_snapshot()", "Ok(current) if current == *baseline",
            "RollbackRouteStatus::Unchanged", "Ok(_) => RollbackRouteStatus::Drifted",
            "Err(_) => RollbackRouteStatus::Unproved",
        ], in: routeTriState)

        for hostileToken in [
            "fresh route helper canonical protocol did not round trip exactly",
            "fresh route wrong header", "fresh route missing final line feed", "fresh route CRLF",
            "fresh route reordered fields", "fresh route duplicate field", "fresh route missing field",
            "fresh route extra field", "fresh route mixed complete and transient fields",
            "fresh route empty UID", "fresh route odd hex UID", "fresh route uppercase hex UID",
            "fresh route nonhex UID", "fresh route whitespace UID", "fresh route NUL UID",
            "fresh route invalid UTF-8 UID", "fresh route oversized UID",
            "fresh route forbidden writer input role", "fresh route forbidden visible output role",
            "fresh route zero transient device", "fresh route noncanonical transient device",
            "fresh route overflowing transient device", "fresh route non-bad-object status",
            "classified default-device read is not exact !obj-only transient semantics",
            ...
            "rollback route tri-state serialization changed",
        ] {
    ...
    func testLauncherEarlyRecoveryRunsThePinnedControllerAsUID501() throws {
        let launcher = try source(launcherPath)
        ...
        assertOrdered([
            #"[ -f "$ROOT_RECOVERY_CONTROLLER" ]"#,
            #"[ ! -L "$ROOT_RECOVERY_CONTROLLER" ]"#,
            #"[ -f "$ROOT_RECOVERY_CONTROLLER_PIN" ]"#,
            ...
            #"DIAGNOSTIC_DRIVER_V8_RECOVERY_COMPLETE\ outcome=committed\ *"#,
            #"DIAGNOSTIC_DRIVER_V8_RECOVERY_COMPLETE\ outcome=rolled-back\ *"#,
            #"DIAGNOSTIC_DRIVER_V8_RECOVERY_COMPLETE\ outcome=prestop-aborted\ *"#,
            #"/usr/bin/printf '%s\n' "$RECOVERY_OUTPUT""#, "exit 0",
        ], in: recovery)
        XCTAssertFalse(recovery.contains(
            #"/usr/bin/sudo -n -- "$ROOT_RECOVERY_CONTROLLER""#
        ))
        XCTAssertFalse(recovery.contains("ROOT_SEALED_ROLLBACK_MODE"))
        XCTAssertFalse(launcher.contains("ROOT_SEALED_ROLLBACK_MODE="))
    }

    func testLauncherHasFreshV8PinsAndPureSelfTestOnly() throws {
        let controller = try source(controllerPath)
        let launcher = try source(launcherPath)
        ...
        XCTAssertTrue(launcher.contains("/reviewed/opensteamer-diagnostic-driver-v8"))
    ...
    func testPureControllerSelfTestPassesWithoutLiveModes() throws {
        ...
        process.arguments = ["--self-test-diagnostic-driver-v8-update"]
        ...
        XCTAssertEqual(output, "DIAGNOSTIC_DRIVER_V8_SELF_TEST_OK tests=112\n")
