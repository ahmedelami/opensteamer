...
final class DiagnosticDriverV5UpdateContractTests: XCTestCase {
    ...
    private let launcherPath = "macOS/scripts/update-opensteamer-diagnostic-driver-v5.sh"
    ...
    func testV5NamespacesModesAndDedicatedReleaseCheckoutAreExact() throws {
        ...
        for token in [
            ...
            "/diagnostic-driver-updates-v5", "/active-diagnostic-driver-update-v5",
            "/diagnostic-driver-update-v5.lock", "/diagnostic-driver-controllers-v5",
            "/diagnostic-driver-bootstrap-v5.txt", "/diagnostic-driver-probes-v5",
        ] {
            XCTAssertTrue(freshConstants.contains(token), "missing fresh V5 token: \(token)")
        }
        for version in 1 ... 4 {
            for staleToken in [
                ...
                XCTAssertFalse(
                    ...
                    "fresh V5 constants retain stale namespace: \(staleToken)"
                )
            }
        }
    ...
    func testRetainedV1V2V3AndV4SourcesAndEvidenceRemainImmutable() throws {
    ...
    func testRetainedV4GuardsAreHeldThroughPreflightAndRootDispatch() throws {
        let controller = try source(controllerPath)
        XCTAssertTrue(controller.contains("retained_v1_v2_v3_v4=immutable"))
        XCTAssertFalse(controller.contains("retained_v1_v2_v3=immutable"))
    ...
    func testRealUmask077CreationAndFinalPublicationModeMatrix() throws {
        let temporaryRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("opensteamer-v5-mode-matrix-\(UUID().uuidString)", isDirectory: true)
    ...
    func testLauncherHasFreshV5PinsAndPureSelfTestOnly() throws {
        let controller = try source(controllerPath)
        let launcher = try source(launcherPath)
        ...
        XCTAssertTrue(launcher.contains("/reviewed/opensteamer-diagnostic-driver-v5"))
    ...
    func testPureControllerSelfTestPassesWithoutLiveModes() throws {
        ...
        process.arguments = ["--self-test-diagnostic-driver-v5-update"]
        ...
        XCTAssertEqual(output, "DIAGNOSTIC_DRIVER_V5_SELF_TEST_OK tests=112\n")
