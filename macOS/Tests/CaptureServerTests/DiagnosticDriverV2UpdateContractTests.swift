...
final class DiagnosticDriverV2UpdateContractTests: XCTestCase {
    ...
    private let launcherPath = "macOS/scripts/update-opensteamer-diagnostic-driver-v2.sh"
    ...
    func testNamespacesAreFreshDisjointAndModesAreNarrow() throws {
        ...
        for token in [
            #"const PREFLIGHT_MODE: &str = "--verify-diagnostic-driver-v2-update-preflight";"#,
            #"const EXECUTE_MODE: &str = "--execute-authorized-diagnostic-driver-v2-update";"#,
            #"const ROLLBACK_MODE: &str = "--rollback-authorized-diagnostic-driver-v2-update";"#,
            #"const SELF_TEST_MODE: &str = "--self-test-diagnostic-driver-v2-update";"#,
            #"const ROOT_MODE: &str = "--root-authorized-diagnostic-driver-v2-update";"#,
            #"const ROOT_ROLLBACK_MODE: &str = "--root-rollback-diagnostic-driver-v2-update";"#,
            #"const ROOT_SEALED_ROLLBACK_MODE: &str = "--root-sealed-rollback-diagnostic-driver-v2-update";"#,
            #"const JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_UPDATE_V2";"#,
            #"const ROOT_JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_UPDATE_V2";"#,
            "/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v2",
            "/Users/ahmed/Library/Application Support/opensteamer/active-diagnostic-driver-update-v2",
        ...
        for mode in [
            "--verify-diagnostic-driver-v2-update-preflight",
            "--execute-authorized-diagnostic-driver-v2-update",
            "--rollback-authorized-diagnostic-driver-v2-update",
            "--self-test-diagnostic-driver-v2-update",
        ] {
            XCTAssertTrue(launcher.contains(mode), "launcher omits public mode: \(mode)")
        }
        XCTAssertFalse(launcher.contains("--root-authorized-diagnostic-driver-v2-update"))
        XCTAssertFalse(launcher.contains("--root-rollback-diagnostic-driver-v2-update"))
        XCTAssertTrue(launcher.contains("--root-sealed-rollback-diagnostic-driver-v2-update"))
        ...
        let retainedConstants = try functionBody(
            controller,
            beginningWith: "const RETAINED_V1_DEVICE:",
            endingBefore: "const HOST_APP:"
        )
        let controllerOutsideRetainedConstants = controller.replacingOccurrences(
            of: retainedConstants,
            with: ""
        )
        let allowedV1OutsideRetainedConstants = [
            "RETAINED_V1_", "retained_v1", "RetainedV1", "retained v1",
            #"b"diagnostic-driver-updates-v1""#,
            #"b"active-diagnostic-driver-update-v1""#,
            #"b"diagnostic-driver-update-v1.lock""#,
            ...
            "OPENSTEAMER_V8_HOST_BUNDLE_FD_MANIFEST_V1",
            "--porcelain=v1",
        ]
        for (offset, line) in controllerOutsideRetainedConstants.split(separator: "\n").enumerated()
        where line.localizedCaseInsensitiveContains("v1") {
            XCTAssertTrue(
                allowedV1OutsideRetainedConstants.contains { line.contains($0) },
                "unscoped v1 reference outside retained constants at filtered line \(offset + 1): \(line)"
            )
        }
        XCTAssertFalse(launcher.localizedCaseInsensitiveContains("v1"))
        for compatibilityIdentifier in [
            "com.elamin.opensteamer.CaptureServer.WorldwidePairing.v1",
            ...
            XCTAssertTrue(
                controller.contains(compatibilityIdentifier),
                "v2 retry changed independent compatibility identifier: \(compatibilityIdentifier)"
            )
        }
    }

    func testFailedV1AttemptSourcesRemainExact() throws {
        let v1Controller = try source(
            "macOS/scripts/opensteamer-diagnostic-driver-v1-update-controller.rs"
        )
        let v1Launcher = try source("macOS/scripts/update-opensteamer-diagnostic-driver-v1.sh")
        let v1Tests = try source(
            "macOS/Tests/CaptureServerTests/DiagnosticDriverV1UpdateContractTests.swift"
        )
        XCTAssertEqual(
            sha256Hex(v1Controller),
            "0a9125e53dce0ee87b76fc83543319064608a8c9b0a8947644e90bf570198e72"
        )
        XCTAssertEqual(
            sha256Hex(v1Launcher),
    ...
    func testRetainedV1AttemptIsExactlyPinnedAndReadOnly() throws {
        let controller = try source(controllerPath)
        ...
        for exactPin in [
            ...
            XCTAssertTrue(constants.contains(exactPin), "retained v1 constants omit exact pin: \(exactPin)")
        }
        XCTAssertFalse(constants.contains("const RETAINED_V1_USER_UPDATE_ROOT:"))
        XCTAssertFalse(constants.contains("const RETAINED_V1_USER_ACTIVE_POINTER:"))

        let rootMetadata = try functionBody(
            controller,
            beginningWith: "fn retained_v1_root_locator_metadata(",
            endingBefore: "fn require_retained_v1_root_namespaces_absent("
        )
        for token in [
            "metadata.uid() != ROOT_ID", "metadata.gid() != ROOT_ID",
            "metadata.nlink() != 1", "mode() & 0o7777 != 0o400",
            "metadata.st_flags() != 0", "metadata.dev() != RETAINED_V1_DEVICE",
            "metadata.ino() != RETAINED_V1_ROOT_BOOTSTRAP_LOCATOR_INODE",
            "metadata.len() != RETAINED_V1_REQUEST_TEXT.len() as u64",
        ] {
            XCTAssertTrue(rootMetadata.contains(token), "retained root locator omits pin: \(token)")
        }

        let absentRoot = try functionBody(
            controller,
            beginningWith: "fn require_retained_v1_root_namespaces_absent(",
            endingBefore: "struct RetainedV1DescriptorGraph"
        )
        for token in [
            ...
            XCTAssertTrue(absentRoot.contains(token), "retained root absence proof omits \(token)")
        }

        let descriptorGate = try functionBody(
            controller,
            beginningWith: "fn require_retained_v1_descriptor(",
            endingBefore: "fn require_retained_v1_support_descriptor("
        )
        for token in [
            ...
            XCTAssertTrue(descriptorGate.contains(token), "retained descriptor gate omits \(token)")
        }

        let supportGate = try functionBody(
            controller,
            beginningWith: "fn require_retained_v1_support_descriptor(",
            endingBefore: "fn require_retained_v1_descriptor_children("
        )
        for token in [
            "metadata.file_type().is_dir()", "metadata.uid() != USER_ID",
            "metadata.gid() != USER_GROUP", "mode() & 0o7777 != 0o700",
            "metadata.st_flags() != 0", "metadata.dev() != RETAINED_V1_DEVICE",
            "metadata.ino() != RETAINED_V1_USER_SUPPORT_INODE",
            "require_descriptor_no_acl_or_xattrs(file, \"retained v1 user support\")?",
        ] {
            XCTAssertTrue(supportGate.contains(token), "retained support gate omits \(token)")
        }

        let childGate = try functionBody(
            controller,
            beginningWith: "fn require_retained_v1_descriptor_children(",
            endingBefore: "fn read_retained_v1_descriptor("
        )
        XCTAssertTrue(childGate.contains("let actual = list_directory_fd(directory)?"))
        XCTAssertTrue(childGate.contains("actual != expected"))

        let descriptorRead = try functionBody(
            controller,
            beginningWith: "fn read_retained_v1_descriptor(",
            endingBefore: "fn retained_v1_root_locator_metadata("
        )
        assertOrdered(
            [
                "let before = file.metadata()?", "before.len() > maximum",
                "let mut reader = file.try_clone()?", "lseek(reader.as_raw_fd(), 0, 0)",
                ".take(maximum.saturating_add(1))", ".read_to_end(&mut bytes)?",
                "let after = file.metadata()?", "require_descriptor_no_acl_or_xattrs(file, label)?",
        ...
        let descriptorMetadataGate = try functionBody(
            controller,
            beginningWith: "fn require_descriptor_no_acl_or_xattrs(",
            endingBefore: "fn readlinkat_exact("
        )
        for token in [
            "descriptor_xattrs(file)?", "attributes.is_empty()",
            "acl_get_fd_np(file.as_raw_fd(), ACL_TYPE_EXTENDED)",
            "acl_get_entry(acl, ACL_FIRST_ENTRY, &mut entry)", "acl_free(acl)",
        ] {
            XCTAssertTrue(
                descriptorMetadataGate.contains(token),
                "descriptor ACL/xattr proof omits \(token)"
            )
        }

        let openGraph = try functionBody(
            controller,
            beginningWith: "fn open_retained_v1_descriptor_graph(",
            endingBefore: "fn verify_retained_v1_descriptor_graph("
        )
        assertOrdered(
            [
                ...
                "openat_child(&evidence, b\"probes\"",
                "openat_child(&support, b\"active-diagnostic-driver-update-v1\"",
                "openat_child(&evidence, b\"journal.log\"",
                "openat_child(&evidence, b\"root-request.txt\"",
                "openat_child(\n        &evidence,\n        b\"opensteamer-diagnostic-snapshot-reader\"",
                "openat_child(&support, b\"diagnostic-driver-update-v1.lock\"",
            ],
            in: openGraph
        )
        XCTAssertEqual(occurrences(of: "openat_child(", in: openGraph), 8)
        XCTAssertEqual(occurrences(of: "Path::new(", in: openGraph), 1)

        let identityProof = try functionBody(
            controller,
            beginningWith: "fn retained_v1_descriptor_graph_identities(",
            endingBefore: "fn verify_retained_v1_descriptor_graph_payload("
        )
        assertOrdered(
            [
                "require_retained_v1_support_descriptor(&graph.support)?",
                "require_retained_v1_descriptor(\n        &graph.update_root",
                "require_retained_v1_descriptor(\n        &graph.evidence",
                "require_retained_v1_descriptor(\n        &graph.probes",
                "require_retained_v1_descriptor(\n        &graph.pointer",
                "require_retained_v1_descriptor(\n        &graph.journal",
                ...
                "require_retained_v1_descriptor(\n        &graph.named_lock",
                "require_retained_v1_descriptor(\n        held_lock",
                "retained v1 held/named lock identity differs",
        ...
        let payloadProof = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v1_descriptor_graph_payload(",
            endingBefore: "fn verify_retained_v1_descriptor_graph("
        )
        assertOrdered(
            [
                "require_retained_v1_descriptor_children(\n        &graph.update_root",
                "require_retained_v1_descriptor_children(\n        &graph.evidence",
                "require_retained_v1_descriptor_children(&graph.probes, &[]",
                "read_retained_v1_descriptor(&graph.pointer, 256",
                "read_retained_v1_descriptor(&graph.journal, 512",
                "read_retained_v1_descriptor(&graph.request, 1_024",
                "read_retained_v1_descriptor(&graph.reader, DIAGNOSTIC_READER_SIZE",
        ...
        let graphProof = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v1_descriptor_graph(",
            endingBefore: "fn verify_retained_v1_user_prestop_attempt_once("
        )
        assertOrdered(
            [
        ...
        XCTAssertEqual(
            occurrences(of: "retained_v1_descriptor_graph_identities(graph, held_lock)?", in: graphProof),
            3
        )
        XCTAssertEqual(
            occurrences(of: "verify_retained_v1_descriptor_graph_payload(graph)?", in: graphProof),
            2
        )
        for proof in [identityProof, payloadProof, graphProof] {
            for forbidden in [
                "Path::new(", "fs::symlink_metadata", "require_regular(", "require_directory(",
                "require_exact_child_names", "read_bounded_utf8", "sha256(&",
            ] {
                XCTAssertFalse(proof.contains(forbidden), "retained graph uses path proof: \(forbidden)")
            }
        }

        let userProof = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v1_user_prestop_attempt_once(",
            endingBefore: "fn verify_retained_v1_user_prestop_attempt("
        )
        assertOrdered(
            [
                "getuid() } != USER_ID", "geteuid() } != USER_ID",
                "let first = open_retained_v1_descriptor_graph()?",
                "verify_retained_v1_descriptor_graph(&first, held_lock)?",
                "let second = open_retained_v1_descriptor_graph()?",
                "verify_retained_v1_descriptor_graph(&second, held_lock)?",
        ...
        XCTAssertEqual(occurrences(of: "open_retained_v1_descriptor_graph()?", in: userProof), 2)
        XCTAssertEqual(
            occurrences(of: "verify_retained_v1_descriptor_graph(", in: userProof),
            2
        )
        for forbidden in ["Path::new(", "fs::symlink_metadata", "sha256(", "read_bounded_utf8"] {
            XCTAssertFalse(userProof.contains(forbidden), "UID501 proof directly reopens a path: \(forbidden)")
        }
        XCTAssertFalse(userProof.contains("retained_v1_root_locator_metadata"))
        XCTAssertFalse(userProof.contains("require_retained_v1_root_namespaces_absent"))
        XCTAssertFalse(userProof.contains("write_"))
        XCTAssertFalse(userProof.contains("remove_"))

        let stableUserProof = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v1_user_prestop_attempt(",
        ...
        assertOrdered(
            [
                ...
                "verify_retained_v1_descriptor_graph(&first_guard, held_lock)?",
                "verify_retained_v1_descriptor_graph(&second_guard, held_lock)?",
        ...
        XCTAssertFalse(stableUserProof.contains("Path::new("))
        XCTAssertFalse(stableUserProof.contains("retained_v1_root_locator_metadata"))
        XCTAssertFalse(stableUserProof.contains("require_retained_v1_root_namespaces_absent"))

        let rootProof = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v1_root_prestop_attempt(",
        ...
        assertOrdered(
            [
                ...
                "digest != RETAINED_V1_REQUEST_SHA256",
        ...
        XCTAssertFalse(rootProof.contains("verify_retained_v1_user_prestop_attempt"))
        XCTAssertFalse(rootProof.contains("RETAINED_V1_USER_"))
        XCTAssertFalse(rootProof.contains("RETAINED_V1_EVIDENCE"))
        XCTAssertFalse(rootProof.contains("write_"))
        XCTAssertFalse(rootProof.contains("remove_"))
    }
    ...
    func testAuthorizationBindsFreshPushedMainWhileCandidateProvenanceStaysFe05() throws {
        ...
        assertOrdered(
            [
                ...
                "let (commit, tree, initial, _initial_retained_v1_guard) =",
                "verify_complete_preflight(repo, true, &retained_v1_lock)?",
                "commit != authorized_commit || tree != authorized_tree",
                "let nonce = random_nonce()?",
                "create_user_layout(&nonce)?",
                "build_diagnostic_reader(repo, &nonce, &layout.reader)?",
                "stage_root_owned_controller(",
                "let (final_commit, final_tree, final_host, final_retained_v1_guard) =",
                "verify_complete_preflight(repo, false, &retained_v1_lock)?",
                ...
                "final_retained_v1_guard.support_ancestry != dispatch_retained_v1_guard.support_ancestry",
                "final_guard_identities != dispatch_guard_identities",
                "retained v1 descriptor guard changed immediately before root dispatch",
    ...
    func testPreflightIsReadOnlyAndDoesNotCreateEvidence() throws {
        ...
        XCTAssertTrue(complete.contains("retained_v1_lock: &File"))
        XCTAssertTrue(
            complete.contains("Result<(String, String, HostGeneration, RetainedV1DescriptorGraph)>")
        )
        assertOrdered(
            [
                "let retained_v1_guard = verify_retained_v1_user_prestop_attempt(retained_v1_lock)?",
                "Ok((provenance.0, provenance.1, host, retained_v1_guard))",
            ],
            in: complete
        )
        XCTAssertFalse(complete.contains("acquire_retained_v1_user_update_lock"))
        for forbidden in [
        ...
        assertOrdered(
            [
                "let retained_v1_lock = acquire_retained_v1_user_update_lock()?",
                "let (commit, tree, host, _retained_v1_guard) =",
                "verify_complete_preflight(repo, true, &retained_v1_lock)?",
            ],
            in: preflight
        )
        XCTAssertTrue(preflight.contains("stable_coreaudio_generation()?"))
        XCTAssertTrue(preflight.contains("DIAGNOSTIC_DRIVER_V2_PREFLIGHT_OK"))
    }

    func testRetainedV1LockAndPrebuiltReaderGuardAllUserEvidenceMutation() throws {
        let controller = try source(controllerPath)
        let retainedLock = try functionBody(
            controller,
            beginningWith: "fn acquire_retained_v1_user_update_lock(",
            endingBefore: "fn acquire_root_update_lock("
        )
        assertOrdered(
            [
                "Path::new(RETAINED_V1_USER_UPDATE_LOCK)",
                "let (file, before_ancestry) = openat_component_walk_with_final_flags(path, O_RDWR)?",
                "let opened = require_retained_v1_descriptor(",
                ...
                "let named = require_retained_v1_descriptor(",
                "retained v1 named update lock",
                "let reopened = require_retained_v1_descriptor(",
        ...
        XCTAssertFalse(retainedLock.contains(".create("))
        XCTAssertFalse(retainedLock.contains(".create_new("))
        XCTAssertFalse(retainedLock.contains("fs::remove_file"))

        let prebuilt = try functionBody(
            controller,
            beginningWith: "fn verify_prebuilt_diagnostic_reader(",
            endingBefore: "fn require_fresh_namespaces("
        )
        assertOrdered(
            [
                "Path::new(PREBUILT_DIAGNOSTIC_READER)",
                "require_regular(reader, USER_ID, USER_GROUP, 0o755)?",
                "metadata.len() != DIAGNOSTIC_READER_SIZE",
                "sha256(reader)? != DIAGNOSTIC_READER_SHA256",
            ],
            in: prebuilt
        )
        for forbidden in ["fs::copy", "write_", "remove_", "create_"] {
            XCTAssertFalse(prebuilt.contains(forbidden), "prebuilt proof mutates via \(forbidden)")
        }
        ...
        let buildReader = try functionBody(
            controller,
        ...
        XCTAssertEqual(
            occurrences(of: "verify_complete_preflight(repo, true, &retained_v1_lock)?", in: execute),
            1
        )
        XCTAssertEqual(
            occurrences(of: "verify_complete_preflight(repo, false, &retained_v1_lock)?", in: execute),
            1
        )
        XCTAssertEqual(
            occurrences(
                of: "verify_retained_v1_descriptor_graph(",
                in: execute
            ),
            2
        )
        guard
            let finalPreflight = execute.range(
                of: "let (final_commit, final_tree, final_host, final_retained_v1_guard) ="
            ),
            let dispatch = execute.range(
                of: "let output = run_sudo_helper(&root_controller, ROOT_MODE, Some(&root_bootstrap_request))?"
            )
        else {
            return XCTFail("missing final retained-v1 guard or root dispatch boundary")
        }
        let finalGuardWindow = String(execute[finalPreflight.lowerBound..<dispatch.upperBound])
        XCTAssertFalse(finalGuardWindow.contains("retained_v1_root_locator_metadata"))
        XCTAssertFalse(finalGuardWindow.contains("require_retained_v1_root_namespaces_absent"))
        XCTAssertFalse(finalGuardWindow.contains("RETAINED_V1_ROOT_"))
    }

    func testRootRetainedV1AttestationIsJournaledAndReprovedAtStopIntent() throws {
        let controller = try source(controllerPath)
        ...
        for exactField in [
            ...
            XCTAssertTrue(journalValidation.contains(exactField), "root journal omits \(exactField)")
        }
        XCTAssertTrue(journalValidation.contains("require_lower_hex(value, 64, \"retained v1 journal attestation\")"))
        XCTAssertTrue(journalValidation.contains("require_canonical_positive_decimal(value, \"journal number\")"))
        ...
        assertOrdered(
            [
                ...
                "rollback reserve allocation failed and pre-stop finalization failed",
                ...
                "match transaction",
                "Err(update_error) =>",
                "rollback_root_transaction(&layout, &mut journal, &initial, Some(&baseline_route))",
            ],
            in: transaction
        )
        XCTAssertEqual(
            occurrences(of: "verify_retained_v1_root_prestop_attempt()?", in: transaction),
            2
        )
        guard
            let reserveAllocated = transaction.range(
                of: "let reserve_lease = match allocate_rollback_reserve(&layout)"
            ),
            let stopIntent = transaction.range(of: "let mut stop_intent_durable = false")
        else {
            return XCTFail("missing rollback reserve or durable HostStopInitiated boundary")
        }
        let allocatedPrestop = String(transaction[reserveAllocated.lowerBound..<stopIntent.lowerBound])
        XCTAssertTrue(allocatedPrestop.contains("let prestop_revalidation = (|| -> Result<u64> {"))
        XCTAssertTrue(allocatedPrestop.contains("available_bytes_on_transaction_filesystem(&layout)?"))
        XCTAssertTrue(allocatedPrestop.contains("revalidate_root_transaction_ancestry(&root_ancestry)?"))
        XCTAssertTrue(allocatedPrestop.contains("verify_root_bootstrap_locator(&request)?"))
        XCTAssertTrue(allocatedPrestop.contains("verify_retained_v1_root_prestop_attempt()?"))
        XCTAssertTrue(allocatedPrestop.contains("if retained_v1_again != retained_v1"))
        XCTAssertTrue(allocatedPrestop.contains("verify_rollback_reserve_lease(&layout, &reserve_lease)?"))
        XCTAssertTrue(allocatedPrestop.contains("let available_bytes = match prestop_revalidation"))
        XCTAssertEqual(
            occurrences(of: "abort_prestop_after_reserve(", in: allocatedPrestop),
            1,
            "every post-allocation pre-stop revalidation failure must converge on durable abort"
        )
        let afterStopIntent = String(transaction[stopIntent.lowerBound...])
        XCTAssertFalse(afterStopIntent.contains("verify_retained_v1"))
        XCTAssertFalse(afterStopIntent.contains("RETAINED_V1_"))

        let abort = try functionBody(
            controller,
            beginningWith: "fn abort_prestop_after_reserve(",
        ...
        XCTAssertFalse(
            abort.contains("return "),
            "cleanup errors must aggregate without skipping later PRESTOP_ABORTED finalization attempts"
        )

        let trackedState = try functionBody(
            controller,
    ...
    func testRollbackAndRecoveryNeverDependOnRetainedV1AfterStop() throws {
        let controller = try source(controllerPath)
        let recoveryBodies = [
            try functionBody(
                controller,
        ...
        for body in recoveryBodies {
            XCTAssertFalse(body.contains("verify_retained_v1"))
            XCTAssertFalse(body.contains("acquire_retained_v1"))
            XCTAssertFalse(body.contains("RETAINED_V1_"))
        }
        let publicRollback = recoveryBodies[1]
        XCTAssertTrue(publicRollback.contains("ROOT_SEALED_ROLLBACK_MODE"))
        XCTAssertTrue(publicRollback.contains("run_sudo_helper("))
        let rootRecovery = recoveryBodies[2]
        XCTAssertTrue(rootRecovery.contains("RootRecoveryPlan::ResumeRollback"))
        XCTAssertTrue(rootRecovery.contains("rollback_root_transaction("))
    }
    ...
    func testPartialBootstrapAndAtomicRootPointerRecoveryPreserveHost() throws {
        ...
        for token in [
            ...
            "DIAGNOSTIC_DRIVER_V2_BOOTSTRAP_INCOMPLETE_HOST_PRESERVED root_pointer=absent",
            "exit 75",
        ] {
    ...
    func testRootShasumParserAcceptsExactPathsWithSpaces() throws {
        let controller = try source(controllerPath)
        let parser = try functionBody(
            controller,
            beginningWith: "fn parse_root_shasum_output(",
            endingBefore: "fn sudo_root_sha256("
        )
        let rootHash = try functionBody(
            controller,
            beginningWith: "fn sudo_root_sha256(",
            endingBefore: "fn sudo_root_require_no_acl_or_xattrs("
        )
        for token in [
            #"format!("  {}\n", path_text(path)?)"#,
            "bytes.len() != 64 + suffix.len()",
            "&bytes[64..] != suffix.as_bytes()",
            "std::str::from_utf8(&bytes[..64])",
            "require_lower_hex(digest, 64, \"restricted root-owned SHA-256\")",
        ] {
            XCTAssertTrue(parser.contains(token), "root shasum parser omits \(token)")
        }
        XCTAssertFalse(parser.contains("split_ascii_whitespace"))
        XCTAssertTrue(rootHash.contains("parse_root_shasum_output(&text, path)"))
        for fixture in [
            "root shasum path with spaces did not parse exactly",
            "root shasum single separator",
            "root shasum wrong path",
            "root shasum trailing output",
            "root shasum uppercase digest",
        ] {
            XCTAssertTrue(controller.contains(fixture), "root shasum self-test omits \(fixture)")
        }
    }
    ...
    func testRollbackReserveHeadroomAndZeroBlockReleaseAreDurable() throws {
        let controller = try source(controllerPath)
        let releaseDescriptor = try functionBody(
            controller,
            beginningWith: "fn release_rollback_reserve_descriptor(",
            endingBefore: "fn allocate_rollback_reserve("
        )
        let allocate = try functionBody(
            controller,
            beginningWith: "fn allocate_rollback_reserve(",
            endingBefore: "fn verify_rollback_reserve_lease("
        )
        let verifyLease = try functionBody(
            controller,
            beginningWith: "fn verify_rollback_reserve_lease(",
        ...
        XCTAssertTrue(controller.contains("struct RollbackReserveLease {\n    pin: RollbackReserve,\n    file: File,"))
        assertOrdered(
            [
                "let mut failures = Vec::new()",
                ...
                "named reserve differs from its exact descriptor",
                "if !failures.is_empty()",
            ],
        ...
        XCTAssertFalse(allocate.contains("create_private_file"))
        for token in [
            "let opened = lease.file.metadata()?", "lease.pin.released",
            "opened.dev() != lease.pin.device", "opened.ino() != lease.pin.inode",
            "named.dev() != lease.pin.device", "named.ino() != lease.pin.inode",
            "rollback_reserve_released(opened.len(), allocated)",
        ] {
            XCTAssertTrue(verifyLease.contains(token), "reserve lease proof omits \(token)")
        }
        for token in [
        ...
        assertOrdered(
            [
                ...
                "journal.record(\n                UpdateState::HostStopInitiated",
                "reserve_lease.pin.device", "reserve_lease.pin.inode", "reserve_bytes",
                "stop_exact_v8_host(&initial)?",
            ],
            in: transaction
        )
    ...
    func testRootRecoveryOutcomeIsImmutableAndUserResultIsCreateOnce() throws {
        ...
        for token in [
            "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V2",
        ...
        for token in [
            "OPENSTEAMER_DIAGNOSTIC_DRIVER_RESULT_V2",
            "status={status}",
    ...
    func testHostileParserStateAndResumeFixturesRemainInPureSelfTest() throws {
        ...
        for fixture in [
            ...
            "Committed",
            "RolledBack",
            "DIAGNOSTIC_DRIVER_V2_SELF_TEST_OK tests=106",
        ] {
            XCTAssertTrue(
    ...
    func testPureControllerSelfTestPassesWithoutLiveModes() throws {
        ...
        process.arguments = ["--self-test-diagnostic-driver-v2-update"]
        ...
        XCTAssertEqual(output, "DIAGNOSTIC_DRIVER_V2_SELF_TEST_OK tests=106\n")
