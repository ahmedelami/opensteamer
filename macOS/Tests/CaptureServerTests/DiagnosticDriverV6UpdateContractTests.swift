...
final class DiagnosticDriverV6UpdateContractTests: XCTestCase {
    ...
    private let launcherPath = "macOS/scripts/update-opensteamer-diagnostic-driver-v6.sh"
    ...
    func testV6NamespacesModesAndDedicatedReleaseCheckoutAreExact() throws {
        ...
        for token in [
            ...
            "/diagnostic-driver-updates-v6", "/active-diagnostic-driver-update-v6",
            "/diagnostic-driver-update-v6.lock", "/diagnostic-driver-controllers-v6",
            "/diagnostic-driver-bootstrap-v6.txt", "/diagnostic-driver-probes-v6",
        ] {
            XCTAssertTrue(freshConstants.contains(token), "missing fresh V6 token: \(token)")
        }
        for version in 1 ... 5 {
            for staleToken in [
                ...
                XCTAssertFalse(
                    ...
                    "fresh V6 constants retain stale namespace: \(staleToken)"
                )
            }
        }
    ...
    func testRetainedV1ThroughV5SourcesAndEvidenceRemainImmutable() throws {
    ...
    func testRetainedV4GuardsAreHeldThroughPreflightAndRootDispatch() throws {
        let controller = try source(controllerPath)
        XCTAssertTrue(controller.contains("retained_v1_v2_v3_v4_v5=immutable"))
        XCTAssertFalse(controller.contains("retained_v1_v2_v3_v4=immutable"))
    ...
    func testRetainedV5LocksSpanForwardAndSealedRecoveryLifetimes() throws {
        ...
        assertOrdered([
            ...
            "DIAGNOSTIC_DRIVER_V6_UPDATE_COMMITTED",
        ], in: rootEntrypoint)
        ...
        assertOrdered([
            ...
            #""retained v5 root boundary changed across the V6 cutover/rollback""#,
            "outcome",
        ], in: rootTransaction)
        XCTAssertGreaterThanOrEqual(
            rootTransaction.components(separatedBy: "verify_retained_v5_root_rolled_back(&retained_v5_root_lock)?").count - 1,
    ...
    func testRealUmask077CreationAndFinalPublicationModeMatrix() throws {
        let temporaryRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("opensteamer-v6-mode-matrix-\(UUID().uuidString)", isDirectory: true)
    ...
    func testLauncherHasFreshV6PinsAndPureSelfTestOnly() throws {
        let controller = try source(controllerPath)
        let launcher = try source(launcherPath)
        ...
        XCTAssertTrue(launcher.contains("/reviewed/opensteamer-diagnostic-driver-v6"))
    ...
    func testPureControllerSelfTestPassesWithoutLiveModes() throws {
        ...
        process.arguments = ["--self-test-diagnostic-driver-v6-update"]
        ...
        XCTAssertEqual(output, "DIAGNOSTIC_DRIVER_V6_SELF_TEST_OK tests=112\n")
