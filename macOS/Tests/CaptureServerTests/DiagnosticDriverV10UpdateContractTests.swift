...
final class DiagnosticDriverV10UpdateContractTests: XCTestCase {
    ...
    private let launcherPath = "macOS/scripts/update-opensteamer-diagnostic-driver-v10.sh"
    ...
    func testV10UsesFreshNamespaceAndRetainsExactV8ReleaseBytes() throws {
        ...
        for token in [
            ...
            "/diagnostic-driver-updates-v10", "/active-diagnostic-driver-update-v10",
            "/diagnostic-driver-update-v10.lock", "/diagnostic-driver-controllers-v10",
            "/diagnostic-driver-bootstrap-v10.txt", "/diagnostic-driver-probes-v10",
        ] {
            XCTAssertTrue(freshConstants.contains(token), "missing fresh V10 token: \(token)")
        }
    ...
    func testV10RetainsExactV9ReleaseBytesAndVerifiesThemBeforeTheCandidate() throws {
        let controller = try source(controllerPath)
        let launcher = try source(launcherPath)
        let retainedFiles: [(String, Int, String)] = [
            (
    ...
    func testRetainedV8UserGraphPinsAllElevenTerminalNodes() throws {
        ...
        XCTAssertFalse(openGraph.contains(#"b"diagnostic-driver-update-v10.lock""#))
    ...
    func testRetainedV8AndV9LocksSpanPreflightForwardAndSealedRecoveryLifetimes() throws {
        let controller = try source(controllerPath)
        ...
        assertOrdered([
            ...
            "retained_v1_v2_v3_v4_v5_v6_v7_v8_v9=immutable",
        ], in: preflight)
        ...
        assertOrdered([
            ...
            "acquire_retained_v7_user_update_lock()?",
            "acquire_retained_v8_user_update_lock()?",
            "acquire_retained_v9_user_update_lock()?", "let _lock = acquire_user_update_lock()?",
        ...
        assertOrdered([
            ...
            #""retained v8 root boundary changed across the V10 cutover/rollback""#,
            "verify_retained_v9_root_prestop_attempt(&retained_v9_root_lock)? != retained_v9",
            #""retained v9 root boundary changed across the V10 cutover/rollback""#,
        ], in: rootTransaction)
        ...
        for fixtureToken in [
            ...
            XCTAssertTrue(controller.contains(fixtureToken), "missing V9 journal-schema fixture: \(fixtureToken)")
        }
    }
    ...
    func testRootSupervisorContainsSplitPGIDsDefersCancellationAndRecoversBeforeReporting() throws {
        ...
        assertOrdered([
            ...
            "output.status.success()", "DIAGNOSTIC_DRIVER_V10_UPDATE_COMMITTED self-test-survivor=true",
    ...
    func testRouteTranscriptIsCanonicalDurableBoundedAndPrivacySafe() throws {
        ...
        assertOrdered([
            ...
            "!text.ends_with('\\n')", "text.contains('\\r')",
            #"Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROUTE_PROOF_V10")"#,
    ...
    func testDisplayRecoveryRepairsWrongReviewedModeAndAuthenticatesEveryAbortDispatchBoundary() throws {
        ...
        assertOrdered([
            ...
            "DIAGNOSTIC_DRIVER_V10_ROOT_PRESTOP_ABORTED host_pid={} display_mode={}",
            "display_mode_identity_token(&host.display_mode)",
        ], in: pointerless)
    ...
    func testV10PureSelfTestExecutesTheNewRouteAndAquaSemantics() throws {
        let controller = try source(controllerPath)
        let launcher = try source(launcherPath)
        ...
        XCTAssertTrue(launcher.contains("/reviewed/opensteamer-diagnostic-driver-v10"))
        ...
        process.arguments = [
            ...
            "--self-test-diagnostic-driver-v10-update",
        ]
        ...
        XCTAssertNotNil(
            output.range(
                of: #"\ADIAGNOSTIC_DRIVER_V10_SELF_TEST_OK tests=[1-9][0-9]*\n\z"#,
