...
final class DiagnosticDriverV13UpdateContractTests: XCTestCase {
    ...
    private let launcherPath = "macOS/scripts/update-opensteamer-diagnostic-driver-v13.sh"
    ...
    private let retainedV12SourcePath =
        "macOS/scripts/opensteamer-diagnostic-driver-v12-update-controller.rs"
    private let retainedV12LauncherPath =
        "macOS/scripts/update-opensteamer-diagnostic-driver-v12.sh"
    private let retainedV12ContractPath =
        "macOS/Tests/CaptureServerTests/DiagnosticDriverV12UpdateContractTests.swift"
    private let probeSourcePath =
        "iOS/opensteamer/scripts/physical-blackhole-microphone-probe.swift"
    private let v13ProbePath =
        "/Users/ahmed/Library/Application Support/opensteamer/reviewed-diagnostic-probes-v13/physical-virtual-microphone-probe"
    ...
    func testV13UsesFreshNamespaceAndRetainsExactV8ReleaseBytes() throws {
        ...
        for token in [
            #"const EXPECTED_RELEASE_BRANCH: &str = "fix/diagnostic-driver-v13-local-probe-source";"#,
            #"const EXPECTED_UPDATER_BASE_COMMIT: &str = "b4604751162cc96d44173eeb374500ea6ff1130a";"#,
            #"const EXPECTED_UPDATER_BASE_TREE: &str = "73912e0985f8c8370b3e8abbb43cf3610f71ff2f";"#,
            #"const PREFLIGHT_MODE: &str = "--verify-diagnostic-driver-v13-update-preflight";"#,
            #"const EXECUTE_MODE: &str = "--execute-authorized-diagnostic-driver-v13-update";"#,
            #"const ROLLBACK_MODE: &str = "--rollback-authorized-diagnostic-driver-v13-update";"#,
            #"const SELF_TEST_MODE: &str = "--self-test-diagnostic-driver-v13-update";"#,
            #"const ROOT_MODE: &str = "--root-authorized-diagnostic-driver-v13-update";"#,
            #"const ROOT_ROLLBACK_MODE: &str = "--root-rollback-diagnostic-driver-v13-update";"#,
            #"const ROOT_SEALED_ROLLBACK_MODE: &str = "--root-sealed-rollback-diagnostic-driver-v13-update";"#,
            #"const JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_UPDATE_V13";"#,
            #"const ROOT_JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_UPDATE_V13";"#,
            ...
            XCTAssertTrue(freshConstants.contains(token), "missing fresh V13 token: \(token)")
        }
        XCTAssertFalse(freshConstants.contains("diagnostic-driver-v8-host-lock"))
        XCTAssertFalse(freshConstants.contains("diagnostic-driver-updates-v12"))
        XCTAssertFalse(freshConstants.contains("diagnostic-driver-controllers-v12"))
        XCTAssertFalse(freshConstants.contains("reviewed-driver-candidates-v12"))
        for candidate in [
            #"const CANDIDATE_ROOT: &str = "/Users/ahmed/Library/Application Support/opensteamer/reviewed-driver-candidates-v13/production-driver-v7";"#,
            #"const CANDIDATE_DRIVER: &str = "/Users/ahmed/Library/Application Support/opensteamer/reviewed-driver-candidates-v13/production-driver-v7/OpensteamerVirtualMicrophone.driver";"#,
            #"const CANDIDATE_PACKAGE: &str = "/Users/ahmed/Library/Application Support/opensteamer/reviewed-driver-candidates-v13/production-driver-v7/OpensteamerVirtualMicrophone-v7.pkg";"#,
            #"const CANDIDATE_MANIFEST: &str = "/Users/ahmed/Library/Application Support/opensteamer/reviewed-driver-candidates-v13/production-driver-v7/candidate-manifest.txt";"#,
        ] {
            XCTAssertTrue(controller.contains(candidate), "missing fresh V13 candidate path: \(candidate)")
        }
        XCTAssertFalse(controller.contains("reviewed-driver-candidates-v11/production-driver-v7"))
        XCTAssertTrue(controller.contains(
            "DIAGNOSTIC_DRIVER_V13_PREFLIGHT_OK host_pid={} host_runs={} display_mode={} coreaudiod_pid={} coreaudiod_runs={} installed_driver={}:{} candidate_commit={} candidate_tree={} release_commit={} release_tree={} pairing=metadata-only legacy=protected retained_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12=immutable reader=passive both_order=no-default-mutation namespaces=fresh"
        ))
        XCTAssertTrue(controller.contains(
            "DIAGNOSTIC_DRIVER_V13_UPDATE_COMMITTED host_pid={} host_runs={} display_mode={} pairing=preserved routes=unchanged legacy=protected retained_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12=immutable namespaces=fresh"
        ))
        XCTAssertTrue(controller.contains(
            "DIAGNOSTIC_DRIVER_V13_UPDATE_COMPLETE evidence={} host_pid={} display_mode={} pairing=preserved routes=unchanged legacy=protected retained_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12=immutable namespaces=fresh"
        ))
        ...
        for privateMode in [
            ...
            "UID501_RETAINED_V8_BOUNDARY_MODE", "UID501_RETAINED_V12_BOUNDARY_MODE",
        ] {
            XCTAssertFalse(publicDispatch.contains(privateMode), "launcher exposes \(privateMode)")
        }
    }

    func testV13ProvesEveryStaticRootStagingSourceThroughExactUID501OpenatHelper() throws {
        let controller = try source(controllerPath)

        for token in [
            #"const BOTH_ORDER_PROBE_PARENT: &str ="#,
            #""/Users/ahmed/Library/Application Support/opensteamer/reviewed-diagnostic-probes-v13""#,
            #"const BOTH_ORDER_PROBE: &str = "/Users/ahmed/Library/Application Support/opensteamer/reviewed-diagnostic-probes-v13/physical-virtual-microphone-probe";"#,
        ] {
            XCTAssertTrue(controller.contains(token), "missing sealed local V13 probe pin: \(token)")
        }
        XCTAssertFalse(controller.contains(
            "/Volumes/t7/opensteamer-diagnostic-driver-v13-probe-build/physical-virtual-microphone-probe"
        ))

        let readerInputs = try functionBody(
            controller,
            beginningWith: "fn verify_reader_inputs(",
            endingBefore: "fn verify_root_staging_static_source_reads("
        )
        assertOrdered([
            ...
            "require_regular(probe, USER_ID, USER_GROUP, 0o755)?",
            "require_no_acl_or_xattrs(probe)?",
            "probe_metadata.len() != BOTH_ORDER_PROBE_SIZE",
            "probe_metadata.nlink() != 1", "probe_metadata.st_flags() != 0",
            "sha256(probe)? != BOTH_ORDER_PROBE_SHA256",
        ], in: readerInputs)

        let staticSources = try functionBody(
            controller,
            beginningWith: "fn verify_root_staging_static_source_reads() -> Result<()> {",
            endingBefore: "fn verify_generated_root_staging_reader("
        )
        for token in [
            "let candidate = Path::new(CANDIDATE_DRIVER)",
            #""Contents/Info.plist""#, "CANDIDATE_INFO_PLIST_SHA256",
            #""Contents/MacOS/OpensteamerVirtualMicrophone""#,
            "CANDIDATE_DRIVER_EXECUTABLE_SHA256",
            #""Contents/Resources/APPLE_SAMPLE_LICENSE.txt""#, "CANDIDATE_LICENSE_SHA256",
            #""Contents/Resources/en.lproj/Localizable.strings""#,
            "CANDIDATE_LOCALIZABLE_SHA256",
            #""Contents/_CodeSignature/CodeResources""#, "CANDIDATE_CODE_RESOURCES_SHA256",
            "for (relative, mode, size, digest) in",
            "read_uid501_openat_bytes(", "&candidate.join(relative)",
            "mode,", "USER_GROUP,", "size,", "Some(digest),",
            "Path::new(BOTH_ORDER_PROBE)", "0o755", "USER_GROUP",
            "BOTH_ORDER_PROBE_SIZE", "Some(BOTH_ORDER_PROBE_SHA256)",
        ] {
            XCTAssertTrue(staticSources.contains(token), "static openat proof omits: \(token)")
        }
        XCTAssertEqual(occurrences(of: "read_uid501_openat_bytes(", in: staticSources), 2)
        for forbidden in ["fs::copy(", "fs::read(", "File::open(", "Command::new("] {
            XCTAssertFalse(
                staticSources.contains(forbidden),
                "static source preflight bypasses the exact UID501 openat helper: \(forbidden)"
            )
        }
    ...
    func testV13ExecuteOpenatProvesFreshEvidenceReaderBeforeRootDispatch() throws {
        let controller = try source(controllerPath)
        let evidenceReaderProof = try functionBody(
            controller,
            beginningWith: "fn verify_generated_root_staging_reader(path: &Path) -> Result<()> {",
            endingBefore: "fn require_retained_descriptor("
        )
        assertOrdered([
            "read_uid501_openat_bytes(", "path", "0o755", "USER_GROUP",
            "DIAGNOSTIC_READER_SIZE", "Some(DIAGNOSTIC_READER_SHA256)",
        ], in: evidenceReaderProof)
        XCTAssertEqual(
            occurrences(of: "read_uid501_openat_bytes(", in: evidenceReaderProof),
            1
        )
        for forbidden in ["fs::copy(", "fs::read(", "File::open(", "Command::new("] {
            XCTAssertFalse(
                evidenceReaderProof.contains(forbidden),
                "fresh reader proof bypasses the exact UID501 openat helper: \(forbidden)"
            )
        }

        let builder = try functionBody(
            controller,
            beginningWith: "fn build_diagnostic_reader(",
            endingBefore: "struct Journal {"
        )
        assertOrdered([
            "fs::copy(built, destination)?", "fs::set_permissions(",
            "require_regular(destination, USER_ID, USER_GROUP, 0o755)?",
            "sha256(destination)? != DIAGNOSTIC_READER_SHA256", "fsync_parent(destination)?",
            "require_no_acl_or_xattrs(destination)?",
            "verify_generated_root_staging_reader(destination)?", "Ok(())",
        ], in: builder)
    ...
    func testV13RetainsExactV9ReleaseBytesAndVerifiesThemBeforeTheCandidate() throws {
        let controller = try source(controllerPath)
        let launcher = try source(launcherPath)
        let retainedFiles: [(String, Int, String)] = [
            (
    ...
    func testRetainedV8UserGraphPinsAllElevenTerminalNodes() throws {
        ...
        XCTAssertFalse(openGraph.contains(#"b"diagnostic-driver-update-v13.lock""#))
    ...
    func testRetainedV9RootGraphAndLockPinEveryKnownNodeAndAbsence() throws {
        ...
        XCTAssertEqual(occurrences(of: "node(", in: identities), 10)
    ...
    func testRetainedV10ReleaseUserAndRootGraphsAreExactAndImmutable() throws {
        ...
        let childSets = try functionBody(
            rootProof,
            beginningWith: "let child_sets = vec![",
            endingBefore: "for (path, expected, label) in child_sets"
        )
    ...
    func testRetainedV12PrestopAbortedUserGraphHasExactlyElevenImmutableNodes() throws {
        let controller = try source(controllerPath)
        let retainedFiles: [(String, Int, String)] = [
            (
                retainedV12SourcePath,
                1_123_499,
                "ba8169bf5e460eeafd9b611e3a30c814d2494c57109c90cb206bdc98b135dbf9"
            ),
            (
                retainedV12LauncherPath,
                18_689,
                "55404bb5527ac150a834fb78cdcf8e481159ca026c2d054e1db5706bace03a07"
            ),
            (
                retainedV12ContractPath,
                220_634,
                "cec7220d5873deb43ce502fca173d804971d740930659cb72773b379d6961bae"
            ),
            (
                probeSourcePath,
                388_802,
        ...
        for (path, size, digest) in retainedFiles {
            let bytes = try data(path)
            XCTAssertEqual(bytes.count, size, "retained V12 file size changed: \(path)")
            XCTAssertEqual(sha256Hex(bytes), digest, "retained V12 file bytes changed: \(path)")
        }
        for token in [
            #"const RETAINED_V12_SOURCE_COMMIT: &str = "b4604751162cc96d44173eeb374500ea6ff1130a";"#,
            #"const RETAINED_V12_SOURCE_TREE: &str = "73912e0985f8c8370b3e8abbb43cf3610f71ff2f";"#,
            #"const RETAINED_V12_SOURCE_SHA256: &str ="#,
            #""ba8169bf5e460eeafd9b611e3a30c814d2494c57109c90cb206bdc98b135dbf9""#,
            #"const RETAINED_V12_CONTROLLER_BINARY_SHA256: &str ="#,
            #""1d970782911acbceb38fadcbd4f5d10ca2cc23022df72addcad98b30448a82bb""#,
            "OPENSTEAMER_DIAGNOSTIC_DRIVER_UPDATE_V12",
            "STATE AUTHENTICATED nonce=5439014f408527b6d4ba661d70c89b42",
            "status=failed",
            "UID501 openat component metadata is unsafe",
        ] {
            XCTAssertTrue(controller.contains(token), "missing retained V12 terminal pin: \(token)")
        }

        let release = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v12_release_files(",
            endingBefore: "// V12 aborted before host stop. Its complete 11-node user graph is immutable."
        )
        assertOrdered([
            "RETAINED_V12_SOURCE_SIZE", "RETAINED_V12_SOURCE_SHA256",
            "RETAINED_V12_LAUNCHER_SIZE", "RETAINED_V12_LAUNCHER_SHA256",
            "RETAINED_V12_CONTRACT_TEST_SIZE", "RETAINED_V12_CONTRACT_TEST_SHA256",
            "RETAINED_V12_PROBE_SOURCE_SIZE", "RETAINED_V12_PROBE_SOURCE_SHA256",
            #"&format!("{RETAINED_V12_SOURCE_COMMIT}^{{tree}}")"#,
            "tree != RETAINED_V12_SOURCE_TREE", "for (relative, size, mode, digest, label) in specs",
            "require_regular(&path, USER_ID, USER_GROUP, mode)?",
            "require_no_acl_or_xattrs(&path)?", "before.len() != size",
            "sha256(&path)? != digest", #"&["show", &object]"#,
            "sha256_bytes(&output.stdout)? != digest",
            "identity_from_metadata(&before) != identity_from_metadata(&after)",
        ], in: release)
        ...
        let identities = try functionBody(
            controller,
            beginningWith: "fn retained_v12_descriptor_graph_identities(",
            endingBefore: "fn verify_retained_v12_descriptor_graph_payload("
        )
        for token in [
            "RETAINED_V12_USER_UPDATE_ROOT_INODE", "RETAINED_V12_EVIDENCE_INODE",
            "RETAINED_V12_PROBES_INODE", "RETAINED_V12_USER_ACTIVE_POINTER_INODE",
            "RETAINED_V12_JOURNAL_INODE", "RETAINED_V12_REQUEST_INODE",
            "RETAINED_V12_READER_INODE", "RETAINED_V12_CONTROLLER_PIN_INODE",
            "RETAINED_V12_CONTROLLER_IDENTITY_INODE", "RETAINED_V12_RESULT_INODE",
            "RETAINED_V12_USER_UPDATE_LOCK_INODE",
            "identity_from_metadata(&held)", "identities.last() != Some(&held_identity)",
        ] {
            XCTAssertTrue(identities.contains(token), "V12 11-node user graph omits: \(token)")
        }
        let identitySpecs = try functionBody(
            identities,
            beginningWith: "let specs = [",
            endingBefore: "let mut identities ="
        )
        XCTAssertEqual(occurrences(of: "&graph.", in: identitySpecs), 11)

        let payload = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v12_descriptor_graph_payload(",
            endingBefore: "fn verify_retained_v12_descriptor_graph("
        )
        assertOrdered([
            "RETAINED_V12_EVIDENCE_LEAF.as_bytes()",
            #"b"controller-identity.txt""#, #"b"controller.sha256""#,
            #"b"journal.log""#, #"b"opensteamer-diagnostic-snapshot-reader""#,
            #"b"probes""#, #"b"result.txt""#, #"b"root-request.txt""#,
            "require_retained_descriptor_children(&graph.probes, &[]",
            "RETAINED_V12_USER_ACTIVE_POINTER_PENDING",
            #"Path::new(RETAINED_V12_EVIDENCE).join("journal.log.pending")"#,
            #"Path::new(RETAINED_V12_EVIDENCE).join("recovery-result.txt")"#,
            #"pointer != format!("{RETAINED_V12_EVIDENCE}\n").as_bytes()"#,
            "journal != RETAINED_V12_JOURNAL_TEXT.as_bytes()",
            "request != RETAINED_V12_REQUEST_TEXT.as_bytes()",
            #"pin != format!("{RETAINED_V12_CONTROLLER_BINARY_SHA256}\n").as_bytes()"#,
            "identity != RETAINED_V12_CONTROLLER_IDENTITY_TEXT.as_bytes()",
            "result != RETAINED_V12_RESULT_TEXT.as_bytes()",
            "RETAINED_V12_USER_ACTIVE_POINTER_SHA256", "RETAINED_V12_JOURNAL_SHA256",
            "RETAINED_V12_REQUEST_SHA256", "DIAGNOSTIC_READER_SHA256",
            "RETAINED_V12_CONTROLLER_PIN_SHA256", "RETAINED_V12_CONTROLLER_IDENTITY_SHA256",
            "RETAINED_V12_RESULT_SHA256",
        ], in: payload)

        let graphProof = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v12_descriptor_graph(",
            endingBefore: "fn verify_retained_v12_user_prestop_aborted_once("
        )
        assertOrdered([
            "retained_v12_descriptor_graph_identities(graph, held_lock)?",
            "verify_retained_v12_descriptor_graph_payload(graph)?",
            "retained_v12_descriptor_graph_identities(graph, held_lock)?",
            "verify_retained_v12_descriptor_graph_payload(graph)?",
            "retained_v12_descriptor_graph_identities(graph, held_lock)?",
        ...
        let stableProof = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v12_user_prestop_aborted(",
            endingBefore: "#[derive(Clone, Debug, Eq, PartialEq)]\nstruct RetainedV1RootAttestation"
        )
        assertOrdered([
            "verify_retained_v12_user_prestop_aborted_once(held_lock)?",
            "thread::sleep(Duration::from_millis(50))",
            "verify_retained_v12_user_prestop_aborted_once(held_lock)?",
            "verify_retained_v12_descriptor_graph(&first, held_lock)?",
            "verify_retained_v12_descriptor_graph(&second, held_lock)?",
        ], in: stableProof)

        for immutableProof in [release, graphProof, stableProof] {
            for forbidden in [
                "write_new_private(", "write_atomic_replace(", "fs::remove_file(",
                "fs::remove_dir(", ".create(true)", ".truncate(true)",
            ] {
                XCTAssertFalse(
                    immutableProof.contains(forbidden),
                    "retained V12 user proof gained a mutation surface: \(forbidden)"
                )
            }
        }
    }

    func testRetainedV12PrestopAbortedRootGraphHasExactlyThirtyFiveImmutableNodes() throws {
        let controller = try source(controllerPath)
        let specs = try functionBody(
            controller,
            beginningWith: "fn retained_v12_root_node_specs()",
            endingBefore: "fn validate_retained_v12_root_identity("
        )
        XCTAssertEqual(occurrences(of: "        file(", in: specs), 19)
        XCTAssertEqual(occurrences(of: "        directory(", in: specs), 16)
        for token in [
            "RETAINED_V12_ROOT_BOOTSTRAP_LOCATOR", "RETAINED_V12_ROOT_CONTROLLER_PARENT",
            "RETAINED_V12_ROOT_PROBE_PARENT", "RETAINED_V12_ROOT_UPDATE_ROOT",
            "RETAINED_V12_ROOT_UPDATE_LOCK_INODE", "RETAINED_V12_ROOT_ACTIVE_POINTER_SHA256",
            "RETAINED_V12_ROOT_JOURNAL_SHA256", "RETAINED_V12_ROOT_STATE_SHA256",
            "RETAINED_V12_ROOT_RESULT_SHA256", "RETAINED_V12_REQUEST_SHA256",
            "RETAINED_V12_CONTROLLER_BINARY_SIZE", "RETAINED_V12_CONTROLLER_BINARY_SHA256",
            "RETAINED_V12_CONTROLLER_PIN_SHA256", "RETAINED_V12_CONTROLLER_IDENTITY_SHA256",
            "DIAGNOSTIC_READER_SIZE", "DIAGNOSTIC_READER_SHA256",
            #"transaction.join("candidate-stage")"#,
            #"transaction.join("prior-driver")"#,
            #"transaction.join("failed-driver")"#,
            #"transaction.join("probes")"#,
            #"transaction.join("journal.log")"#,
            #"transaction.join("state.txt")"#,
            #"transaction.join("result.txt")"#,
        ] {
            XCTAssertTrue(specs.contains(token), "V12 35-node root manifest omits: \(token)")
        }

        let rootProof = try functionBody(
            controller,
            beginningWith: "fn verify_retained_v12_root_graph_once(",
            endingBefore: "fn uid501_verify_retained_v12_root_prestop_aborted("
        )
        assertOrdered([
            "let specs = retained_v12_root_node_specs()",
            "if specs.len() != 35",
            "retained v12 root manifest does not contain exactly 35 nodes",
            "collect_retained_v12_root_node_identities(restricted_uid501, held_root_lock)?",
            "let child_sets = vec![", "for (path, expected, label) in child_sets",
            "RETAINED_V12_ROOT_ACTIVE_POINTER_PENDING",
            #"transaction.join("journal.log.pending")"#,
            #"transaction.join("prestop-abort-journal.txt")"#,
            #"transaction.join("recovery-result.txt")"#,
            #"transaction.join("rollback-reserve.bin")"#,
            #"probe_transaction.join("physical-virtual-microphone-probe")"#,
            "hash_retained_v4_root_file", "read_retained_v4_root_file",
            ...
            "retained v12 PRESTOP_ABORTED root semantics changed",
            "collect_retained_v12_root_node_identities(restricted_uid501, held_root_lock)?",
            "if before != after",
        ], in: rootProof)
        XCTAssertTrue(controller.contains(
        ...
        let stableRootProof = try functionBody(
            controller,
            beginningWith: "fn uid501_verify_retained_v12_root_prestop_aborted(",
            endingBefore: "fn verify_prebuilt_diagnostic_reader("
        )
        assertOrdered([
            "verify_retained_v12_root_graph_once(true, None)?",
            "thread::sleep(Duration::from_millis(50))",
            "verify_retained_v12_root_graph_once(true, None)?",
            "fn verify_retained_v12_root_prestop_aborted(",
            "verify_retained_v12_root_graph_once(false, Some(held_root_lock))?",
            "thread::sleep(Duration::from_millis(50))",
            "verify_retained_v12_root_graph_once(false, Some(held_root_lock))?",
        ], in: stableRootProof)

        for immutableProof in [specs, rootProof, stableRootProof] {
            for forbidden in [
                "write_new_private(", "write_atomic_replace(", "fs::remove_file(",
                "fs::remove_dir(", ".create(true)", ".truncate(true)",
            ] {
                XCTAssertFalse(
                    immutableProof.contains(forbidden),
                    "retained V12 root proof gained a mutation surface: \(forbidden)"
                )
            }
        }
    }

    func testRetainedV12GuardsSpanPreflightAndBothSidesOfRootDispatch() throws {
        let controller = try source(controllerPath)
        let completePreflight = try functionBody(
            controller,
            beginningWith: "fn verify_complete_preflight(",
        ...
        assertOrdered([
            ...
            "verify_complete_preflight(", "&retained_v11_lock", "&retained_v12_lock",
            "retained_v1_v2_v3_v4_v5_v6_v7_v8_v9_v10_v11_v12=immutable",
        ], in: preflight)
        ...
        for token in [
            ...
            "verify_retained_v12_descriptor_graph(&dispatch_retained_v12_guard, &retained_v12_lock)?",
            "run_sudo_helper(&root_controller, ROOT_MODE",
            "verify_retained_v12_descriptor_graph(", "&post_dispatch_retained_v12_guard,",
            "retained v12 user boundary changed across root dispatch",
        ] {
            XCTAssertTrue(execute.contains(token), "execute omits V12 immutable guard: \(token)")
        }
        ...
        assertOrdered([
            ...
            "retained v12 root boundary changed across the V13 cutover/rollback",
        ], in: rootTransaction)
        ...
        assertOrdered([
            ...
            "verify_retained_v12_user_prestop_aborted_once(&retained_v12_lock)?",
            "retained v12 user boundary changed immediately before sealed root recovery",
            "run_sudo_helper(", "ROOT_SEALED_ROLLBACK_MODE",
            "let post_recovery_retained_v12 =",
            "verify_retained_v12_user_prestop_aborted_once(&retained_v12_lock)?",
            "verify_retained_v12_descriptor_graph(", "&post_recovery_retained_v12,",
            "retained v12 user boundary changed across sealed root recovery",
        ], in: publicRollback)
        ...
        assertOrdered([
            ...
            "verify_retained_v12_root_prestop_aborted(&retained_v12_root_lock)? != retained_v12",
            "root boundary changed before pointerless recovery finalization",
            "finalize_sealed_bootstrap_without_root_pointer(&fixed_digest)?",
            "verify_retained_v12_root_prestop_aborted(&retained_v12_root_lock)? != retained_v12",
            "retained v12 root boundary changed across pointerless recovery finalization",
            "parse_sealed_root_request(&layout.recovery_request)?",
            "verify_retained_v12_root_prestop_aborted(&retained_v12_root_lock)? != retained_v12",
            "root boundary changed before sealed recovery",
            "complete_root_recovery(request, layout)?",
            "verify_retained_v12_root_prestop_aborted(&retained_v12_root_lock)? != retained_v12",
            "retained v12 root boundary changed across sealed recovery",
        ], in: sealedRecovery)
        ...
        assertOrdered([
            "mode == UID501_RETAINED_V12_BOUNDARY_MODE",
            "acquire_retained_v12_user_update_lock()?",
            "uid501_verify_retained_v12_root_prestop_aborted()?",
            "verify_retained_v12_user_prestop_aborted(&retained_v12_lock)?",
            "OPENSTEAMER_RETAINED_V12_PRESTOP_ABORTED_BOUNDARY_OK",
        ], in: privateDispatch)
        ...
        for token in [
            #""retained_v12_journal_sha256""#, "RETAINED_V12_JOURNAL_SHA256",
            #""retained_v12_root_journal_sha256""#, "RETAINED_V12_ROOT_JOURNAL_SHA256",
            #""retained_v12_result_sha256""#, "RETAINED_V12_RESULT_SHA256",
            #""retained_v12_locator_device""#, "retained_v12.nodes[0].device",
            #""retained_v12_locator_inode""#, "retained_v12.nodes[0].inode",
            #""retained_v12_request_sha256""#, "RETAINED_V12_REQUEST_SHA256",
        ] {
            XCTAssertTrue(journalFields.contains(token), "root journal omits V12 attestation: \(token)")
        }
        ...
        for token in [
            #""retained_v12_journal_sha256""#,
            #""retained_v12_root_journal_sha256""#,
            #""retained_v12_result_sha256""#,
            #""retained_v12_locator_device""#,
            #""retained_v12_locator_inode""#,
            #""retained_v12_request_sha256""#,
        ] {
            XCTAssertTrue(journalSchema.contains(token), "journal schema omits V12 field: \(token)")
        }
    }
    ...
    func testRetainedV8V9V10AndV11LocksSpanPreflightForwardAndSealedRecoveryLifetimes() throws {
        ...
        assertOrdered([
            ...
            #""retained v8 root boundary changed across the V13 cutover/rollback""#,
            "verify_retained_v9_root_prestop_attempt(&retained_v9_root_lock)? != retained_v9",
            #""retained v9 root boundary changed across the V13 cutover/rollback""#,
            "verify_retained_v10_root_rolled_back(&retained_v10_root_lock)? != retained_v10",
            #""retained v10 root boundary changed across the V13 cutover/rollback""#,
        ], in: rootTransaction)
    ...
    func testRetainedV11GuardsSpanEveryPreflightDispatchAndRecoveryBoundary() throws {
        ...
        assertOrdered([
            ...
            #""retained v11 root boundary changed across the V13 cutover/rollback""#,
        ], in: rootTransaction)
    ...
    func testMirrorLoopbackProductionWriterIsDescriptorRelativeDurableAndChecked() throws {
        ...
        let probeBytes = try Data(contentsOf: URL(fileURLWithPath: v13ProbePath))
    ...
    func testPinnedWriterCanaryWorksBeneathNontraversableAncestorAndRejectsMutations() throws {
        ...
        let temporaryRoot = fileManager.temporaryDirectory
            .appendingPathComponent("opensteamer-v13-writer-tests-\(UUID().uuidString)")
        ...
        let nonce = "v13-nontraversable-writer-canary-001"
        let process = Process()
        process.executableURL = URL(fileURLWithPath: "/bin/sh")
        process.arguments = [
            "-c",
            #"cd "$1" || exit 97; chmod 000 "$2" || exit 98; exec "$3" mirror-loopback-writer-canary --nonce "$4" --result both-order.json"#,
            "v13-relative-writer", inheritedWorkingDirectory.path,
            privateAncestor.path, v13ProbePath, nonce,
        ]
        let stdout = Pipe()
        let stderr = Pipe()
        ...
        func runProbe(_ arguments: [String], in directory: URL) throws -> (Int32, Data, Data) {
            let child = Process()
            child.executableURL = URL(fileURLWithPath: v13ProbePath)
        ...
        let invalid = try runProbe([
            "mirror-loopback-writer-canary", "--nonce", "v13-invalid-leaf-canary-001",
        ...
        let collision = try runProbe([
            "mirror-loopback-writer-canary", "--nonce", "v13-collision-canary-001",
            "--result", "both-order.json",
    ...
    func testRootSupervisorContainsSplitPGIDsDefersCancellationAndRecoversBeforeReporting() throws {
        let controller = try source(controllerPath)
        for token in [
            #"const ROOT_SUDO_SUPERVISOR_MODE: &str = "--root-supervise-diagnostic-driver-v13-worker";"#,
        ...
        assertOrdered([
            ...
            "OPENSTEAMER_DIAGNOSTIC_DRIVER_V13_SUDO_SUPERVISOR_SELF_TEST_RECOVERY",
        ...
        assertOrdered([
            ...
            "output.status.success()", "DIAGNOSTIC_DRIVER_V13_UPDATE_COMMITTED self-test-survivor=true",
    ...
    func testRouteTranscriptIsCanonicalDurableBoundedAndPrivacySafe() throws {
        ...
        for token in [
            ...
            "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROUTE_PROOF_V13",
        ...
        assertOrdered([
            ...
            "!text.ends_with('\\n')", "text.contains('\\r')",
            #"Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROUTE_PROOF_V13")"#,
    ...
    func testDisplayRecoveryRepairsWrongReviewedModeAndAuthenticatesEveryAbortDispatchBoundary() throws {
        ...
        assertOrdered([
            ...
            "DIAGNOSTIC_DRIVER_V13_ROOT_PRESTOP_ABORTED host_pid={} display_mode={}",
            "display_mode_identity_token(&host.display_mode)",
        ], in: pointerless)
        ...
        for token in [
            ...
            "\"release_commit\"",
            "\"release_tree\"",
            "authenticated", ".keys()", ".map(String::as_str)",
            ".collect::<BTreeSet<_>>()", "== expected_keys",
    ...
    func testV13PureSelfTestExecutesTheNewRouteAndAquaSemantics() throws {
        let controller = try source(controllerPath)
        let launcher = try source(launcherPath)
        ...
        XCTAssertTrue(launcher.contains("/reviewed/opensteamer-diagnostic-driver-v13"))
        ...
        guard sourcePin == sha256Hex(controller) else {
            throw XCTSkip("V13 final source/binary pins are intentionally not fixed by this contract patch")
        }
        ...
        process.arguments = [
            ...
            "--self-test-diagnostic-driver-v13-update",
        ]
        ...
        {
            throw XCTSkip("V13 final binary pin is intentionally not fixed by this contract patch")
        }
        XCTAssertEqual(process.terminationStatus, 0, error)
        XCTAssertNotNil(
            output.range(
                of: #"\ADIAGNOSTIC_DRIVER_V13_SELF_TEST_OK tests=[1-9][0-9]*\n\z"#,
