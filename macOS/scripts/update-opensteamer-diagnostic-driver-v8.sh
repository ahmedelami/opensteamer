...
PREFLIGHT_MODE='--verify-diagnostic-driver-v8-update-preflight'
EXECUTE_MODE='--execute-authorized-diagnostic-driver-v8-update'
ROLLBACK_MODE='--rollback-authorized-diagnostic-driver-v8-update'
SELF_TEST_MODE='--self-test-diagnostic-driver-v8-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer-diagnostic-v3'
USER_UPDATE_ROOT='/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v8'
USER_ACTIVE_POINTER='/Users/ahmed/Library/Application Support/opensteamer/active-diagnostic-driver-update-v8'
ROOT_SUPPORT='/Library/Application Support/opensteamer'
ROOT_CONTROLLER_PARENT='/Library/Application Support/opensteamer/diagnostic-driver-controllers-v8'
...
ROOT_ACTIVE_POINTER='/Library/Application Support/opensteamer/active-diagnostic-driver-update-v8'
ROOT_BOOTSTRAP_LOCATOR='/Library/Application Support/opensteamer/diagnostic-driver-bootstrap-v8.txt'
DIAGNOSTIC_READER_SHA256='6d3f5d71b399ada1244b84177d351c2406d426055da8f54e8f689af1c22a8ded'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-diagnostic-driver-v8-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='d35f3e4082608c0323efd9c868100318566d418975244478e0f88b3812772229'
EXPECTED_BINARY_SHA256='da20b7efb3fd8399a4c73e9f1e056969799116e5254b4e3f773fdd985aa73fed'
BUILD_PARENT='/Volumes/t7/opensteamer-diagnostic-driver-v8-controller-builds'
...
if [ "$MODE" = "$ROLLBACK_MODE" ]; then
    ...
    if [ ! -e "$ROOT_ACTIVE_POINTER" ] && [ ! -L "$ROOT_ACTIVE_POINTER" ] \
        ...
        if [ -f "$ROOT_BOOTSTRAP_LOCATOR" ] && [ ! -L "$ROOT_BOOTSTRAP_LOCATOR" ] \
            ...
            && [ "$(/usr/bin/sed -n '1p' "$ROOT_BOOTSTRAP_LOCATOR")" = 'OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V8' ]; then
                ...
                *)
                    if [ "${#LOCATOR_NONCE}" -eq 32 ] \
                        ...
                        case "$LOCATOR_EVIDENCE" in
                            "$USER_UPDATE_ROOT"/diagnostic-driver-v8-*"$LOCATOR_NONCE") LOCATOR_READY=1 ;;
                        esac
                    fi
                    ;;
            esac
        ...
        if [ "$FIXED_READY" -eq 0 ]; then
            ...
            echo 'DIAGNOSTIC_DRIVER_V8_BOOTSTRAP_INCOMPLETE_HOST_PRESERVED root_pointer=absent' >&2
            exit 75
        fi
    fi
    [ -d "$ROOT_SUPPORT" ] && [ ! -L "$ROOT_SUPPORT" ] \
    ...
    case "$RECOVERY_OUTPUT" in
        DIAGNOSTIC_DRIVER_V8_RECOVERY_COMPLETE\ outcome=committed\ *) RECOVERY_OUTCOME='committed' ;;
        DIAGNOSTIC_DRIVER_V8_RECOVERY_COMPLETE\ outcome=rolled-back\ *) RECOVERY_OUTCOME='rolled-back' ;;
        DIAGNOSTIC_DRIVER_V8_RECOVERY_COMPLETE\ outcome=prestop-aborted\ *) RECOVERY_OUTCOME='prestop-aborted' ;;
        *)
    ...
    if [ -f "$USER_ACTIVE_POINTER" ] && [ ! -L "$USER_ACTIVE_POINTER" ] \
        ...
        case "$RECOVERY_EVIDENCE" in
            "$USER_UPDATE_ROOT"/diagnostic-driver-v8-*)
...
compile_controller() {
    ...
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        ...
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-diagnostic-driver-v8" \
        --remap-path-prefix "$build_root=/reviewed/opensteamer-diagnostic-driver-v8-build" \
        "$SOURCE" -o "$controller"
