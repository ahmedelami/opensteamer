...
const PREFLIGHT_MODE: &str = "--verify-diagnostic-driver-v4-update-preflight";
const EXECUTE_MODE: &str = "--execute-authorized-diagnostic-driver-v4-update";
const ROLLBACK_MODE: &str = "--rollback-authorized-diagnostic-driver-v4-update";
const SELF_TEST_MODE: &str = "--self-test-diagnostic-driver-v4-update";
const ROOT_MODE: &str = "--root-authorized-diagnostic-driver-v4-update";
const ROOT_ROLLBACK_MODE: &str = "--root-rollback-diagnostic-driver-v4-update";
const ROOT_SEALED_ROLLBACK_MODE: &str = "--root-sealed-rollback-diagnostic-driver-v4-update";
const LOCK_PROBE_MODE: &str = "--probe-diagnostic-driver-v4-host-lock";
const LOCK_FREE_PROBE_MODE: &str = "--probe-diagnostic-driver-v4-host-lock-free";
const LOCK_RECORD_PROBE_MODE: &str = "--read-diagnostic-driver-v4-host-lock-record";
...
const EXPECTED_RELEASE_BRANCH: &str = "fix/diagnostic-driver-v4-current-host";
const EXPECTED_UPDATER_BASE_COMMIT: &str = "cd3b1b0936fb897eb8a0f0d8efcbb118e658eb45";
const EXPECTED_UPDATER_BASE_TREE: &str = "23b69c6db976455ceb663a3dc9642940caf93613";
...
const ROOT_RECOVERY_CONTROLLER_PIN: &str = "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v4/recovery-controller.sha256";
...
const JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_UPDATE_V4";
const ROOT_JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_UPDATE_V4";
...
fn verify_git_provenance(repo: &Path) -> Result<(String, String)> {
    ...
    if branch != EXPECTED_RELEASE_BRANCH {
        return Err(ControllerError(
            "live release checkout is not the exact dedicated V4 branch".to_owned(),
        ));
    }
...
fn preflight(repo: &Path) -> Result<()> {
    ...
    let retained_v3_lock = acquire_retained_v3_user_update_lock()?;
    let (commit, tree, host, _retained_v1_guard, _retained_v2_guard, _retained_v3_guard) =
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V4_PREFLIGHT_OK host_pid={} host_runs={} coreaudiod_pid={} coreaudiod_runs={} installed_driver={}:{} candidate_commit={} candidate_tree={} release_commit={} release_tree={} pairing=metadata-only legacy=protected retained_v1_v2_v3=immutable reader=passive both_order=no-default-mutation namespaces=fresh",
        host.pid,
        host.runs,
        coreaudio.pid,
        coreaudio.runs,
...
fn create_user_layout(nonce: &str) -> Result<UserLayout> {
    ...
    let evidence = Path::new(USER_UPDATE_ROOT).join(format!(
        "diagnostic-driver-v4-{seconds}-{}-{nonce}",
        std::process::id()
    ));
...
fn parse_root_request_text(text: &str) -> Result<RootRequest> {
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V4") {
    ...
    if request.reader_sha256 != DIAGNOSTIC_READER_SHA256
        ...
        || !request
            ...
            .and_then(OsStr::to_str)
            .is_some_and(|leaf| {
                leaf.starts_with("diagnostic-driver-v4-") && leaf.ends_with(&request.nonce)
            })
...
fn parse_root_state_text(
    ...
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_STATE_V4") || !text.ends_with('\n')
    {
        return Err(ControllerError(
            ...
            .is_some_and(|evidence| {
                evidence.parent() == Some(Path::new(USER_UPDATE_ROOT))
                    && evidence
                        .file_name()
                        .and_then(OsStr::to_str)
                        .is_some_and(|leaf| leaf.starts_with("diagnostic-driver-v4-"))
...
fn run_passive_driver_validation(
    ...
    let nonce = format!("diagnostic-driver-v4-{}", request.nonce);
...
fn write_root_result(layout: &RootLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RESULT_V4\nstatus={status}\ndetail={detail}\n");
...
fn write_root_recovery_result(layout: &RootLayout, outcome: &str, detail: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V4\noutcome={outcome}\ndetail={detail}\n"
    );
    match fs::symlink_metadata(&layout.recovery_result) {
        ...
        Ok(_) => {
            ...
            if !existing.starts_with("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V4\n")
                || !existing.ends_with('\n')
...
fn root_authorized_update(request_path: &Path) -> Result<()> {
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V4_UPDATE_COMMITTED host_pid={} host_runs={} pairing=preserved routes=unchanged legacy=protected",
        host.pid, host.runs
    );
    Ok(())
}
...
fn write_user_result(layout: &UserLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_RESULT_V4\nstatus={status}\ndetail={detail}\n");
...
fn execute_authorized_update(
    ...
    if commit != authorized_commit || tree != authorized_tree {
        return Err(ControllerError(
            "explicit authorization differs from the clean pushed dedicated V4 branch HEAD/tree"
                .to_owned(),
        ));
    }
    ...
    {
        return Err(ControllerError(
            "retained v1/v2/v3 descriptor guard changed immediately before root dispatch"
                .to_owned(),
        ));
    }
    ...
    if stdout.lines().count() != 1 || !stdout.starts_with("DIAGNOSTIC_DRIVER_V4_UPDATE_COMMITTED ")
    {
        return Err(ControllerError(
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V4_UPDATE_COMPLETE evidence={} host_pid={} pairing=preserved routes=unchanged legacy=protected",
...
fn rollback_authorized_update(repo: &Path) -> Result<()> {
    ...
    let recovery_kind = if line.starts_with("DIAGNOSTIC_DRIVER_V4_ROOT_ROLLBACK_COMPLETE ") {
        "rolled-back"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V4_ROOT_PRESTOP_ABORTED ") {
        "prestop-aborted"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V4_ROOT_RECOVERY_COMMITTED ") {
        "committed"
    } else {
        return Err(ControllerError(
    ...
    verify_pairing_metadata_only()?;
    println!(
        "DIAGNOSTIC_DRIVER_V4_RECOVERY_COMPLETE outcome={recovery_kind} host_pid={} pairing=preserved routes={route_status} legacy=protected",
        host.pid
    );
    Ok(())
}
...
fn complete_root_recovery(_request: RootRequest, layout: RootLayout) -> Result<()> {
    ...
    let (outcome, marker) = match plan {
        RootRecoveryPlan::PrestopPreserveHost => (
            ...
            "DIAGNOSTIC_DRIVER_V4_ROOT_PRESTOP_ABORTED",
        ),
        RootRecoveryPlan::RepairCommittedState | RootRecoveryPlan::ReportCommitted => (
            ...
            "DIAGNOSTIC_DRIVER_V4_ROOT_RECOVERY_COMMITTED",
        ),
        RootRecoveryPlan::RepairRolledBackState | RootRecoveryPlan::ReportRolledBack => (
            ...
            "DIAGNOSTIC_DRIVER_V4_ROOT_ROLLBACK_COMPLETE",
        ),
        ...
        RootRecoveryPlan::ResumeRollback => {
            ...
            (outcome, "DIAGNOSTIC_DRIVER_V4_ROOT_ROLLBACK_COMPLETE")
        }
    ...
    let recovery_outcome = match marker {
        "DIAGNOSTIC_DRIVER_V4_ROOT_RECOVERY_COMMITTED" => "committed",
        "DIAGNOSTIC_DRIVER_V4_ROOT_PRESTOP_ABORTED" => "prestop-aborted",
        "DIAGNOSTIC_DRIVER_V4_ROOT_ROLLBACK_COMPLETE" => "rolled-back",
        _ => {
            return Err(ControllerError(
...
fn write_or_verify_prestop_abort_witness(layout: &RootLayout, journal: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_PRESTOP_ABORT_JOURNAL_V4\noutcome=prestop-aborted\njournal={journal}\n"
    );
    match fs::symlink_metadata(&path) {
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
...
fn finalize_sealed_bootstrap_without_root_pointer(fixed_digest: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_BOOTSTRAP_ABORT_V4\nnonce={}\nhost_pid={}\noutcome=prestop-aborted\n",
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V4_ROOT_PRESTOP_ABORTED host_pid={} routes=unchanged pairing=preserved",
        host.pid
    );
    Ok(())
}
...
fn self_test() -> Result<()> {
    ...
    let root_hash_path = Path::new(
        "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v4/controller-0123456789abcdef0123456789abcdef/controller",
    );
    ...
    let synthetic = RootRequest {
        ...
        evidence: PathBuf::from(
            "/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v4/diagnostic-driver-v4-1-1-0123456789abcdef0123456789abcdef",
        ),
    ...
    for (fixture, label) in [
        ...
        (
            request_text.replace(
                ...
                "evidence=/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v4/../escape\n",
            ),
        ...
        (
            request_text.replace(
                ...
                "root_controller=/Library/Application Support/opensteamer/diagnostic-driver-controllers-v4/controller-0123456789abcdef0123456789abcdef/../controller\n",
            ),
    ...
    let pointer_bytes = b"/Library/Application Support/opensteamer/diagnostic-driver-updates-v4/transaction-0123456789abcdef0123456789abcdef\n";
    ...
    if !uid501_source_path_is_allowed(Path::new(BOTH_ORDER_PROBE), false)
        ...
        || uid501_source_path_is_allowed(
            Path::new("/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v4/../escape/probes/both-order.json"),
            true,
        )
    {
        return Err(ControllerError(
    ...
    let source = include_str!("opensteamer-diagnostic-driver-v4-update-controller.rs");
    let forbidden = [
    ...
    println!("DIAGNOSTIC_DRIVER_V4_SELF_TEST_OK tests=112");
    Ok(())
}
