...
PREFLIGHT_MODE='--verify-diagnostic-driver-v7-update-preflight'
EXECUTE_MODE='--execute-authorized-diagnostic-driver-v7-update'
ROLLBACK_MODE='--rollback-authorized-diagnostic-driver-v7-update'
SELF_TEST_MODE='--self-test-diagnostic-driver-v7-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer-diagnostic-v3'
USER_UPDATE_ROOT='/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v7'
USER_ACTIVE_POINTER='/Users/ahmed/Library/Application Support/opensteamer/active-diagnostic-driver-update-v7'
ROOT_SUPPORT='/Library/Application Support/opensteamer'
ROOT_CONTROLLER_PARENT='/Library/Application Support/opensteamer/diagnostic-driver-controllers-v7'
...
ROOT_ACTIVE_POINTER='/Library/Application Support/opensteamer/active-diagnostic-driver-update-v7'
ROOT_BOOTSTRAP_LOCATOR='/Library/Application Support/opensteamer/diagnostic-driver-bootstrap-v7.txt'
ROOT_SEALED_ROLLBACK_MODE='--root-sealed-rollback-diagnostic-driver-v7-update'
DIAGNOSTIC_READER_SHA256='6d3f5d71b399ada1244b84177d351c2406d426055da8f54e8f689af1c22a8ded'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-diagnostic-driver-v7-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='9133275f480d2ddfa0f448041e8531c463db7e15891c9f0323c5ec9906ff2c0e'
EXPECTED_BINARY_SHA256='5699bf177678c5a0c3814208324cd263eda9a2558994c4986c38a062ceb15b36'
BUILD_PARENT='/Volumes/t7/opensteamer-diagnostic-driver-v7-controller-builds'
...
if [ "$MODE" = "$ROLLBACK_MODE" ]; then
    ...
    if [ ! -e "$ROOT_ACTIVE_POINTER" ] && [ ! -L "$ROOT_ACTIVE_POINTER" ] \
        ...
        if [ -f "$ROOT_BOOTSTRAP_LOCATOR" ] && [ ! -L "$ROOT_BOOTSTRAP_LOCATOR" ] \
            ...
            && [ "$(/usr/bin/sed -n '1p' "$ROOT_BOOTSTRAP_LOCATOR")" = 'OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V7' ]; then
                ...
                *)
                    if [ "${#LOCATOR_NONCE}" -eq 32 ] \
                        ...
                        case "$LOCATOR_EVIDENCE" in
                            "$USER_UPDATE_ROOT"/diagnostic-driver-v7-*"$LOCATOR_NONCE") LOCATOR_READY=1 ;;
                        esac
                    fi
                    ;;
            esac
        ...
        if [ "$FIXED_READY" -eq 0 ]; then
            ...
            echo 'DIAGNOSTIC_DRIVER_V7_BOOTSTRAP_INCOMPLETE_HOST_PRESERVED root_pointer=absent' >&2
            exit 75
        fi
    fi
    [ -d "$ROOT_SUPPORT" ] && [ ! -L "$ROOT_SUPPORT" ] \
    ...
    case "$RECOVERY_OUTPUT" in
        DIAGNOSTIC_DRIVER_V7_ROOT_RECOVERY_COMMITTED\ *) RECOVERY_OUTCOME='committed' ;;
        DIAGNOSTIC_DRIVER_V7_ROOT_ROLLBACK_COMPLETE\ *) RECOVERY_OUTCOME='rolled-back' ;;
        DIAGNOSTIC_DRIVER_V7_ROOT_PRESTOP_ABORTED\ *) RECOVERY_OUTCOME='prestop-aborted' ;;
        *)
    ...
    if [ -f "$USER_ACTIVE_POINTER" ] && [ ! -L "$USER_ACTIVE_POINTER" ] \
        ...
        case "$RECOVERY_EVIDENCE" in
            "$USER_UPDATE_ROOT"/diagnostic-driver-v7-*)
...
compile_controller() {
    ...
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        ...
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-diagnostic-driver-v7" \
        --remap-path-prefix "$build_root=/reviewed/opensteamer-diagnostic-driver-v7-build" \
        "$SOURCE" -o "$controller"
