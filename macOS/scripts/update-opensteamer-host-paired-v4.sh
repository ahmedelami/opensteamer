#!/bin/sh
# Compile and invoke the reviewed Rust pairing-preserving v4 host updater. The launcher performs
# no cutover work itself and accepts only the explicit controller modes below.
set -eu
umask 077

PREFLIGHT_MODE='--verify-paired-v4-host-update-preflight'
EXECUTE_MODE='--execute-authorized-paired-v4-host-update'
ROLLBACK_MODE='--rollback-authorized-paired-v4-host-update'
SELF_TEST_MODE='--self-test-paired-v4-host-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-host-paired-v4-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='e688f39358399f80629fde49198b5610f7ea5628ec0abf081fb76ec088f67034'
...
[ "$(/usr/bin/id -u)" = 501 ] || {
    echo 'paired-v4 updater must run as Ahmed without sudo' >&2
    exit 1
}
for reviewed_source in "$SOURCE" "$V1_SOURCE"; do
    [ -f "$reviewed_source" ] && [ ! -L "$reviewed_source" ] \
        ...
        echo "reviewed paired-v4 source has unsafe metadata: $reviewed_source" >&2
        exit 1
    }
done
[ "$(/usr/bin/shasum -a 256 "$SOURCE" | /usr/bin/awk '{print $1}')" = \
  "$EXPECTED_SOURCE_SHA256" ] || {
    echo 'paired-v4 controller source differs from the reviewed bytes' >&2
    exit 1
}
...
BUILD_DIR=$(/usr/bin/mktemp -d "$BUILD_PARENT/.paired-v4-controller-build.XXXXXX") || exit 1
cleanup() {
...
compile_controller() {
    OPENSTEAMER_PAIRED_V4_INCLUDED_SOURCE="$INCLUDED_SOURCE" \
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        --sysroot "$RUSTC_SYSROOT" \
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-paired-v4" \
        --remap-path-prefix "$BUILD_DIR=/reviewed/opensteamer-paired-v4-build" \
        "$SOURCE" -o "$CONTROLLER"
...
/usr/bin/cmp -s "$FIRST" "$CONTROLLER" || {
    echo 'two reviewed paired-v4 controller compilations were not byte-identical' >&2
    exit 1
}
[ "$(/usr/bin/shasum -a 256 "$CONTROLLER" | /usr/bin/awk '{print $1}')" = \
  "$EXPECTED_BINARY_SHA256" ] || {
    echo 'compiled paired-v4 controller differs from the reviewed binary hash' >&2
    exit 1
}
/bin/chmod u+w "$FIRST"
/bin/rm -f "$FIRST"
