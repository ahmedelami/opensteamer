...
const SELF_TEST_SPLIT_PGID_WORKER_MODE: &str = "--self-test-v9-split-pgid-delayed-marker";
const SELF_TEST_EXIT_ZERO_SURVIVOR_MODE: &str = "--self-test-v9-exit-zero-with-split-pgid-survivor";
const LOCK_PROBE_MODE: &str = "--probe-diagnostic-driver-v9-host-lock";
const LOCK_FREE_PROBE_MODE: &str = "--probe-diagnostic-driver-v9-host-lock-free";
const LOCK_RECORD_PROBE_MODE: &str = "--read-diagnostic-driver-v9-host-lock-record";
...
const EXPECTED_RELEASE_BRANCH: &str = "fix/diagnostic-driver-v9-aqua-route-proof";
const EXPECTED_UPDATER_BASE_COMMIT: &str = "06ce81d649707023900d180f422e94175438613a";
const EXPECTED_UPDATER_BASE_TREE: &str = "f2eed67ec095293da5dbf530829d66d2cd0e238b";
...
const ROOT_RECOVERY_CONTROLLER_PIN: &str = "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v9/recovery-controller.sha256";
...
fn install_exact_uid501_hal_environment() -> Result<()> {
    let environment = unsafe { _NSGetEnviron() };
    if environment.is_null() {
        return Err(ControllerError(
            "could not resolve Aqua HAL child environment storage".to_owned(),
        ));
    }
    ...
    for (name, value) in UID501_HAL_ENVIRONMENT {
        ...
        if unsafe { setenv(name.as_ptr(), value.as_ptr(), 1) } != 0 {
            return Err(ControllerError(format!(
                "could not install exact Aqua HAL child environment: {}",
...
fn drop_aqua_hal_dispatcher_to_uid501() -> Result<()> {
    ...
    install_exact_uid501_hal_environment()
}
...
fn parse_post_reload_route_transcript(text: &str) -> Result<()> {
    ...
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROUTE_PROOF_V9") {
        return Err(ControllerError(
...
fn create_post_reload_route_transcript(probes: &Path) -> Result<PostReloadRouteTranscript> {
    ...
    let header = b"OPENSTEAMER_DIAGNOSTIC_DRIVER_ROUTE_PROOF_V9\n";
...
fn fresh_route_helper_record(observation: &PostReloadRouteObservation) -> String {
    match observation {
        PostReloadRouteObservation::Complete(snapshot) => format!(
            "OPENSTEAMER_DIAGNOSTIC_DRIVER_FRESH_ROUTE_V9\nstatus=complete\ninput_uid_hex={}\noutput_uid_hex={}\nsystem_output_uid_hex={}\n",
...
fn parse_fresh_route_helper_record(text: &str) -> Result<PostReloadRouteObservation> {
    ...
    let lines = text
        ...
        .split('\n')
        .collect::<Vec<_>>();
    if lines.first().copied() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_FRESH_ROUTE_V9") {
        return Err(ControllerError(
...
fn verify_git_provenance(repo: &Path) -> Result<(String, String)> {
    ...
    if branch != EXPECTED_RELEASE_BRANCH {
        return Err(ControllerError(
            "live release checkout is not the exact dedicated V9 branch".to_owned(),
        ));
    }
...
// The terminal V8 attempt is an immutable predecessor; V9 proves its descriptor graph under lock.
#[derive(Debug)]
...
fn create_user_layout(nonce: &str) -> Result<UserLayout> {
    ...
    let evidence = Path::new(USER_UPDATE_ROOT).join(format!(
        "diagnostic-driver-v9-{seconds}-{}-{nonce}",
        std::process::id()
    ));
...
fn parse_root_request_text(text: &str) -> Result<RootRequest> {
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V9") {
    ...
    if request.reader_sha256 != DIAGNOSTIC_READER_SHA256
        ...
        || !request
            ...
            .and_then(OsStr::to_str)
            .is_some_and(|leaf| {
                leaf.starts_with("diagnostic-driver-v9-") && leaf.ends_with(&request.nonce)
            })
...
fn root_recovery_output_is_terminal(output: &Output) -> Result<&'static str> {
    if !output.status.success() || !output.stderr.is_empty() {
        return Err(ControllerError(
            "supervised sealed recovery did not exit cleanly".to_owned(),
        ));
    }
    ...
    if line.starts_with("DIAGNOSTIC_DRIVER_V9_ROOT_RECOVERY_COMMITTED ") {
        Ok("committed")
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V9_ROOT_ROLLBACK_COMPLETE ") {
        Ok("rolled-back")
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V9_ROOT_PRESTOP_ABORTED ") {
        Ok("prestop-aborted")
    } else {
        Err(ControllerError(
...
fn parse_root_state_text(
    ...
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_STATE_V9") || !text.ends_with('\n')
    {
        return Err(ControllerError(
            ...
            .is_some_and(|evidence| {
                evidence.parent() == Some(Path::new(USER_UPDATE_ROOT))
                    && evidence
                        .file_name()
                        .and_then(OsStr::to_str)
                        .is_some_and(|leaf| leaf.starts_with("diagnostic-driver-v9-"))
...
fn run_passive_driver_validation(
    ...
    let nonce = format!("diagnostic-driver-v9-{}", request.nonce);
...
fn write_root_result(layout: &RootLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RESULT_V9\nstatus={status}\ndetail={detail}\n");
...
fn write_root_recovery_result(layout: &RootLayout, outcome: &str, detail: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V9\noutcome={outcome}\ndetail={detail}\n"
    );
    match fs::symlink_metadata(&layout.recovery_result) {
        ...
        Ok(_) => {
            ...
            if !existing.starts_with("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V9\n")
                || !existing.ends_with('\n')
...
fn perform_root_transaction(request_path: &Path) -> Result<HostGeneration> {
        ...
        Err(update_error) => {
            match rollback_root_transaction(&layout, &mut journal, &initial, Some(&baseline_route))
            {
                Ok(rollback) => {
                    ...
                    Err(ControllerError(format!(
                    "candidate deployment failed and exact v9 rollback completed with host pid {} routes={route_status}: {}",
    ...
    if verify_retained_v5_root_rolled_back(&retained_v5_root_lock)? != retained_v5 {
        return Err(ControllerError(
            "retained v5 root boundary changed across the V9 cutover/rollback".to_owned(),
        ));
    }
    if verify_retained_v6_root_rolled_back(&retained_v6_root_lock)? != retained_v6 {
        return Err(ControllerError(
            "retained v6 root boundary changed across the V9 cutover/rollback".to_owned(),
        ));
    }
    if verify_retained_v7_root_rolled_back(&retained_v7_root_lock)? != retained_v7 {
        return Err(ControllerError(
            "retained v7 root boundary changed across the V9 cutover/rollback".to_owned(),
        ));
    }
    if verify_retained_v8_root_rolled_back(&retained_v8_root_lock)? != retained_v8 {
        return Err(ControllerError(
            "retained v8 root boundary changed across the V9 cutover/rollback".to_owned(),
        ));
    }
    outcome
}
...
fn write_user_result(layout: &UserLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_RESULT_V9\nstatus={status}\ndetail={detail}\n");
...
fn execute_authorized_update(
    ...
    if commit != authorized_commit || tree != authorized_tree {
        return Err(ControllerError(
            "explicit authorization differs from the clean pushed dedicated V9 branch HEAD/tree"
                .to_owned(),
        ));
    }
    ...
    {
        return Err(ControllerError(
            "retained v1/v2/v3/v4/v5/v6/v7/v8 descriptor guard changed immediately before root dispatch"
                .to_owned(),
        ));
    }
    ...
    if stdout.lines().count() != 1 || !stdout.starts_with("DIAGNOSTIC_DRIVER_V9_UPDATE_COMMITTED ")
    {
        return Err(ControllerError(
...
fn rollback_authorized_update(repo: &Path) -> Result<()> {
    ...
    let recovery_kind = if line.starts_with("DIAGNOSTIC_DRIVER_V9_ROOT_ROLLBACK_COMPLETE ") {
        "rolled-back"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V9_ROOT_PRESTOP_ABORTED ") {
        "prestop-aborted"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V9_ROOT_RECOVERY_COMMITTED ") {
        "committed"
    } else {
        return Err(ControllerError(
...
fn complete_root_recovery(request: RootRequest, layout: RootLayout) -> Result<()> {
    ...
    let (outcome, marker) = match plan {
        ...
        RootRecoveryPlan::RepairCommittedState | RootRecoveryPlan::ReportCommitted => (
            ...
            "DIAGNOSTIC_DRIVER_V9_ROOT_RECOVERY_COMMITTED",
        ),
        RootRecoveryPlan::RepairRolledBackState | RootRecoveryPlan::ReportRolledBack => (
            ...
            "DIAGNOSTIC_DRIVER_V9_ROOT_ROLLBACK_COMPLETE",
        ),
        ...
        RootRecoveryPlan::ResumeRollback => {
            ...
            (outcome, "DIAGNOSTIC_DRIVER_V9_ROOT_ROLLBACK_COMPLETE")
        }
    ...
    let recovery_outcome = match marker {
        "DIAGNOSTIC_DRIVER_V9_ROOT_RECOVERY_COMMITTED" => "committed",
        "DIAGNOSTIC_DRIVER_V9_ROOT_PRESTOP_ABORTED" => "prestop-aborted",
        "DIAGNOSTIC_DRIVER_V9_ROOT_ROLLBACK_COMPLETE" => "rolled-back",
        _ => {
            return Err(ControllerError(
...
fn write_or_verify_prestop_abort_witness(layout: &RootLayout, journal: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_PRESTOP_ABORT_JOURNAL_V9\noutcome=prestop-aborted\njournal={journal}\n"
    );
    match fs::symlink_metadata(&path) {
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
...
fn root_sealed_rollback_authorized_update() -> Result<()> {
    ...
    let Some(layout) = reconcile_root_pointer_for_recovery(&locator_request)? else {
        ...
        {
            return Err(ControllerError(
                "retained v4/v5/v6/v7/v8 root boundary changed before pointerless recovery finalization"
                    .to_owned(),
            ));
        }
    ...
    {
        return Err(ControllerError(
            "retained v4/v5/v6/v7/v8 root boundary changed before sealed recovery".to_owned(),
        ));
    }
...
const SUDO_SUPERVISOR_SELF_TEST_MARKER_PREFIX: &str = "opensteamer-v9-sudo-supervisor-self-test-";
...
fn spawn_self_test_split_pgid_delayed_mutation(marker: &Path) -> Result<Child> {
    ...
    command
        .args([
            ...
            "opensteamer-v9-sudo-supervisor-delayed-mutation",
            path_text(marker)?,
        ])
        .current_dir("/")
        .env_clear()
...
fn self_test_exit_zero_with_split_pgid_survivor(marker: &Path) -> Result<()> {
    ...
    drop(child);
    println!("DIAGNOSTIC_DRIVER_V9_UPDATE_COMMITTED self-test-survivor=true");
    Ok(())
}
...
fn self_test_uid501_blocking_mutating_sudo_staging_guard() -> Result<()> {
    ...
    let root = Path::new("/tmp").join(format!(
        "opensteamer-v9-mutating-sudo-staging-self-test-{nonce}"
    ));
...
fn self_test_completed_sudo_worker_survivor_is_containment_violation() -> Result<()> {
    ...
    let test_result = (|| -> Result<()> {
        ...
        if !output.status.success()
            || output.stdout != b"DIAGNOSTIC_DRIVER_V9_UPDATE_COMMITTED self-test-survivor=true\n"
            || !output.stderr.is_empty()
            || !proof.used_sigkill
...
fn self_test_sudo_helper_deadline_terms_leader_before_slow_observer() -> Result<()> {
    ...
    command
        .args([
            ...
            "opensteamer-v9-slow-observer-leader-mutation",
            path_text(&marker)?,
        ])
        .current_dir("/")
        .env_clear()
...
fn self_test() -> Result<()> {
    ...
    let root_hash_path = Path::new(
        "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v9/controller-0123456789abcdef0123456789abcdef/controller",
    );
    ...
    let synthetic = RootRequest {
        ...
        evidence: PathBuf::from(
            "/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v9/diagnostic-driver-v9-1-1-0123456789abcdef0123456789abcdef",
        ),
    ...
    for (fixture, label) in [
        ...
        (
            request_text.replace(
                ...
                "evidence=/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v9/../escape\n",
            ),
        ...
        (
            request_text.replace(
                ...
                "root_controller=/Library/Application Support/opensteamer/diagnostic-driver-controllers-v9/controller-0123456789abcdef0123456789abcdef/../controller\n",
            ),
    ...
    let pointer_bytes = b"/Library/Application Support/opensteamer/diagnostic-driver-updates-v9/transaction-0123456789abcdef0123456789abcdef\n";
    ...
    let mut over_cap_transcript = "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROUTE_PROOF_V9\n".to_owned();
    ...
    if !uid501_source_path_is_allowed(Path::new(BOTH_ORDER_PROBE), false)
        ...
        || uid501_source_path_is_allowed(
            Path::new("/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v9/../escape/probes/both-order.json"),
            true,
        )
    {
        return Err(ControllerError(
    ...
    ) || !drop_source.contains("install_exact_uid501_hal_environment()")
    {
        return Err(ControllerError(
            "Aqua HAL environment/credential contract changed".to_owned(),
        ));
    ...
    println!("DIAGNOSTIC_DRIVER_V9_SELF_TEST_OK tests=120");
    Ok(())
}
