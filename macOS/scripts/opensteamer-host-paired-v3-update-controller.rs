...
//! This controller owns a disjoint v3 journal/pointer namespace, never starts an interactive
//! host, never resets pairing, and rolls back to the exact committed pairing-preserving v2 host.

#[allow(dead_code)]
mod paired_v3 {
    include!(env!("OPENSTEAMER_PAIRED_V3_INCLUDED_SOURCE"));

    const V3_PREFLIGHT_MODE: &str = "--verify-paired-v3-host-update-preflight";
    const V3_EXECUTE_MODE: &str = "--execute-authorized-paired-v3-host-update";
    const V3_ROLLBACK_MODE: &str = "--rollback-authorized-paired-v3-host-update";
    const V3_SELF_TEST_MODE: &str = "--self-test-paired-v3-host-update";
    const V3_EXPECTED_REPO: &str = "/Users/ahmed/Documents/Codex/opensteamer";

    const V3_UPDATE_ROOT: &str =
        "/Users/ahmed/Library/Application Support/opensteamer/paired-host-updates-v3";
    const V3_ACTIVE_UPDATE: &str =
        "/Users/ahmed/Library/Application Support/opensteamer/active-paired-host-update-v3";
    const V3_UPDATE_LOCK: &str = UPDATE_LOCK;
    const V3_JOURNAL_HEADER: &str = "OPENSTEAMER_PAIRED_HOST_UPDATE_V3";
    const HIDDEN_INSTALL_PREFIX: &str = ".opensteamer-paired-v3-install-";
    ...
    const CURRENT_BASELINE_APP: &str = "/Users/ahmed/Library/Application Support/opensteamer/paired-host-updates-v2/paired-v2-update-1785977369-24182-475e6219-114a-4bd9-b9df-c934579faf75/deployment-reference/opensteamer Host.app";
    const CURRENT_BASELINE_SOURCE_EXPORT: &str = "/Users/ahmed/Library/Application Support/opensteamer/paired-host-updates-v2/paired-v2-update-1785977369-24182-475e6219-114a-4bd9-b9df-c934579faf75/source-export";
    const CURRENT_BASELINE_EXECUTABLE_SHA256: &str =
        "7cc60fc9a1677ff10e17f4a6e09647e502a92b5492db46170567bed98c09f3bc";
    const CURRENT_BASELINE_CDHASH: &str = "e503fb26b65b3550404cf5eaff3307fe68ba1e38";
    ...
    impl V3Journal {
        fn create(path: &Path) -> Result<Self> {
            let mut file = OpenOptions::new()
                ...
                .open(path)
                .map_err(|error| ControllerError(format!("cannot create v3 journal: {error}")))?;
            validate_open_journal_file(path, &file)?;
            writeln!(file, "{V3_JOURNAL_HEADER}")?;
            file.sync_all()?;
            fsync_parent(path)?;
        ...
        fn open(path: &Path) -> Result<Self> {
            ...
            let complete_text = std::str::from_utf8(&bytes[..complete_length])
                .map_err(|_| ControllerError("v3 journal is not UTF-8".to_owned()))?;
            let state = parse_v3_journal(complete_text)?;
            if complete_length != bytes.len() {
                if !is_plausible_v3_torn_tail(&bytes[complete_length..], state) {
                    return Err(ControllerError(
                        "v3 journal has an implausible incomplete final record".to_owned(),
                    ));
                }
                validate_open_journal_file(path, &file)?;
        ...
        fn record(&mut self, state: V3State, fields: &[(&str, String)]) -> Result<()> {
            self.require_healthy()?;
            validate_v3_transition(self.state, state)?;
            validate_v3_fields(state, fields)?;
                ...
                .and_then(|_| self.file.sync_all())
            {
                ...
                return match recovery {
                    Ok(()) => Err(ControllerError(format!(
                        "cannot durably append v3 journal record: {error}"
                    ))),
                    Err(recovery_error) => Err(ControllerError(format!(
                        "cannot durably append v3 journal record ({error}) or restore prior length ({recovery_error})"
                    ))),
                };
            }
        ...
        fn require_healthy(&mut self) -> Result<()> {
            if !self.healthy {
                return Err(ControllerError(
                    "v3 journal is poisoned after an unrecovered append failure".to_owned(),
                ));
            }
        ...
        fn restore_after_failed_append(&mut self, prior_length: u64) -> Result<()> {
            ...
            if bytes.len() as u64 != prior_length || !bytes.ends_with(b"\n") {
                return Err(ControllerError(
                    "v3 journal append recovery did not restore prior durable length".to_owned(),
                ));
            }
            let text = std::str::from_utf8(&bytes)
                .map_err(|_| ControllerError("recovered v3 journal is not UTF-8".to_owned()))?;
            if parse_v3_journal(text)? != self.state {
                return Err(ControllerError(
                    "v3 journal append recovery did not restore prior state".to_owned(),
                ));
            }
    ...
    impl V3Layout {
        fn new(repo: PathBuf, evidence: PathBuf, nonce: &str) -> Self {
            ...
            let install_hold_root = PathBuf::from(format!(
                "/Applications/.opensteamer-paired-v3-install-{nonce}"
            ));
            Self {
                repo,
    ...
    pub fn entry() {
        if let Err(error) = paired_v3_real_main() {
            eprintln!("opensteamer paired-v3 update controller: {error}");
            std::process::exit(1);
        }
    }

    fn paired_v3_real_main() -> Result<()> {
        let arguments: Vec<String> = env::args().collect();
        verify_optimized_binary_scrub()?;
        match parse_v3_command(&arguments)? {
            V3Command::Preflight(repo) => {
                ...
                let generation = verify_paired_v3_runtime()?;
                let provenance = verify_paired_v3_git_provenance(&repo, true)?;
                verify_isolated_pairing_items_present()?;
                require_path_absent(Path::new(V3_ACTIVE_UPDATE), "active paired-v3 pointer")?;
                require_v3_update_root_unused()?;
                println!(
                    "PAIRED_V3_UPDATE_PREFLIGHT_OK pid={} runs={} baseline=sole-ready pairing=preserved v1=immutable v2=immutable v3=absent commit={} tree={}",
            ...
            V3Command::Execute {
                ...
                authorized_commit,
                authorized_tree,
            } => execute_paired_v3_update(
                canonical_repo(&repo)?,
                &authorized_commit,
                &authorized_tree,
            ),
            V3Command::Rollback(repo) => rollback_existing_paired_v3_update(canonical_repo(&repo)?),
            V3Command::SelfTest => paired_v3_self_test(),
            V3Command::ProbeLock { runtime, lock, pid } => {
    ...
    fn parse_v3_command(arguments: &[String]) -> Result<V3Command> {
        match arguments {
            [_, mode, repo] if mode == V3_PREFLIGHT_MODE => {
                Ok(V3Command::Preflight(repo.clone()))
            }
            [_, mode, repo, authorized_commit, authorized_tree] if mode == V3_EXECUTE_MODE => {
                ...
                Ok(V3Command::Execute {
                    repo: repo.clone(),
                    authorized_commit: authorized_commit.clone(),
                    authorized_tree: authorized_tree.clone(),
                })
            }
            [_, mode, repo] if mode == V3_ROLLBACK_MODE => Ok(V3Command::Rollback(repo.clone())),
            [_, mode] if mode == V3_SELF_TEST_MODE => Ok(V3Command::SelfTest),
            [_, mode, runtime, lock, pid] if mode == PROBE_LOCK_MODE => {
                Ok(V3Command::ProbeLock {
            ...
            _ => Err(ControllerError(format!(
                "usage: {} {V3_PREFLIGHT_MODE} <canonical-repo>\n       {} {V3_EXECUTE_MODE} <canonical-repo> <authorized-commit> <authorized-tree>\n       {} {V3_ROLLBACK_MODE} <canonical-repo>\n       {} {V3_SELF_TEST_MODE}\n       {} {PROBE_LOCK_MODE} <runtime-dir> <lock-file> <pid>",
    ...
    fn verify_optimized_binary_scrub() -> Result<()> {
        ...
        let executable = env::current_exe().map_err(|error| {
            ControllerError(format!("cannot resolve the paired-v3 controller: {error}"))
        })?;
        let mut file = File::open(&executable).map_err(|error| {
            ControllerError(format!("cannot inspect the paired-v3 controller: {error}"))
        })?;
        ...
        {
            return Err(ControllerError(
                "paired-v3 controller binary has unsafe metadata".to_owned(),
            ));
        }
        ...
        {
            return Err(ControllerError(
                "paired-v3 controller changed while being scrub-verified".to_owned(),
            ));
        }
        for (index, encoded) in FORBIDDEN_MARKER_HEX.iter().enumerate() {
            ...
            {
                return Err(ControllerError(format!(
                    "optimized paired-v3 controller retained forbidden legacy marker {index}"
                )));
            }
        }
        Ok(())
    ...
    fn decode_marker_hex(encoded: &str) -> Result<Vec<u8>> {
        if encoded.len() % 2 != 0 {
            return Err(ControllerError(
                "paired-v3 binary scrub marker has odd length".to_owned(),
            ));
        }
        encoded
            .as_bytes()
    ...
    fn decode_hex_nibble(value: u8) -> Result<u8> {
        match value {
            ...
            _ => Err(ControllerError(
                "paired-v3 binary scrub marker is not lowercase hexadecimal".to_owned(),
            )),
        }
    }

    fn require_v3_update_root_unused() -> Result<()> {
        require_v3_update_root_unused_at(Path::new(V3_UPDATE_ROOT))
    }

    fn require_v3_update_root_unused_at(root: &Path) -> Result<()> {
        match fs::symlink_metadata(root) {
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(()),
            Err(error) => {
                return Err(ControllerError(format!(
                    "cannot inspect paired-v3 update root {}: {error}",
                    root.display()
                )))
            }
        ...
        let mut entries = fs::read_dir(root).map_err(|error| {
            ControllerError(format!(
                "cannot enumerate paired-v3 update root {}: {error}",
                root.display()
            ))
        })?;
        if entries.next().transpose()?.is_some() {
            return Err(ControllerError(
                "paired-v3 update root already contains retained attempt evidence; a second attempt is not authorized"
                    .to_owned(),
            ));
        }
        Ok(())
    ...
    fn prepare_v3_update_root_for_first_attempt() -> Result<()> {
        let root = Path::new(V3_UPDATE_ROOT);
        match fs::symlink_metadata(root) {
        ...
        require_v3_update_root_unused_at(root)
    }

    fn execute_paired_v3_update(
        repo: PathBuf,
        ...
        require_canonical_git_oid(authorized_tree, "authorized tree")?;
        verify_machine_contract()?;
        let transaction_lock = acquire_update_transaction_lock_at(Path::new(V3_UPDATE_LOCK))?;
        verify_committed_v2_baseline()?;
        let initial_generation = verify_paired_v3_runtime()?;
        verify_isolated_pairing_items_present()?;
        let provenance = verify_paired_v3_git_provenance(&repo, true)?;
        ...
        require_available_bytes(
            ...
            2 * 1_024 * 1_024 * 1_024,
            "before creating paired-v3 update evidence",
        )?;

        let nonce = new_nonce()?;
        let evidence = PathBuf::from(V3_UPDATE_ROOT).join(format!(
            "paired-v3-update-{}-{}-{}",
            unix_seconds()?,
            std::process::id(),
            nonce
        ...
        let layout = V3Layout::new(repo, evidence, &nonce);
        create_private_directory(&layout.rollback_dir)?;
        create_private_directory(&layout.failed_dir)?;
        let mut journal = V3Journal::create(&layout.journal)?;
        record_v3_install_hold_name(&layout)?;

        let result = perform_paired_v3_update(
            &layout,
            &mut journal,
            &provenance,
            &initial_generation,
        ...
        match result {
            Ok(()) => Ok(()),
            Err(primary) => {
                if journal.state == V3State::Committed {
                    let _ = write_result(
                        &layout.result,
                    ...
                    eprintln!(
                        "warning: paired-v3 update committed but final reporting failed: {primary}"
                    );
                    return Ok(());
                }
                let pointer_absent_before_stop = journal.state == V3State::StopInitiated
                    && matches!(
                        fs::symlink_metadata(V3_ACTIVE_UPDATE),
                        Err(error) if error.kind() == std::io::ErrorKind::NotFound
                    );
                let crossed_stop = journal.state >= V3State::StopInitiated
                    && journal.state < V3State::Committed
                    && !pointer_absent_before_stop;
                if !crossed_stop {
                    if layout.rollback_reserve.exists() {
                        let _ = release_rollback_reserve(&layout.rollback_reserve);
                    }
                    let _ = archive_v3_install_hold_if_present(&layout);
                    let _ = write_result(
                        &layout.result,
                        "failed-before-stop",
                        Some(&primary.to_string()),
                ...
                verify_v3_active_pointer(&layout.evidence)?;
                match rollback_to_current_baseline(&layout, &mut journal, &transaction_lock) {
                    Ok(()) => {
                        ...
                        retire_v3_active_pointer(&layout)?;
                    ...
                    Err(rollback) => {
                        ...
                        Err(ControllerError(format!(
                            "CRITICAL: paired-v3 update and rollback both failed; keep host offline; evidence={}: primary={primary}; rollback={rollback}",
                            layout.evidence.display()
                        )))
                    }
                }
    ...
    fn perform_paired_v3_update(
        layout: &V3Layout,
        journal: &mut V3Journal,
        provenance: &Provenance,
        initial_generation: &LaunchGeneration,
        authorized_commit: &str,
        authorized_tree: &str,
    ) -> Result<()> {
        export_v3_source(layout, provenance)?;
        journal.record(
            V3State::SourceExported,
            &[
        ...
        build_and_verify_v3_staged_app(layout)?;
        prepare_v3_deployment_reference(layout)?;
        journal.record(
            V3State::BuildVerified,
            &[(
                "executable_sha256",
        ...
        verify_v3_deployment_reference(layout)?;
        verify_isolated_pairing_items_present()?;
        verify_v2_pointer_unchanged()?;
        let revalidated = verify_paired_v3_runtime()?;
        ...
        let boundary_provenance = verify_paired_v3_git_provenance(&layout.repo, true)?;
        ...
        publish_v3_active_pointer(&layout.evidence)?;
        prepare_v3_install_hold(layout)?;
        journal.record(V3State::InstallHoldVerified, &[])?;

        verify_v3_active_pointer(&layout.evidence)?;
        verify_v3_deployment_reference(layout)?;
        verify_isolated_pairing_items_present()?;
        verify_v2_pointer_unchanged()?;
        let final_generation = verify_paired_v3_runtime()?;
        ...
        journal.record(V3State::CurrentStopped, &[])?;
        ...
        journal.record(V3State::CurrentHeld, &[])?;
        ...
        fs::remove_dir(&layout.install_hold_root).map_err(|error| {
            ControllerError(format!(
                "cannot remove empty paired-v3 install-hold root {}: {error}",
                layout.install_hold_root.display()
            ))
        })?;
        fsync_parent(&layout.install_hold_root)?;
        verify_v3_installed_matches_reference(layout)?;
        verify_isolated_pairing_items_present()?;
        verify_v2_pointer_unchanged()?;
        journal.record(V3State::NewPublished, &[])?;
        drop(lock);

        let checkpoint = capture_log_checkpoint()?;
        bootstrap_exact_new_job()?;
        journal.record(V3State::PersistentBootstrapped, &[])?;
        let generation = wait_for_paired_v3_launch_generation(Duration::from_secs(45))?;
        verify_paired_v3_deployment(layout, &checkpoint, &generation)?;
        verify_isolated_pairing_items_present()?;
        verify_v2_pointer_unchanged()?;
        journal.record(
        ...
        journal.record(V3State::Committed, &[])?;
        if let Err(error) = write_result(&layout.result, "success", None) {
            eprintln!("warning: paired-v3 update committed but result recording failed: {error}");
        }
        println!(
            "PAIRED_V3_HOST_UPDATE_COMMITTED evidence={} pid={} rollback=current-isolated-retained pairing=preserved",
            layout.evidence.display(),
            generation.pid
        );
        Ok(())
    ...
    fn rollback_existing_paired_v3_update(repo: PathBuf) -> Result<()> {
        verify_machine_contract()?;
        let transaction_lock = acquire_update_transaction_lock_at(Path::new(V3_UPDATE_LOCK))?;
        verify_committed_v2_baseline()?;
        let evidence =
            read_update_pointer_at(Path::new(V3_ACTIVE_UPDATE), Path::new(V3_UPDATE_ROOT))?;
        let layout = v3_layout_from_existing(repo, evidence)?;
        let mut journal = V3Journal::open(&layout.journal)?;
        if journal.state == V3State::RolledBack {
            verify_paired_v3_runtime()?;
            verify_isolated_pairing_items_present()?;
            ensure_rolled_back_result(&layout.result)?;
            retire_v3_active_pointer(&layout)?;
            println!("PAIRED_V3_HOST_UPDATE_ALREADY_ROLLED_BACK");
            return Ok(());
        }
        ...
        retire_v3_active_pointer(&layout)?;
        println!(
            "PAIRED_V3_HOST_UPDATE_ROLLED_BACK evidence={} pairing=preserved",
            layout.evidence.display()
        );
        Ok(())
    }
    ...
    fn rollback_to_current_baseline(
        ...
        let already_rolling_back = matches!(
            journal.state,
            V3State::RollbackStarted
                | V3State::FailedNewArchived
                | V3State::CurrentRestored
                | V3State::CurrentBootstrapped
        );
        if !already_rolling_back {
            journal.record(V3State::RollbackStarted, &[])?;
        }
        if layout.rollback_reserve.exists() {
            release_rollback_reserve(&layout.rollback_reserve)?;
        }

        bootout_paired_v3_job_if_loaded(layout)?;
        wait_for_no_capture_servers(Duration::from_secs(30))?;
        require_service_absent(NEW_LAUNCH_AGENT_LABEL)?;
        verify_protected_legacy_absent()?;
        let lock = acquire_unowned_shared_lock()?;
        verify_isolated_pairing_items_present()?;
        verify_v2_pointer_unchanged()?;
        archive_v3_install_hold_if_present(layout)?;
        ...
        if rollback_exists {
            ...
            if canonical_exists {
                ...
                if journal.state == V3State::RollbackStarted {
                    journal.record(V3State::FailedNewArchived, &[])?;
                }
            } else if failed_exists && journal.state == V3State::RollbackStarted {
                journal.record(V3State::FailedNewArchived, &[])?;
            }
            ...
            if matches!(
                journal.state,
                V3State::RollbackStarted | V3State::FailedNewArchived
            ) {
                journal.record(V3State::CurrentRestored, &[])?;
            }
        ...
        if !matches!(
            journal.state,
            V3State::CurrentRestored | V3State::CurrentBootstrapped
        ) {
            return Err(ControllerError(format!(
                "paired-v3 rollback topology is not resumable from {}",
                journal.state.token()
            )));
        }
        verify_isolated_pairing_items_present()?;
        ...
        let checkpoint = capture_log_checkpoint()?;
        bootstrap_exact_new_job()?;
        if journal.state == V3State::CurrentRestored {
            journal.record(V3State::CurrentBootstrapped, &[])?;
        }
        let generation = wait_for_paired_v3_launch_generation(Duration::from_secs(45))?;
        ...
        verify_v2_pointer_unchanged()?;
        verify_protected_legacy_absent()?;
        journal.record(V3State::RolledBack, &[])?;
        verify_paired_v3_runtime()?;
        Ok(())
    }

    fn v3_layout_from_existing(repo: PathBuf, evidence: PathBuf) -> Result<V3Layout> {
        require_descendant(Path::new(V3_UPDATE_ROOT), &evidence)?;
        ...
        let nonce = install_hold_root
            ...
            .ok_or_else(|| {
                ControllerError("paired-v3 install-hold name is malformed".to_owned())
            })?;
        let expected = V3Layout::new(repo, evidence, nonce);
        if expected.install_hold_root != install_hold_root {
            return Err(ControllerError(
                "paired-v3 install-hold path escaped its recorded layout".to_owned(),
            ));
        }
        require_v3_install_hold_layout(&expected.install_hold_root, &expected.install_hold)?;
        Ok(expected)
    }

    fn export_v3_source(layout: &V3Layout, provenance: &Provenance) -> Result<()> {
        require_path_absent(&layout.source_tar, "v3 source archive")?;
        require_path_absent(&layout.source_export, "v3 source export")?;
        ...
        let status = Command::new("/usr/bin/git")
            ...
            .stderr(Stdio::piped())
            .status()?;
        require_success(status, "git archive for paired-v3 update")?;
        ...
        require_output_success(&output, "extract paired-v3 source archive")?;
        require_regular(
            &layout
                .source_export
        ...
        require_regular(
            &layout
                .source_export
                .join("macOS/scripts/opensteamer-host-paired-v3-update-controller.rs"),
            0o600,
        )?;
    ...
    fn build_and_verify_v3_staged_app(layout: &V3Layout) -> Result<()> {
        require_path_absent(&layout.stage_output, "paired-v3 staged output")?;
        require_path_absent(&layout.scratch, "paired-v3 SwiftPM scratch")?;
        ...
        let status = Command::new(&build_script)
            ...
            .status()?;
        require_success(status, "fresh signed paired-v3 host build")?;
        ...
        if sha256(&executable)? == CURRENT_BASELINE_EXECUTABLE_SHA256 {
            return Err(ControllerError(
                "paired-v3 staged executable is byte-identical to the current isolated baseline"
                    .to_owned(),
            ));
        }
    ...
    fn verify_staged_pairing_namespace(executable: &Path) -> Result<()> {
        ...
        require_output_success(&strings, "inspect paired-v3 staged pairing namespace")?;
        let text = decode_utf8(&strings.stdout, "paired-v3 strings output")?;
        ...
        if isolated_count != 1 || protected_count != 0 {
            return Err(ControllerError(format!(
                "paired-v3 staged pairing namespace is not isolated: isolated_count={isolated_count} protected_count={protected_count}"
            )));
        }
        Ok(())
    }

    fn prepare_v3_deployment_reference(layout: &V3Layout) -> Result<()> {
        require_path_absent(
            &layout.deployment_reference_dir,
            "paired-v3 deployment-reference directory",
        )?;
        ...
        require_output_success(&output, "copy paired-v3 deployment reference")?;
        verify_v3_deployment_reference(layout)
    }

    fn verify_v3_deployment_reference(layout: &V3Layout) -> Result<()> {
        ...
        for app in [&layout.staged_app, &layout.deployment_reference_app] {
            ...
            if sha256(&executable)? == CURRENT_BASELINE_EXECUTABLE_SHA256 {
                return Err(ControllerError(
                    "paired-v3 deployment reference equals current isolated baseline".to_owned(),
                ));
            }
            verify_staged_pairing_namespace(&executable)?;
        }
    ...
    fn prepare_v3_install_hold(layout: &V3Layout) -> Result<()> {
        require_v3_install_hold_layout(&layout.install_hold_root, &layout.install_hold)?;
        require_path_absent(&layout.install_hold_root, "paired-v3 hidden install hold")?;
        ...
        require_output_success(&output, "copy paired-v3 install hold")?;
        verify_bundle(
            &layout.source_export,
            &layout.install_hold,
            false,
    ...
    fn record_v3_install_hold_name(layout: &V3Layout) -> Result<()> {
        require_v3_install_hold_layout(&layout.install_hold_root, &layout.install_hold)?;
    ...
    fn archive_v3_install_hold_if_present(layout: &V3Layout) -> Result<()> {
        require_v3_install_hold_layout(&layout.install_hold_root, &layout.install_hold)?;
        ...
        if path_exists_without_follow(&layout.install_hold)? {
            ...
            require_path_absent(&archive, "unused paired-v3 install-hold archive")?;
    ...
    fn verify_v3_installed_matches_reference(layout: &V3Layout) -> Result<()> {
        verify_bundle(
            &layout.source_export,
            Path::new(NEW_APP),
            true,
    ...
    fn verify_paired_v3_deployment(
        layout: &V3Layout,
    ...
    fn verify_v2_pointer_unchanged() -> Result<()> {
        verify_committed_v2_baseline()
    }
    ...
    fn verify_reviewed_launch_agent_unchanged() -> Result<()> {
        ...
        if sha256(&source)? != REVIEWED_LAUNCH_AGENT_SHA256 {
            return Err(ControllerError(
                "committed v2 LaunchAgent source changed".to_owned(),
            ));
        }
        Ok(())
    }
    ...
    fn verify_paired_v3_runtime() -> Result<LaunchGeneration> {
        verify_committed_v2_baseline()?;
        verify_protected_legacy_absent()?;
        verify_reviewed_launch_agent_unchanged()?;
        verify_current_baseline_app_at(Path::new(NEW_APP), true)?;
    ...
    fn verify_paired_v3_git_provenance(repo: &Path, require_remote: bool) -> Result<Provenance> {
        let status = command_output(
            "/usr/bin/git",
            &["status", "--porcelain=v1", "--untracked-files=all"],
            Some(repo),
        )?;
        require_output_success(&status, "inspect paired-v3 git worktree")?;
        if !status.stdout.is_empty() {
            return Err(ControllerError(
                "repository must be completely clean before a paired-v3 host update".to_owned(),
            ));
        }
        ...
        if require_remote {
            ...
            require_output_success(&output, "verify pushed paired-v3 source commit")?;
            let text = decode_utf8(&output.stdout, "paired-v3 git ls-remote output")?;
            let records: Vec<&str> = text.lines().collect();
            let expected_ref = format!("refs/heads/{EXPECTED_SOURCE_BRANCH}");
            if records.len() != 1 {
                return Err(ControllerError(
                    "paired-v3 source branch is absent or ambiguous on origin".to_owned(),
                ));
            }
            ...
            {
                return Err(ControllerError(
                    "origin paired-v3 source branch does not resolve to local HEAD".to_owned(),
                ));
            }
        }
        Ok(Provenance {
    ...
    fn wait_for_paired_v3_launch_generation(timeout: Duration) -> Result<LaunchGeneration> {
        wait_for_launch_generation(timeout)
    }

    fn bootout_paired_v3_job_if_loaded(layout: &V3Layout) -> Result<()> {
        ...
        if state.status.success() {
            let loaded = parse_loaded_launch_job(decode_utf8(
                &state.stdout,
                "paired-v3 rollback launchctl state",
            )?)?;
            ...
            require_output_success(&output, "boot out paired-v3 LaunchAgent during rollback")?;
    ...
    fn verify_live_canonical_process(
        ...
        require_output_success(&output, "verify canonical paired-v3 host mapped code")
    }

    fn require_v3_install_hold_layout(root: &Path, app: &Path) -> Result<()> {
        ...
        {
            return Err(ControllerError(format!(
                "paired-v3 install hold escaped its reviewed layout: {}",
                app.display()
            )));
        }
        Ok(())
    ...
    fn publish_v3_active_pointer(evidence: &Path) -> Result<()> {
        require_descendant(Path::new(V3_UPDATE_ROOT), evidence)?;
        let pending = PathBuf::from(format!("{V3_ACTIVE_UPDATE}.pending-{}", std::process::id()));
        require_path_absent(&pending, "pending paired-v3 pointer")?;
        require_path_absent(Path::new(V3_ACTIVE_UPDATE), "active paired-v3 pointer")?;
        let mut file = create_new_private(&pending)?;
        writeln!(file, "{}", evidence.display())?;
        file.sync_all()?;
        rename_exclusive(&pending, Path::new(V3_ACTIVE_UPDATE))?;
        fsync_parent(Path::new(V3_ACTIVE_UPDATE))
    }

    fn verify_v3_active_pointer(expected_evidence: &Path) -> Result<()> {
        verify_update_pointer_at(
            Path::new(V3_ACTIVE_UPDATE),
            expected_evidence,
            Path::new(V3_UPDATE_ROOT),
        )
    }

    fn retire_v3_active_pointer(layout: &V3Layout) -> Result<()> {
    ...
    fn parse_v3_journal(text: &str) -> Result<V3State> {
        let mut lines = text.lines();
        if lines.next() != Some(V3_JOURNAL_HEADER) {
            return Err(ControllerError(
                "paired-v3 journal header is malformed".to_owned(),
            ));
        }
        let mut state = None;
        for line in lines {
            ...
            if fields.next() != Some("STATE") {
                return Err(ControllerError(
                    "paired-v3 journal record is malformed".to_owned(),
                ));
            }
            let next = fields
                .next()
                .and_then(V3State::parse)
                .ok_or_else(|| ControllerError("paired-v3 journal state is unknown".to_owned()))?;
            if let Some(previous) = state {
                validate_v3_transition(previous, next)?;
            } else if next != V3State::Begun {
                return Err(ControllerError(
                    "paired-v3 journal does not begin at BEGUN".to_owned(),
                ));
            }
            let fields: Vec<&str> = fields.collect();
            let expected = v3_field_schema(next);
            if fields.len() != expected.len() {
                return Err(ControllerError(
                    "paired-v3 journal field count is invalid".to_owned(),
                ));
            }
            for (field, expected_key) in fields.into_iter().zip(expected) {
                let (key, value) = field.split_once('=').ok_or_else(|| {
                    ControllerError("paired-v3 journal field is malformed".to_owned())
                })?;
                if key != *expected_key || !is_safe_journal_value(value) {
                    return Err(ControllerError(
                        "paired-v3 journal field is unsafe".to_owned(),
                    ));
                }
            }
            state = Some(next);
        }
        state.ok_or_else(|| ControllerError("paired-v3 journal has no state".to_owned()))
    }

    fn v3_field_schema(state: V3State) -> &'static [&'static str] {
        match state {
            V3State::Begun
            | V3State::InstallHoldVerified
            | V3State::CurrentStopped
            | V3State::CurrentHeld
            ...
            | V3State::CurrentBootstrapped
            | V3State::RolledBack => &[],
            V3State::SourceExported => &["commit", "tree", "initial_pid"],
            V3State::BuildVerified => &["executable_sha256"],
            V3State::StopInitiated => &["reserve_device", "reserve_inode", "reserve_bytes"],
            V3State::ReadyVerified => &["pid", "runs", "nonce"],
            V3State::CriticalFailure => &["phase"],
        }
    }

    fn validate_v3_fields(state: V3State, fields: &[(&str, String)]) -> Result<()> {
        let expected = v3_field_schema(state);
        if fields.len() != expected.len() {
            return Err(ControllerError(
                "paired-v3 journal record has wrong field count".to_owned(),
            ));
        }
        for ((key, value), expected_key) in fields.iter().zip(expected) {
            if *key != *expected_key || !is_safe_journal_value(value) {
                return Err(ControllerError("unsafe paired-v3 journal field".to_owned()));
            }
        }
        Ok(())
    }

    fn validate_v3_transition(previous: V3State, next: V3State) -> Result<()> {
        if previous == V3State::Begun && next == V3State::Begun {
            return Ok(());
        }
        let forward = matches!(
            (previous, next),
            (V3State::Begun, V3State::SourceExported)
                | (V3State::SourceExported, V3State::BuildVerified)
                | (V3State::BuildVerified, V3State::StopInitiated)
                | (V3State::StopInitiated, V3State::InstallHoldVerified)
                | (V3State::InstallHoldVerified, V3State::CurrentStopped)
                | (V3State::CurrentStopped, V3State::CurrentHeld)
                | (V3State::CurrentHeld, V3State::NewPublished)
                | (V3State::NewPublished, V3State::PersistentBootstrapped)
                | (V3State::PersistentBootstrapped, V3State::ReadyVerified)
                | (V3State::ReadyVerified, V3State::Committed)
        );
        let rollback_entry = next == V3State::RollbackStarted
            && ((previous >= V3State::StopInitiated && previous <= V3State::Committed)
                || previous == V3State::CriticalFailure)
            && previous != V3State::RollbackStarted;
        let rollback = matches!(
            (previous, next),
            (V3State::RollbackStarted, V3State::FailedNewArchived)
                | (V3State::RollbackStarted, V3State::CurrentRestored)
                | (V3State::FailedNewArchived, V3State::CurrentRestored)
                | (V3State::CurrentRestored, V3State::CurrentBootstrapped)
                | (V3State::CurrentBootstrapped, V3State::RolledBack)
        );
        let critical = next == V3State::CriticalFailure
            && previous >= V3State::RollbackStarted
            && previous < V3State::RolledBack;
        if forward || rollback_entry || rollback || critical {
            Ok(())
        } else {
            Err(ControllerError(format!(
                "invalid paired-v3 journal transition: {} -> {}",
                previous.token(),
                next.token()
            )))
        }
    ...
    const ALL_V3_STATES: [V3State; 17] = [
        V3State::Begun,
        V3State::SourceExported,
        V3State::BuildVerified,
        V3State::StopInitiated,
    ...
    fn is_plausible_v3_torn_tail(tail: &[u8], previous: V3State) -> bool {
        ...
        ALL_V3_STATES
            .iter()
            .copied()
            .filter(|next| validate_v3_transition(previous, *next).is_ok())
            .any(|next| is_plausible_v3_record_prefix(tail, next))
    }

    fn is_plausible_v3_record_prefix(tail: &str, state: V3State) -> bool {
    ...
    fn paired_v3_self_test() -> Result<()> {
        verify_v3_cli_surface()?;
        let valid = format!(
        ...
        if parse_v3_journal(&valid)? != V3State::Committed {
            return Err(ControllerError(
                "paired-v3 committed journal parser self-test failed".to_owned(),
            ));
        }
        ...
        if parse_v3_journal(&rolled_back)? != V3State::RolledBack {
            return Err(ControllerError(
                "paired-v3 rollback journal parser self-test failed".to_owned(),
            ));
        }
        if parse_v3_journal(&format!(
            "{V3_JOURNAL_HEADER}\nSTATE BEGUN\nSTATE NEW_PUBLISHED\n"
        ))
        .is_ok()
        {
            return Err(ControllerError(
                "paired-v3 journal parser accepted a skipped transition".to_owned(),
            ));
        }
        let hold_root = Path::new("/Applications/.opensteamer-paired-v3-install-selftest");
        let hold_app = hold_root.join("opensteamer Host.app");
        require_v3_install_hold_layout(hold_root, &hold_app)?;
        if require_v3_install_hold_layout(
            Path::new("/Applications/opensteamer Host.app"),
            &hold_app,
        )
        .is_ok()
            || require_v3_install_hold_layout(hold_root, &hold_root.join("unreviewed Host.app"))
                .is_ok()
        {
            return Err(ControllerError(
                "paired-v3 install-hold layout self-test failed".to_owned(),
            ));
        }
        paired_v3_dynamic_self_test()?;
        println!("SELF_TEST_OK paired-v3-host-update-controller");
        Ok(())
    }

    fn verify_v3_cli_surface() -> Result<()> {
        let executable = "controller".to_owned();
        let repo = V3_EXPECTED_REPO.to_owned();
        let authorized_commit = "a".repeat(40);
        let authorized_tree = "b".repeat(40);
        let allowed = [
            vec![
                executable.clone(),
                V3_PREFLIGHT_MODE.to_owned(),
                repo.clone(),
            ],
            vec![
                executable.clone(),
                V3_EXECUTE_MODE.to_owned(),
                repo.clone(),
            ...
            vec![
                executable.clone(),
                V3_ROLLBACK_MODE.to_owned(),
                repo.clone(),
            ],
            vec![executable.clone(), V3_SELF_TEST_MODE.to_owned()],
            vec![
                executable.clone(),
        ...
        if !matches!(parse_v3_command(&allowed[0]), Ok(V3Command::Preflight(_)))
            || !matches!(parse_v3_command(&allowed[1]), Ok(V3Command::Execute { .. }))
            || !matches!(parse_v3_command(&allowed[2]), Ok(V3Command::Rollback(_)))
            || !matches!(parse_v3_command(&allowed[3]), Ok(V3Command::SelfTest))
            || !matches!(
                parse_v3_command(&allowed[4]),
                Ok(V3Command::ProbeLock { .. })
            )
        {
            return Err(ControllerError(
                "paired-v3 CLI rejected a reviewed command shape".to_owned(),
            ));
        }

        let malformed = [
            vec![executable.clone(), V3_PREFLIGHT_MODE.to_owned()],
            vec![executable.clone(), V3_EXECUTE_MODE.to_owned(), repo.clone()],
            vec![
                executable.clone(),
                V3_EXECUTE_MODE.to_owned(),
                repo.clone(),
        ...
        if malformed
            .iter()
            .any(|arguments| parse_v3_command(arguments).is_ok())
        {
            return Err(ControllerError(
                "paired-v3 CLI accepted an unreviewed command shape".to_owned(),
            ));
        }
        ...
        {
            return Err(ControllerError(
                "paired-v3 authorization binding accepted a mismatched commit or tree".to_owned(),
            ));
        }
        ...
        for encoded_mode in [
            ...
            let mode = String::from_utf8(decode_marker_hex(encoded_mode)?).map_err(|_| {
                ControllerError("paired-v3 CLI test mode is not UTF-8".to_owned())
            })?;
            let arguments = vec![executable.clone(), mode, repo.clone()];
            if parse_v3_command(&arguments).is_ok() {
                return Err(ControllerError(
                    "paired-v3 CLI exposed an inherited update mode".to_owned(),
                ));
            }
        }
        Ok(())
    ...
    fn paired_v3_dynamic_self_test() -> Result<()> {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map_err(|_| ControllerError("system clock predates Unix epoch".to_owned()))?
            .as_nanos();
        let directory = PathBuf::from(format!(
            "/private/tmp/opensteamer-paired-v3-selftest-{}-{unique}",
            std::process::id()
        ));
        require_path_absent(&directory, "paired-v3 self-test directory")?;
        fs::create_dir(&directory)?;
        fs::set_permissions(&directory, fs::Permissions::from_mode(0o700))?;
        let result = paired_v3_dynamic_self_test_in(&directory);
        let cleanup = fs::remove_dir_all(&directory);
        match (result, cleanup) {
            (Err(error), _) => Err(error),
            (Ok(()), Err(error)) => Err(ControllerError(format!(
                "cannot remove paired-v3 self-test directory: {error}"
            ))),
            (Ok(()), Ok(())) => Ok(()),
        }
    }

    fn paired_v3_dynamic_self_test_in(directory: &Path) -> Result<()> {
        let recoverable = directory.join("recoverable.log");
        let mut journal = V3Journal::create(&recoverable)?;
        journal.record(
            V3State::SourceExported,
            &[
                ("commit", "a".repeat(40)),
        ...
        {
            return Err(ControllerError(
                "paired-v3 journal validation failure changed durable bytes".to_owned(),
            ));
        }
        drop(journal);
        ...
        partial.sync_all()?;
        drop(partial);
        let mut reopened = V3Journal::open(&recoverable)?;
        if reopened.state != V3State::SourceExported {
            return Err(ControllerError(
                "paired-v3 journal recovery accepted an incomplete final record".to_owned(),
            ));
        }
        reopened.record(
            V3State::BuildVerified,
        ...
        let mut corrupt_journal = V3Journal::create(&corrupt)?;
        ...
        drop(corrupt_append);
        if V3Journal::open(&corrupt).is_ok() {
            return Err(ControllerError(
                "paired-v3 journal recovery accepted a malformed complete record".to_owned(),
            ));
        }
        ...
        if acquire_update_transaction_lock_at(&transaction_lock).is_ok() {
            return Err(ControllerError(
                "paired-v3 transaction lock allowed concurrent ownership".to_owned(),
            ));
        }
        drop(first);
        ...
        require_v3_update_root_unused_at(&failed_attempt_root)?;
        ...
        if require_v3_update_root_unused_at(&failed_attempt_root).is_ok() {
            return Err(ControllerError(
                "paired-v3 single-attempt gate accepted failed-before-stop evidence".to_owned(),
            ));
        }
        ...
        require_v3_update_root_unused_at(&rolled_back_root)?;
        ...
        if require_v3_update_root_unused_at(&rolled_back_root).is_ok() {
            return Err(ControllerError(
                "paired-v3 single-attempt gate accepted rolled-back evidence".to_owned(),
            ));
        }

        let pointer_fixture = directory.join("v2-pointer-fixture");
        let mut pointer = create_new_private(&pointer_fixture)?;
        pointer.write_all(COMMITTED_V2_EVIDENCE.as_bytes())?;
        pointer.write_all(b"\n")?;
        pointer.sync_all()?;
        drop(pointer);
        ...
        if verify_self_test_pinned_file(&pointer_fixture, &pointer_hash).is_ok() {
            return Err(ControllerError(
                "v2 pointer corruption was accepted".to_owned(),
            ));
        }

        let evidence_fixture = directory.join("v2-evidence-fixture");
        ...
        if verify_self_test_pinned_file(&evidence_fixture, &evidence_hash).is_ok() {
            return Err(ControllerError(
                "v2 evidence corruption was accepted".to_owned(),
            ));
        }

        self_test_v2_oracle_pin_mutation(directory)?;
    ...
    fn self_test_v2_oracle_pin_mutation(directory: &Path) -> Result<()> {
        for (name, mode) in [
            ...
            writeln!(oracle, "immutable-v2-oracle={name}")?;
            oracle.sync_all()?;
            drop(oracle);
            ...
            if verify_self_test_pinned_file_with_mode(&path, mode, &expected_sha256).is_ok() {
                return Err(ControllerError(format!(
                    "v2 rollback oracle corruption was accepted: {name}"
                )));
            }
        }
        Ok(())
    ...
    fn self_test_publication_boundary_case(
        ...
        const REPLACEMENT_BYTES: &str = "paired-v3-replacement";
        ...
        let pointer = case_root.join("pinned-v2-pointer-fixture");

        let mut pointer_file = create_new_private(&pointer)?;
        writeln!(pointer_file, "{COMMITTED_V2_EVIDENCE}")?;
        pointer_file.sync_all()?;
        drop(pointer_file);
        if sha256(&pointer)? != COMMITTED_V2_POINTER_SHA256 {
            return Err(ControllerError(
                "publication matrix v2 pointer does not match the committed bytes".to_owned(),
            ));
        }
        verify_self_test_pinned_file(&pointer, COMMITTED_V2_POINTER_SHA256)?;
        ...
        if case.current_held {
            self_test_checked_rename(&canonical, &rollback, &pointer, COMMITTED_V2_POINTER_SHA256)?;
        }
        if case.new_published {
            self_test_checked_rename(&pending, &canonical, &pointer, COMMITTED_V2_POINTER_SHA256)?;
        }
        ...
        if path_exists_without_follow(&rollback)? {
            ...
            self_test_checked_rename(&rollback, &canonical, &pointer, COMMITTED_V2_POINTER_SHA256)?;
        }
