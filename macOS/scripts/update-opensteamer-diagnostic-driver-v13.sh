...
PREFLIGHT_MODE='--verify-diagnostic-driver-v13-update-preflight'
EXECUTE_MODE='--execute-authorized-diagnostic-driver-v13-update'
ROLLBACK_MODE='--rollback-authorized-diagnostic-driver-v13-update'
SELF_TEST_MODE='--self-test-diagnostic-driver-v13-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer-diagnostic-v3'
USER_UPDATE_ROOT='/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v13'
USER_ACTIVE_POINTER='/Users/ahmed/Library/Application Support/opensteamer/active-diagnostic-driver-update-v13'
ROOT_SUPPORT='/Library/Application Support/opensteamer'
ROOT_CONTROLLER_PARENT='/Library/Application Support/opensteamer/diagnostic-driver-controllers-v13'
...
ROOT_ACTIVE_POINTER='/Library/Application Support/opensteamer/active-diagnostic-driver-update-v13'
ROOT_BOOTSTRAP_LOCATOR='/Library/Application Support/opensteamer/diagnostic-driver-bootstrap-v13.txt'
DIAGNOSTIC_READER_SHA256='6d3f5d71b399ada1244b84177d351c2406d426055da8f54e8f689af1c22a8ded'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-diagnostic-driver-v13-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='1ea3c460e9e3df0c150840330d6ace6b80694128b983f2f0b2e19a4e5c89aff1'
EXPECTED_BINARY_SHA256='4b0e2bb7ef1110e3a98834600811a221fb9ca3b21319f63902f5c9933dbcfefc'
BUILD_PARENT='/Volumes/t7/opensteamer-diagnostic-driver-v13-controller-builds'
...
if [ "$MODE" = "$ROLLBACK_MODE" ]; then
    ...
    if [ ! -e "$ROOT_ACTIVE_POINTER" ] && [ ! -L "$ROOT_ACTIVE_POINTER" ] \
        ...
        if [ -f "$ROOT_BOOTSTRAP_LOCATOR" ] && [ ! -L "$ROOT_BOOTSTRAP_LOCATOR" ] \
            ...
            && [ "$(/usr/bin/sed -n '1p' "$ROOT_BOOTSTRAP_LOCATOR")" = 'OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V13' ]; then
                        ...
                        case "$LOCATOR_DISPLAY_SELECTED" in
                            1080:1920:1080:1920:60000|603:1311:1206:2622:60000|540:1170:1080:2340:60000|540:960:1080:1920:60000|414:896:828:1792:60000|750:1334:750:1334:60000)
                                case "$LOCATOR_EVIDENCE" in
                                    "$USER_UPDATE_ROOT"/diagnostic-driver-v13-*"$LOCATOR_NONCE") LOCATOR_READY=1 ;;
                                esac
                                ;;
                        esac
                    fi
        ...
        if [ "$FIXED_READY" -eq 0 ]; then
            ...
            echo 'DIAGNOSTIC_DRIVER_V13_BOOTSTRAP_INCOMPLETE_HOST_PRESERVED root_pointer=absent' >&2
            exit 75
        fi
    fi
    [ -d "$ROOT_SUPPORT" ] && [ ! -L "$ROOT_SUPPORT" ] \
    ...
    case "$RECOVERY_OUTPUT" in
        DIAGNOSTIC_DRIVER_V13_RECOVERY_COMPLETE\ outcome=committed\ *) RECOVERY_OUTCOME='committed' ;;
        DIAGNOSTIC_DRIVER_V13_RECOVERY_COMPLETE\ outcome=rolled-back\ *) RECOVERY_OUTCOME='rolled-back' ;;
        DIAGNOSTIC_DRIVER_V13_RECOVERY_COMPLETE\ outcome=prestop-aborted\ *) RECOVERY_OUTCOME='prestop-aborted' ;;
        *)
    ...
    if [ -f "$USER_ACTIVE_POINTER" ] && [ ! -L "$USER_ACTIVE_POINTER" ] \
        ...
        case "$RECOVERY_EVIDENCE" in
            "$USER_UPDATE_ROOT"/diagnostic-driver-v13-*)
...
compile_controller() {
    ...
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        ...
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-diagnostic-driver-v13" \
        --remap-path-prefix "$build_root=/reviewed/opensteamer-diagnostic-driver-v13-build" \
        "$SOURCE" -o "$controller"
