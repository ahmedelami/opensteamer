#!/bin/sh
# Compile and invoke the reviewed Rust pairing-preserving v6 host updater. The launcher performs
# no cutover work itself and accepts only the explicit controller modes below.
set -eu
umask 077

PREFLIGHT_MODE='--verify-paired-v6-host-update-preflight'
EXECUTE_MODE='--execute-authorized-paired-v6-host-update'
ROLLBACK_MODE='--rollback-authorized-paired-v6-host-update'
SELF_TEST_MODE='--self-test-paired-v6-host-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-host-paired-v6-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='3ee23b156017ce72e800882fb75c91f32a813586c4d20c4ee71ef047e38026f5'
...
[ "$(/usr/bin/id -u)" = 501 ] || {
    echo 'paired-v6 updater must run as Ahmed without sudo' >&2
    exit 1
}
for reviewed_source in "$SOURCE" "$V1_SOURCE"; do
    [ -f "$reviewed_source" ] && [ ! -L "$reviewed_source" ] \
        ...
        echo "reviewed paired-v6 source has unsafe metadata: $reviewed_source" >&2
        exit 1
    }
done
[ "$(/usr/bin/shasum -a 256 "$SOURCE" | /usr/bin/awk '{print $1}')" = \
  "$EXPECTED_SOURCE_SHA256" ] || {
    echo 'paired-v6 controller source differs from the reviewed bytes' >&2
    exit 1
}
...
BUILD_DIR=$(/usr/bin/mktemp -d "$BUILD_PARENT/.paired-v6-controller-build.XXXXXX") || exit 1
cleanup() {
...
compile_controller() {
    OPENSTEAMER_PAIRED_V6_INCLUDED_SOURCE="$INCLUDED_SOURCE" \
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        --sysroot "$RUSTC_SYSROOT" \
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-paired-v6" \
        --remap-path-prefix "$BUILD_DIR=/reviewed/opensteamer-paired-v6-build" \
        "$SOURCE" -o "$CONTROLLER"
...
/usr/bin/cmp -s "$FIRST" "$CONTROLLER" || {
    echo 'two reviewed paired-v6 controller compilations were not byte-identical' >&2
    exit 1
}
[ "$(/usr/bin/shasum -a 256 "$CONTROLLER" | /usr/bin/awk '{print $1}')" = \
  "$EXPECTED_BINARY_SHA256" ] || {
    echo 'compiled paired-v6 controller differs from the reviewed binary hash' >&2
    exit 1
}
/bin/chmod u+w "$FIRST"
/bin/rm -f "$FIRST"
