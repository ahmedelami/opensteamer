...
PREFLIGHT_MODE='--verify-diagnostic-driver-v11-update-preflight'
EXECUTE_MODE='--execute-authorized-diagnostic-driver-v11-update'
ROLLBACK_MODE='--rollback-authorized-diagnostic-driver-v11-update'
SELF_TEST_MODE='--self-test-diagnostic-driver-v11-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer-diagnostic-v3'
USER_UPDATE_ROOT='/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v11'
USER_ACTIVE_POINTER='/Users/ahmed/Library/Application Support/opensteamer/active-diagnostic-driver-update-v11'
ROOT_SUPPORT='/Library/Application Support/opensteamer'
ROOT_CONTROLLER_PARENT='/Library/Application Support/opensteamer/diagnostic-driver-controllers-v11'
...
ROOT_ACTIVE_POINTER='/Library/Application Support/opensteamer/active-diagnostic-driver-update-v11'
ROOT_BOOTSTRAP_LOCATOR='/Library/Application Support/opensteamer/diagnostic-driver-bootstrap-v11.txt'
DIAGNOSTIC_READER_SHA256='6d3f5d71b399ada1244b84177d351c2406d426055da8f54e8f689af1c22a8ded'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-diagnostic-driver-v11-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='a5d5d037f0f0d5804cef2a01bdd3cb8c18de99b3dc6b54d72af6e2d5d87f5eb0'
EXPECTED_BINARY_SHA256='0b76b8411a6f504d9210a28977363aab5d8af03bae0144d3fcb4b50a31fb3403'
BUILD_PARENT='/Volumes/t7/opensteamer-diagnostic-driver-v11-controller-builds'
...
if [ "$MODE" = "$ROLLBACK_MODE" ]; then
    ...
    if [ ! -e "$ROOT_ACTIVE_POINTER" ] && [ ! -L "$ROOT_ACTIVE_POINTER" ] \
        ...
        if [ -f "$ROOT_BOOTSTRAP_LOCATOR" ] && [ ! -L "$ROOT_BOOTSTRAP_LOCATOR" ] \
            ...
            && [ "$(/usr/bin/sed -n '1p' "$ROOT_BOOTSTRAP_LOCATOR")" = 'OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V11' ]; then
                        ...
                        case "$LOCATOR_DISPLAY_SELECTED" in
                            1080:1920:1080:1920:60000|603:1311:1206:2622:60000|540:1170:1080:2340:60000|540:960:1080:1920:60000|414:896:828:1792:60000|750:1334:750:1334:60000)
                                case "$LOCATOR_EVIDENCE" in
                                    "$USER_UPDATE_ROOT"/diagnostic-driver-v11-*"$LOCATOR_NONCE") LOCATOR_READY=1 ;;
                                esac
                                ;;
                        esac
                    fi
        ...
        if [ "$FIXED_READY" -eq 0 ]; then
            ...
            echo 'DIAGNOSTIC_DRIVER_V11_BOOTSTRAP_INCOMPLETE_HOST_PRESERVED root_pointer=absent' >&2
            exit 75
        fi
    fi
    [ -d "$ROOT_SUPPORT" ] && [ ! -L "$ROOT_SUPPORT" ] \
    ...
    case "$RECOVERY_OUTPUT" in
        DIAGNOSTIC_DRIVER_V11_RECOVERY_COMPLETE\ outcome=committed\ *) RECOVERY_OUTCOME='committed' ;;
        DIAGNOSTIC_DRIVER_V11_RECOVERY_COMPLETE\ outcome=rolled-back\ *) RECOVERY_OUTCOME='rolled-back' ;;
        DIAGNOSTIC_DRIVER_V11_RECOVERY_COMPLETE\ outcome=prestop-aborted\ *) RECOVERY_OUTCOME='prestop-aborted' ;;
        *)
    ...
    if [ -f "$USER_ACTIVE_POINTER" ] && [ ! -L "$USER_ACTIVE_POINTER" ] \
        ...
        case "$RECOVERY_EVIDENCE" in
            "$USER_UPDATE_ROOT"/diagnostic-driver-v11-*)
...
compile_controller() {
    ...
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        ...
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-diagnostic-driver-v11" \
        --remap-path-prefix "$build_root=/reviewed/opensteamer-diagnostic-driver-v11-build" \
        "$SOURCE" -o "$controller"
