...
PREFLIGHT_MODE='--verify-diagnostic-driver-v5-update-preflight'
EXECUTE_MODE='--execute-authorized-diagnostic-driver-v5-update'
ROLLBACK_MODE='--rollback-authorized-diagnostic-driver-v5-update'
SELF_TEST_MODE='--self-test-diagnostic-driver-v5-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer-diagnostic-v3'
USER_UPDATE_ROOT='/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v5'
USER_ACTIVE_POINTER='/Users/ahmed/Library/Application Support/opensteamer/active-diagnostic-driver-update-v5'
ROOT_SUPPORT='/Library/Application Support/opensteamer'
ROOT_CONTROLLER_PARENT='/Library/Application Support/opensteamer/diagnostic-driver-controllers-v5'
...
ROOT_ACTIVE_POINTER='/Library/Application Support/opensteamer/active-diagnostic-driver-update-v5'
ROOT_BOOTSTRAP_LOCATOR='/Library/Application Support/opensteamer/diagnostic-driver-bootstrap-v5.txt'
ROOT_SEALED_ROLLBACK_MODE='--root-sealed-rollback-diagnostic-driver-v5-update'
DIAGNOSTIC_READER_SHA256='6d3f5d71b399ada1244b84177d351c2406d426055da8f54e8f689af1c22a8ded'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-diagnostic-driver-v5-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='bfb0d05711a75384321f7c0b61c21144068e5ad3a6c85e33ff34f4fdc52f889a'
EXPECTED_BINARY_SHA256='b323c5866e8632a8ceb3f325cc02be50f9c677e23d55fc12820e100581fa9e24'
BUILD_PARENT='/Volumes/t7/opensteamer-diagnostic-driver-v5-controller-builds'
...
if [ "$MODE" = "$ROLLBACK_MODE" ]; then
    ...
    if [ ! -e "$ROOT_ACTIVE_POINTER" ] && [ ! -L "$ROOT_ACTIVE_POINTER" ] \
        ...
        if [ -f "$ROOT_BOOTSTRAP_LOCATOR" ] && [ ! -L "$ROOT_BOOTSTRAP_LOCATOR" ] \
            ...
            && [ "$(/usr/bin/sed -n '1p' "$ROOT_BOOTSTRAP_LOCATOR")" = 'OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V5' ]; then
                ...
                *)
                    if [ "${#LOCATOR_NONCE}" -eq 32 ] \
                        ...
                        case "$LOCATOR_EVIDENCE" in
                            "$USER_UPDATE_ROOT"/diagnostic-driver-v5-*"$LOCATOR_NONCE") LOCATOR_READY=1 ;;
                        esac
                    fi
                    ;;
            esac
        ...
        if [ "$FIXED_READY" -eq 0 ]; then
            ...
            echo 'DIAGNOSTIC_DRIVER_V5_BOOTSTRAP_INCOMPLETE_HOST_PRESERVED root_pointer=absent' >&2
            exit 75
        fi
    fi
    [ -d "$ROOT_SUPPORT" ] && [ ! -L "$ROOT_SUPPORT" ] \
    ...
    case "$RECOVERY_OUTPUT" in
        DIAGNOSTIC_DRIVER_V5_ROOT_RECOVERY_COMMITTED\ *) RECOVERY_OUTCOME='committed' ;;
        DIAGNOSTIC_DRIVER_V5_ROOT_ROLLBACK_COMPLETE\ *) RECOVERY_OUTCOME='rolled-back' ;;
        DIAGNOSTIC_DRIVER_V5_ROOT_PRESTOP_ABORTED\ *) RECOVERY_OUTCOME='prestop-aborted' ;;
        *)
    ...
    if [ -f "$USER_ACTIVE_POINTER" ] && [ ! -L "$USER_ACTIVE_POINTER" ] \
        ...
        case "$RECOVERY_EVIDENCE" in
            "$USER_UPDATE_ROOT"/diagnostic-driver-v5-*)
...
compile_controller() {
    ...
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        ...
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-diagnostic-driver-v5" \
        --remap-path-prefix "$build_root=/reviewed/opensteamer-diagnostic-driver-v5-build" \
        "$SOURCE" -o "$controller"
