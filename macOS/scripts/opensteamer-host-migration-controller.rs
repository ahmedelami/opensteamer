...
const V14_EVIDENCE_PATH: &str = "/Users/ahmed/Library/Application Support/opensteamer/migrations/migration-v14-after-v13-1785637636-18044";
...
const EMBEDDED_DEPLOYMENT_VERIFIER_SHA256: &str =
    "09ab6fc87be7c3c275bfd71bd75febeaaedca2c57c4c2560803e3a78fddf3b88";
...
impl Layout {
    ...
    fn create(repo: PathBuf) -> Result<Self> {
        ...
        let evidence = PathBuf::from(V14_EVIDENCE_PATH);
...
fn verify_prior_retry_state_cli() -> Result<()> {
    ...
    match recover_active_pointer(private_root)? {
        ...
        ActivePointerRecovery::Active => {
            return Err(ControllerError(
                "a version-14 active transaction already exists".to_owned(),
            ))
        }
    }
    verify_chflags_tool()?;
    ...
    if entry_exists(Path::new(V14_EVIDENCE_PATH))? {
        return Err(ControllerError(
            "the deterministic version-14 evidence path is already occupied".to_owned(),
        ));
    }
    println!("PRIOR_RETRY_STATE_OK v9=v10=v11=v12=v13 legacy=sole-ready v14=absent");
    Ok(())
}
...
/// nonexistent `/bin/chflags`. Its retained evidence must remain untouched. Version 14 may start
/// only after independently pinning the exact v9 tombstone together with the exact fully rolled-back
/// v10, v11, v12, and v13 tombstones and reproving the untouched legacy runtime. Any other residue
/// fails closed.
...
fn recover_active(repo: PathBuf, private_root: &PinnedDirectory) -> Result<()> {
    ...
    match journal.state {
        ...
        State::CriticalFailure => Err(ControllerError(format!(
            "active v14 transaction is in CRITICAL_FAILURE; keep Mac offline; evidence={}",
            layout.evidence.display()
        ))),
        _ => {
...
fn rollback_hidden_paths(layout: &Layout) -> Result<(PathBuf, PathBuf, PathBuf, PathBuf)> {
    ...
    Ok((
        Path::new(APPLICATIONS_DIRECTORY).join(format!(".opensteamer-disabled-v14-{tag}")),
        Path::new("/Users/ahmed/Library/LaunchAgents").join(format!(
            ".org.example.opensteamer.worldwide.plist.disabled-v14-{tag}"
        )),
...
fn recover_active_pointer(parent: &PinnedDirectory) -> Result<ActivePointerRecovery> {
    ...
    if linearized_exists {
        ...
        let _ = parse_v14_active_record(&linearized_bytes)?;
    ...
    if finalizing_exists {
        ...
        let finalizing_bytes =
            read_pinned_regular(parent, ACTIVE_TRANSACTION_FINALIZING_NAME, 0o600)?;
        let _ = parse_v14_active_record(&finalizing_bytes)?;
        if active_exists {
    ...
    match (active_exists, pending_exists) {
        ...
        (true, false) => {
            let _ = parse_v14_active_record(&read_pinned_regular(
                parent,
        ...
        (true, true) => {
            ...
            let _ = parse_v14_active_record(&active_bytes)?;
        ...
        (false, true) => {
            ...
            let _ = parse_v14_active_record(&bytes).map_err(|error| {
                ControllerError(format!(
...
fn read_active(parent: &PinnedDirectory) -> Result<PathBuf> {
    parse_v14_active_record(&read_pinned_regular(
        parent,
        ACTIVE_TRANSACTION_NAME,
        0o600,
    )?)
...
fn parse_v14_active_record(bytes: &[u8]) -> Result<PathBuf> {
    let evidence = parse_active_record(bytes)?;
    if evidence != Path::new(V14_EVIDENCE_PATH) {
        return Err(ControllerError(format!(
            "v14 active pointer selects unreviewed evidence: {}",
            evidence.display()
        )));
    }
    Ok(evidence)
