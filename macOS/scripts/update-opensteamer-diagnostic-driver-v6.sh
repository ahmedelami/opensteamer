...
PREFLIGHT_MODE='--verify-diagnostic-driver-v6-update-preflight'
EXECUTE_MODE='--execute-authorized-diagnostic-driver-v6-update'
ROLLBACK_MODE='--rollback-authorized-diagnostic-driver-v6-update'
SELF_TEST_MODE='--self-test-diagnostic-driver-v6-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer-diagnostic-v3'
USER_UPDATE_ROOT='/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v6'
USER_ACTIVE_POINTER='/Users/ahmed/Library/Application Support/opensteamer/active-diagnostic-driver-update-v6'
ROOT_SUPPORT='/Library/Application Support/opensteamer'
ROOT_CONTROLLER_PARENT='/Library/Application Support/opensteamer/diagnostic-driver-controllers-v6'
...
ROOT_ACTIVE_POINTER='/Library/Application Support/opensteamer/active-diagnostic-driver-update-v6'
ROOT_BOOTSTRAP_LOCATOR='/Library/Application Support/opensteamer/diagnostic-driver-bootstrap-v6.txt'
ROOT_SEALED_ROLLBACK_MODE='--root-sealed-rollback-diagnostic-driver-v6-update'
DIAGNOSTIC_READER_SHA256='6d3f5d71b399ada1244b84177d351c2406d426055da8f54e8f689af1c22a8ded'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-diagnostic-driver-v6-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='e09e2df3b0e6c1d0aa774ded78fcc9fd7dbdc2a37dfa35e84983cf1135b50cc0'
EXPECTED_BINARY_SHA256='34c9e3be35106640a4cae66efd0d67897c59d748effa63b2466f2d58c3cf2726'
BUILD_PARENT='/Volumes/t7/opensteamer-diagnostic-driver-v6-controller-builds'
...
if [ "$MODE" = "$ROLLBACK_MODE" ]; then
    ...
    if [ ! -e "$ROOT_ACTIVE_POINTER" ] && [ ! -L "$ROOT_ACTIVE_POINTER" ] \
        ...
        if [ -f "$ROOT_BOOTSTRAP_LOCATOR" ] && [ ! -L "$ROOT_BOOTSTRAP_LOCATOR" ] \
            ...
            && [ "$(/usr/bin/sed -n '1p' "$ROOT_BOOTSTRAP_LOCATOR")" = 'OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V6' ]; then
                ...
                *)
                    if [ "${#LOCATOR_NONCE}" -eq 32 ] \
                        ...
                        case "$LOCATOR_EVIDENCE" in
                            "$USER_UPDATE_ROOT"/diagnostic-driver-v6-*"$LOCATOR_NONCE") LOCATOR_READY=1 ;;
                        esac
                    fi
                    ;;
            esac
        ...
        if [ "$FIXED_READY" -eq 0 ]; then
            ...
            echo 'DIAGNOSTIC_DRIVER_V6_BOOTSTRAP_INCOMPLETE_HOST_PRESERVED root_pointer=absent' >&2
            exit 75
        fi
    fi
    [ -d "$ROOT_SUPPORT" ] && [ ! -L "$ROOT_SUPPORT" ] \
    ...
    case "$RECOVERY_OUTPUT" in
        DIAGNOSTIC_DRIVER_V6_ROOT_RECOVERY_COMMITTED\ *) RECOVERY_OUTCOME='committed' ;;
        DIAGNOSTIC_DRIVER_V6_ROOT_ROLLBACK_COMPLETE\ *) RECOVERY_OUTCOME='rolled-back' ;;
        DIAGNOSTIC_DRIVER_V6_ROOT_PRESTOP_ABORTED\ *) RECOVERY_OUTCOME='prestop-aborted' ;;
        *)
    ...
    if [ -f "$USER_ACTIVE_POINTER" ] && [ ! -L "$USER_ACTIVE_POINTER" ] \
        ...
        case "$RECOVERY_EVIDENCE" in
            "$USER_UPDATE_ROOT"/diagnostic-driver-v6-*)
...
compile_controller() {
    ...
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        ...
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-diagnostic-driver-v6" \
        --remap-path-prefix "$build_root=/reviewed/opensteamer-diagnostic-driver-v6-build" \
        "$SOURCE" -o "$controller"
