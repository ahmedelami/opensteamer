#!/bin/zsh
# Builds, assembles, signs, and verifies `opensteamer Host.app` from the repository root.
#
...
#
# Environment:
# - OPENSTEAMER_HOST_APP_OUTPUT_DIR: destination parent (defaults to `<repo>/build`).
# - OPENSTEAMER_HOST_PREBUILT_BIN_DIR: reuse already-built SwiftPM products instead of invoking a build.
# - OPENSTEAMER_HOST_CODESIGN_IDENTITY: explicit identity; `-` selects ad-hoc signing.
# - OPENSTEAMER_EXPECTED_TEAM_ID: optional TeamIdentifier enforced by the final verifier.
#
...
APP_OUTPUT_DIR="${OPENSTEAMER_HOST_APP_OUTPUT_DIR:-$ROOT_DIR/build}"
APP_DIR="$APP_OUTPUT_DIR/opensteamer Host.app"
...
if [[ -n "${OPENSTEAMER_HOST_PREBUILT_BIN_DIR:-}" ]]; then
    ...
    BIN_DIR="$OPENSTEAMER_HOST_PREBUILT_BIN_DIR"
else
...
cp "macOS/OpensteamerHost/Info.plist" "$CONTENTS_DIR/Info.plist"
...
# Preserve the shipped designated requirement so Keychain and TCC continuity survive rebrand.
...
if [[ -n "${OPENSTEAMER_EXPECTED_TEAM_ID:-}" ]]; then
    VERIFY_ARGUMENTS+=("$OPENSTEAMER_EXPECTED_TEAM_ID")
fi
"$ROOT_DIR/macOS/scripts/verify-mac-host-bundle.sh" "${VERIFY_ARGUMENTS[@]}"

echo "Signed opensteamer Host with: $SIGNING_IDENTITY" >&2
echo "$APP_DIR"
