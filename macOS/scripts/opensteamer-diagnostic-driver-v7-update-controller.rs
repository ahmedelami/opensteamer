...
const LOCK_PROBE_MODE: &str = "--probe-diagnostic-driver-v7-host-lock";
const LOCK_FREE_PROBE_MODE: &str = "--probe-diagnostic-driver-v7-host-lock-free";
const LOCK_RECORD_PROBE_MODE: &str = "--read-diagnostic-driver-v7-host-lock-record";
...
const ROOT_RECOVERY_CONTROLLER: &str =
    "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v7/recovery-controller";
const ROOT_RECOVERY_CONTROLLER_PIN: &str = "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v7/recovery-controller.sha256";
...
fn audio_device_uid_classified(device: u32) -> std::result::Result<String, RouteCaptureFailure> {
    let address = AudioObjectPropertyAddress {
...
fn audio_device_uid(device: u32) -> Result<String> {
    audio_device_uid_classified(device).map_err(route_capture_failure)
}
...
fn capture_route_snapshot_classified() -> std::result::Result<RouteSnapshot, RouteCaptureFailure> {
    let input = audio_default_device(SELECTOR_DEFAULT_INPUT).map_err(RouteCaptureFailure::Fatal)?;
    let output =
        audio_default_device(SELECTOR_DEFAULT_OUTPUT).map_err(RouteCaptureFailure::Fatal)?;
    let system_output =
        audio_default_device(SELECTOR_DEFAULT_SYSTEM_OUTPUT).map_err(RouteCaptureFailure::Fatal)?;
    require_route_roles(RouteSnapshot {
...
fn stable_coreaudio_generation_before(deadline: Instant) -> Result<CoreAudioGeneration> {
    let first = read_coreaudio_generation_before(deadline)?;
    let remaining = remaining_before_post_reload_deadline(deadline)?;
    thread::sleep(std::cmp::min(Duration::from_millis(250), remaining));
    let second = read_coreaudio_generation_before(deadline)?;
    if first != second {
        return Err(ControllerError(
            "coreaudiod generation changed during deadline-bounded proof".to_owned(),
        ));
    }
    Ok(first)
}
...
fn verify_git_provenance(repo: &Path) -> Result<(String, String)> {
    ...
    if branch != EXPECTED_RELEASE_BRANCH {
        return Err(ControllerError(
            "live release checkout is not the exact dedicated V7 branch".to_owned(),
        ));
    }
...
fn preflight(repo: &Path) -> Result<()> {
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V7_PREFLIGHT_OK host_pid={} host_runs={} coreaudiod_pid={} coreaudiod_runs={} installed_driver={}:{} candidate_commit={} candidate_tree={} release_commit={} release_tree={} pairing=metadata-only legacy=protected retained_v1_v2_v3_v4_v5_v6=immutable reader=passive both_order=no-default-mutation namespaces=fresh",
        host.pid,
        host.runs,
        coreaudio.pid,
        coreaudio.runs,
...
fn create_user_layout(nonce: &str) -> Result<UserLayout> {
    ...
    let evidence = Path::new(USER_UPDATE_ROOT).join(format!(
        "diagnostic-driver-v7-{seconds}-{}-{nonce}",
        std::process::id()
    ));
...
fn parse_root_request_text(text: &str) -> Result<RootRequest> {
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V7") {
    ...
    if request.reader_sha256 != DIAGNOSTIC_READER_SHA256
        ...
        || !request
            ...
            .and_then(OsStr::to_str)
            .is_some_and(|leaf| {
                leaf.starts_with("diagnostic-driver-v7-") && leaf.ends_with(&request.nonce)
            })
...
fn parse_root_state_text(
    ...
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_STATE_V7") || !text.ends_with('\n')
    {
        return Err(ControllerError(
            ...
            .is_some_and(|evidence| {
                evidence.parent() == Some(Path::new(USER_UPDATE_ROOT))
                    && evidence
                        .file_name()
                        .and_then(OsStr::to_str)
                        .is_some_and(|leaf| leaf.starts_with("diagnostic-driver-v7-"))
...
fn run_passive_driver_validation(
    ...
    let nonce = format!("diagnostic-driver-v7-{}", request.nonce);
...
fn write_root_result(layout: &RootLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RESULT_V7\nstatus={status}\ndetail={detail}\n");
...
fn write_root_recovery_result(layout: &RootLayout, outcome: &str, detail: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V7\noutcome={outcome}\ndetail={detail}\n"
    );
    match fs::symlink_metadata(&layout.recovery_result) {
        ...
        Ok(_) => {
            ...
            if !existing.starts_with("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V7\n")
                || !existing.ends_with('\n')
...
fn perform_root_transaction(request_path: &Path) -> Result<HostGeneration> {
    ...
    if verify_retained_v5_root_rolled_back(&retained_v5_root_lock)? != retained_v5 {
        return Err(ControllerError(
            "retained v5 root boundary changed across the V7 cutover/rollback".to_owned(),
        ));
    }
    if verify_retained_v6_root_rolled_back(&retained_v6_root_lock)? != retained_v6 {
        return Err(ControllerError(
            "retained v6 root boundary changed across the V7 cutover/rollback".to_owned(),
        ));
    }
    outcome
}
...
fn root_authorized_update(request_path: &Path) -> Result<()> {
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V7_UPDATE_COMMITTED host_pid={} host_runs={} pairing=preserved routes=unchanged legacy=protected",
        host.pid, host.runs
    );
    Ok(())
}
...
fn write_user_result(layout: &UserLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_RESULT_V7\nstatus={status}\ndetail={detail}\n");
...
fn execute_authorized_update(
    ...
    if commit != authorized_commit || tree != authorized_tree {
        return Err(ControllerError(
            "explicit authorization differs from the clean pushed dedicated V7 branch HEAD/tree"
                .to_owned(),
        ));
    }
    ...
    if stdout.lines().count() != 1 || !stdout.starts_with("DIAGNOSTIC_DRIVER_V7_UPDATE_COMMITTED ")
    {
        return Err(ControllerError(
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V7_UPDATE_COMPLETE evidence={} host_pid={} pairing=preserved routes=unchanged legacy=protected",
...
fn rollback_authorized_update(repo: &Path) -> Result<()> {
    ...
    let recovery_kind = if line.starts_with("DIAGNOSTIC_DRIVER_V7_ROOT_ROLLBACK_COMPLETE ") {
        "rolled-back"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V7_ROOT_PRESTOP_ABORTED ") {
        "prestop-aborted"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V7_ROOT_RECOVERY_COMMITTED ") {
        "committed"
    } else {
        return Err(ControllerError(
    ...
    verify_pairing_metadata_only()?;
    println!(
        "DIAGNOSTIC_DRIVER_V7_RECOVERY_COMPLETE outcome={recovery_kind} host_pid={} pairing=preserved routes={route_status} legacy=protected",
        host.pid
    );
    Ok(())
}
...
fn complete_root_recovery(_request: RootRequest, layout: RootLayout) -> Result<()> {
    ...
    let (outcome, marker) = match plan {
        RootRecoveryPlan::PrestopPreserveHost => (
            ...
            "DIAGNOSTIC_DRIVER_V7_ROOT_PRESTOP_ABORTED",
        ),
        RootRecoveryPlan::RepairCommittedState | RootRecoveryPlan::ReportCommitted => (
            ...
            "DIAGNOSTIC_DRIVER_V7_ROOT_RECOVERY_COMMITTED",
        ),
        RootRecoveryPlan::RepairRolledBackState | RootRecoveryPlan::ReportRolledBack => (
            ...
            "DIAGNOSTIC_DRIVER_V7_ROOT_ROLLBACK_COMPLETE",
        ),
        ...
        RootRecoveryPlan::ResumeRollback => {
            ...
            (outcome, "DIAGNOSTIC_DRIVER_V7_ROOT_ROLLBACK_COMPLETE")
        }
    ...
    let recovery_outcome = match marker {
        "DIAGNOSTIC_DRIVER_V7_ROOT_RECOVERY_COMMITTED" => "committed",
        "DIAGNOSTIC_DRIVER_V7_ROOT_PRESTOP_ABORTED" => "prestop-aborted",
        "DIAGNOSTIC_DRIVER_V7_ROOT_ROLLBACK_COMPLETE" => "rolled-back",
        _ => {
            return Err(ControllerError(
...
fn write_or_verify_prestop_abort_witness(layout: &RootLayout, journal: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_PRESTOP_ABORT_JOURNAL_V7\noutcome=prestop-aborted\njournal={journal}\n"
    );
    match fs::symlink_metadata(&path) {
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
...
fn finalize_sealed_bootstrap_without_root_pointer(fixed_digest: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_BOOTSTRAP_ABORT_V7\nnonce={}\nhost_pid={}\noutcome=prestop-aborted\n",
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V7_ROOT_PRESTOP_ABORTED host_pid={} routes=unchanged pairing=preserved",
        host.pid
    );
    Ok(())
}
...
fn root_sealed_rollback_authorized_update() -> Result<()> {
    ...
    let Some(layout) = reconcile_root_pointer_for_recovery(&locator_request)? else {
        ...
        {
            return Err(ControllerError(
                "retained v4/v5/v6 root boundary changed before pointerless recovery finalization"
                    .to_owned(),
            ));
        }
    ...
    {
        return Err(ControllerError(
            "retained v4/v5/v6 root boundary changed before sealed recovery".to_owned(),
        ));
    }
...
fn self_test() -> Result<()> {
    ...
    let root_hash_path = Path::new(
        "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v7/controller-0123456789abcdef0123456789abcdef/controller",
    );
    ...
    let synthetic = RootRequest {
        ...
        evidence: PathBuf::from(
            "/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v7/diagnostic-driver-v7-1-1-0123456789abcdef0123456789abcdef",
        ),
    ...
    for (fixture, label) in [
        ...
        (
            request_text.replace(
                ...
                "evidence=/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v7/../escape\n",
            ),
        ...
        (
            request_text.replace(
                ...
                "root_controller=/Library/Application Support/opensteamer/diagnostic-driver-controllers-v7/controller-0123456789abcdef0123456789abcdef/../controller\n",
            ),
    ...
    let pointer_bytes = b"/Library/Application Support/opensteamer/diagnostic-driver-updates-v7/transaction-0123456789abcdef0123456789abcdef\n";
    ...
    let all_transient =
        vec![PostReloadRouteObservation::TransientBadObject; POST_RELOAD_ROUTE_MAX_ATTEMPTS];
    let mut one_complete_at_cap =
        vec![PostReloadRouteObservation::TransientBadObject; POST_RELOAD_ROUTE_MAX_ATTEMPTS - 1];
    ...
    if !uid501_source_path_is_allowed(Path::new(BOTH_ORDER_PROBE), false)
        ...
        || uid501_source_path_is_allowed(
            Path::new("/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v7/../escape/probes/both-order.json"),
            true,
        )
    {
        return Err(ControllerError(
    ...
    let source = include_str!("opensteamer-diagnostic-driver-v7-update-controller.rs");
    let forbidden = [
    ...
    println!("DIAGNOSTIC_DRIVER_V7_SELF_TEST_OK tests=112");
    Ok(())
}
