...
PREFLIGHT_MODE='--verify-diagnostic-driver-v1-update-preflight'
EXECUTE_MODE='--execute-authorized-diagnostic-driver-v1-update'
ROLLBACK_MODE='--rollback-authorized-diagnostic-driver-v1-update'
SELF_TEST_MODE='--self-test-diagnostic-driver-v1-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer'
USER_UPDATE_ROOT='/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v1'
USER_ACTIVE_POINTER='/Users/ahmed/Library/Application Support/opensteamer/active-diagnostic-driver-update-v1'
ROOT_SUPPORT='/Library/Application Support/opensteamer'
ROOT_CONTROLLER_PARENT='/Library/Application Support/opensteamer/diagnostic-driver-controllers-v1'
...
ROOT_ACTIVE_POINTER='/Library/Application Support/opensteamer/active-diagnostic-driver-update-v1'
ROOT_BOOTSTRAP_LOCATOR='/Library/Application Support/opensteamer/diagnostic-driver-bootstrap-v1.txt'
ROOT_SEALED_ROLLBACK_MODE='--root-sealed-rollback-diagnostic-driver-v1-update'
DIAGNOSTIC_READER_SHA256='6d3f5d71b399ada1244b84177d351c2406d426055da8f54e8f689af1c22a8ded'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-diagnostic-driver-v1-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='0a9125e53dce0ee87b76fc83543319064608a8c9b0a8947644e90bf570198e72'
EXPECTED_BINARY_SHA256='26f27b033bc02b9c21860e533a4032644c405b9335c5208b9eba9c8769d4e0ec'
BUILD_PARENT='/Volumes/t7/opensteamer-diagnostic-driver-v1-controller-builds'
...
if [ "$MODE" = "$ROLLBACK_MODE" ]; then
    ...
    if [ ! -e "$ROOT_ACTIVE_POINTER" ] && [ ! -L "$ROOT_ACTIVE_POINTER" ] \
        ...
        if [ -f "$ROOT_BOOTSTRAP_LOCATOR" ] && [ ! -L "$ROOT_BOOTSTRAP_LOCATOR" ] \
            ...
            && [ "$(/usr/bin/sed -n '1p' "$ROOT_BOOTSTRAP_LOCATOR")" = 'OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V1' ]; then
                ...
                *)
                    if [ "${#LOCATOR_NONCE}" -eq 32 ] \
                        ...
                        case "$LOCATOR_EVIDENCE" in
                            "$USER_UPDATE_ROOT"/diagnostic-driver-v1-*"$LOCATOR_NONCE") LOCATOR_READY=1 ;;
                        esac
                    fi
                    ;;
            esac
        ...
        if [ "$FIXED_READY" -eq 0 ]; then
            ...
            echo 'DIAGNOSTIC_DRIVER_V1_BOOTSTRAP_INCOMPLETE_HOST_PRESERVED root_pointer=absent' >&2
            exit 75
        fi
    fi
    [ -d "$ROOT_SUPPORT" ] && [ ! -L "$ROOT_SUPPORT" ] \
    ...
    case "$RECOVERY_OUTPUT" in
        DIAGNOSTIC_DRIVER_V1_ROOT_RECOVERY_COMMITTED\ *) RECOVERY_OUTCOME='committed' ;;
        DIAGNOSTIC_DRIVER_V1_ROOT_ROLLBACK_COMPLETE\ *) RECOVERY_OUTCOME='rolled-back' ;;
        DIAGNOSTIC_DRIVER_V1_ROOT_PRESTOP_ABORTED\ *) RECOVERY_OUTCOME='prestop-aborted' ;;
        *)
    ...
    if [ -f "$USER_ACTIVE_POINTER" ] && [ ! -L "$USER_ACTIVE_POINTER" ] \
        ...
        case "$RECOVERY_EVIDENCE" in
            "$USER_UPDATE_ROOT"/diagnostic-driver-v1-*)
...
compile_controller() {
    ...
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        ...
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-diagnostic-driver-v1" \
        --remap-path-prefix "$build_root=/reviewed/opensteamer-diagnostic-driver-v1-build" \
        "$SOURCE" -o "$controller"
