#!/bin/sh
# Compile and invoke the reviewed Rust pairing-preserving v5 host updater. The launcher performs
# no cutover work itself and accepts only the explicit controller modes below.
set -eu
umask 077

PREFLIGHT_MODE='--verify-paired-v5-host-update-preflight'
EXECUTE_MODE='--execute-authorized-paired-v5-host-update'
ROLLBACK_MODE='--rollback-authorized-paired-v5-host-update'
SELF_TEST_MODE='--self-test-paired-v5-host-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-host-paired-v5-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='bf377d7881b0707a1fd93a2a28c02a16a17c0380c879d91b93d1a05e2dc21e49'
...
EXPECTED_BINARY_SHA256='09cbef7a14dd3e2454878193dabc732b7c0b0be2295ab26a0166ada9ce769aa9'
...
[ "$(/usr/bin/id -u)" = 501 ] || {
    echo 'paired-v5 updater must run as Ahmed without sudo' >&2
    exit 1
}
for reviewed_source in "$SOURCE" "$V1_SOURCE"; do
    [ -f "$reviewed_source" ] && [ ! -L "$reviewed_source" ] \
        ...
        echo "reviewed paired-v5 source has unsafe metadata: $reviewed_source" >&2
        exit 1
    }
done
[ "$(/usr/bin/shasum -a 256 "$SOURCE" | /usr/bin/awk '{print $1}')" = \
  "$EXPECTED_SOURCE_SHA256" ] || {
    echo 'paired-v5 controller source differs from the reviewed bytes' >&2
    exit 1
}
...
BUILD_DIR=$(/usr/bin/mktemp -d "$BUILD_PARENT/.paired-v5-controller-build.XXXXXX") || exit 1
cleanup() {
...
compile_controller() {
    OPENSTEAMER_PAIRED_V5_INCLUDED_SOURCE="$INCLUDED_SOURCE" \
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        --sysroot "$RUSTC_SYSROOT" \
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-paired-v5" \
        --remap-path-prefix "$BUILD_DIR=/reviewed/opensteamer-paired-v5-build" \
        "$SOURCE" -o "$CONTROLLER"
...
/usr/bin/cmp -s "$FIRST" "$CONTROLLER" || {
    echo 'two reviewed paired-v5 controller compilations were not byte-identical' >&2
    exit 1
}
[ "$(/usr/bin/shasum -a 256 "$CONTROLLER" | /usr/bin/awk '{print $1}')" = \
  "$EXPECTED_BINARY_SHA256" ] || {
    echo 'compiled paired-v5 controller differs from the reviewed binary hash' >&2
    exit 1
}
/bin/chmod u+w "$FIRST"
/bin/rm -f "$FIRST"
