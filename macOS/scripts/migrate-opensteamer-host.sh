...
EXPECTED_CONTROLLER_SOURCE_SHA256='c405742ab93e12e0642622b3210bb90092f483e0b49c707b95600c86a9dafccc'
...
[ "$SOURCE_SHA" = "$EXPECTED_CONTROLLER_SOURCE_SHA256" ] || {
    echo "controller source hash differs from the reviewed v13 postimage" >&2
    exit 1
}
verify_companion_script "$BUILD_SCRIPT" "$EXPECTED_BUILD_SCRIPT_SHA256" \
    ...
    echo "one or more controller companion scripts differ from the reviewed v13 postimage" >&2
    exit 1
}
...
BUILD_DIR=$(/usr/bin/mktemp -d "$BUILD_PARENT/.controller-build-v13.XXXXXX") || {
