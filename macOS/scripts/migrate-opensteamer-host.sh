...
EXPECTED_CONTROLLER_SOURCE_SHA256='65e894c896402e79853f7025f30f6c7f4db19075589b0b34e22dbea97d9f2905'
EXPECTED_CONTROLLER_BINARY_SHA256='d9a32468966eac830ffd6b0453b6f9282fc7d010919dc4f414d20f1af8542770'
EXPECTED_BUILD_SCRIPT_SHA256='96ffbce9ecff2ece81f1901001108ad3b14d3991e0d8481341db7125fa81365a'
EXPECTED_BUNDLE_VERIFIER_SHA256='02a348a88d25b76ab95d45620d823339212bb53ee0f39bfb3a52f04240d3d745'
EXPECTED_PRIVATE_VIRTUAL_DISPLAY_IMPORT_VERIFIER_SHA256='e0e54968452471c4120dce3e9fb744874882871718ba49c23c8f0adaeda823b1'
...
PRIVATE_VIRTUAL_DISPLAY_IMPORT_VERIFIER="$ROOT/macOS/scripts/verify-no-private-virtual-display-imports.sh"
...
[ "$SOURCE_SHA" = "$EXPECTED_CONTROLLER_SOURCE_SHA256" ] || {
    echo "controller source hash differs from the reviewed v20 postimage" >&2
    exit 1
}
verify_companion_script "$BUILD_SCRIPT" "$EXPECTED_BUILD_SCRIPT_SHA256" \
    ...
    && verify_companion_script "$PRIVATE_VIRTUAL_DISPLAY_IMPORT_VERIFIER" \
      "$EXPECTED_PRIVATE_VIRTUAL_DISPLAY_IMPORT_VERIFIER_SHA256" \
    ...
    echo "one or more controller companion scripts differ from the reviewed v20 postimage" >&2
    exit 1
}
...
BUILD_DIR=$(/usr/bin/mktemp -d "$BUILD_PARENT/.controller-build-v20.XXXXXX") || {
...
copy_companion_script "$BUILD_SCRIPT" "$EXPECTED_BUILD_SCRIPT_SHA256" \
    ...
    && copy_companion_script "$PRIVATE_VIRTUAL_DISPLAY_IMPORT_VERIFIER" \
      "$EXPECTED_PRIVATE_VIRTUAL_DISPLAY_IMPORT_VERIFIER_SHA256" \
...
verify_private_companion_script "$BUILD_SCRIPT" "$EXPECTED_BUILD_SCRIPT_SHA256" \
    ...
    && verify_private_companion_script "$PRIVATE_VIRTUAL_DISPLAY_IMPORT_VERIFIER" \
      "$EXPECTED_PRIVATE_VIRTUAL_DISPLAY_IMPORT_VERIFIER_SHA256" \
...
[ "$BINARY_SHA" = "$EXPECTED_CONTROLLER_BINARY_SHA256" ] || {
    echo "fresh controller binary differs from the reviewed reproducible postimage: expected $EXPECTED_CONTROLLER_BINARY_SHA256, got $BINARY_SHA" >&2
    exit 1
}
