...
//! This controller owns a disjoint v5 journal/pointer namespace, never starts an interactive
//! host, never resets pairing, and rolls back to the exact committed pairing-preserving v4 host.

#[allow(dead_code)]
mod paired_v5 {
    include!(env!("OPENSTEAMER_PAIRED_V5_INCLUDED_SOURCE"));

    const V5_PREFLIGHT_MODE: &str = "--verify-paired-v5-host-update-preflight";
    const V5_EXECUTE_MODE: &str = "--execute-authorized-paired-v5-host-update";
    const V5_ROLLBACK_MODE: &str = "--rollback-authorized-paired-v5-host-update";
    const V5_SELF_TEST_MODE: &str = "--self-test-paired-v5-host-update";
    const V5_EXPECTED_REPO: &str = "/Users/ahmed/Documents/Codex/opensteamer";

    const V5_UPDATE_ROOT: &str =
        "/Users/ahmed/Library/Application Support/opensteamer/paired-host-updates-v5";
    const V5_ACTIVE_UPDATE: &str =
        "/Users/ahmed/Library/Application Support/opensteamer/active-paired-host-update-v5";
    const V5_UPDATE_LOCK: &str = UPDATE_LOCK;
    const V5_JOURNAL_HEADER: &str = "OPENSTEAMER_PAIRED_HOST_UPDATE_V5";
    const HIDDEN_INSTALL_PREFIX: &str = ".opensteamer-paired-v5-install-";
    ...
    const CURRENT_BASELINE_APP: &str = "/Users/ahmed/Library/Application Support/opensteamer/paired-host-updates-v4/paired-v4-update-1786291257-27621-6a237b6f-a9cc-4adb-a48a-129d364f8073/deployment-reference/opensteamer Host.app";
    const CURRENT_BASELINE_SOURCE_EXPORT: &str = "/Users/ahmed/Library/Application Support/opensteamer/paired-host-updates-v4/paired-v4-update-1786291257-27621-6a237b6f-a9cc-4adb-a48a-129d364f8073/source-export";
    const CURRENT_BASELINE_EXECUTABLE_SHA256: &str =
        "ce0c1347aa6ddf7ecd290729d8351c65dc1bc43d99416f6a4c17141db7371a4b";
    const CURRENT_BASELINE_CDHASH: &str = "47ff9ae616f6b0b14880e7e419b00ec6a88193d7";
    ...
    const REQUIRED_MICROPHONE_TELEMETRY_COMMIT: &str = "77cfe939813b9b719ca328b6cb0e69196ce3cf2d";
    ...
    impl V5Journal {
        fn create(path: &Path) -> Result<Self> {
            let mut file = OpenOptions::new()
                ...
                .open(path)
                .map_err(|error| ControllerError(format!("cannot create v5 journal: {error}")))?;
            validate_open_journal_file(path, &file)?;
            writeln!(file, "{V5_JOURNAL_HEADER}")?;
            file.sync_all()?;
            fsync_parent(path)?;
        ...
        fn open(path: &Path) -> Result<Self> {
            ...
            let complete_text = std::str::from_utf8(&bytes[..complete_length])
                .map_err(|_| ControllerError("v5 journal is not UTF-8".to_owned()))?;
            let state = parse_v5_journal(complete_text)?;
            if complete_length != bytes.len() {
                if !is_plausible_v5_torn_tail(&bytes[complete_length..], state) {
                    return Err(ControllerError(
                        "v5 journal has an implausible incomplete final record".to_owned(),
                    ));
                }
                validate_open_journal_file(path, &file)?;
        ...
        fn record(&mut self, state: V5State, fields: &[(&str, String)]) -> Result<()> {
            self.require_healthy()?;
            validate_v5_transition(self.state, state)?;
            validate_v5_fields(state, fields)?;
                ...
                .and_then(|_| self.file.sync_all())
            {
                ...
                return match recovery {
                    Ok(()) => Err(ControllerError(format!(
                        "cannot durably append v5 journal record: {error}"
                    ))),
                    Err(recovery_error) => Err(ControllerError(format!(
                        "cannot durably append v5 journal record ({error}) or restore prior length ({recovery_error})"
                    ))),
                };
            }
        ...
        fn require_healthy(&mut self) -> Result<()> {
            if !self.healthy {
                return Err(ControllerError(
                    "v5 journal is poisoned after an unrecovered append failure".to_owned(),
                ));
            }
        ...
        fn restore_after_failed_append(&mut self, prior_length: u64) -> Result<()> {
            ...
            if bytes.len() as u64 != prior_length || !bytes.ends_with(b"\n") {
                return Err(ControllerError(
                    "v5 journal append recovery did not restore prior durable length".to_owned(),
                ));
            }
            let text = std::str::from_utf8(&bytes)
                .map_err(|_| ControllerError("recovered v5 journal is not UTF-8".to_owned()))?;
            if parse_v5_journal(text)? != self.state {
                return Err(ControllerError(
                    "v5 journal append recovery did not restore prior state".to_owned(),
                ));
            }
    ...
    impl V5Layout {
        fn new(repo: PathBuf, evidence: PathBuf, nonce: &str) -> Self {
            ...
            let install_hold_root = PathBuf::from(format!(
                "/Applications/.opensteamer-paired-v5-install-{nonce}"
            ));
            Self {
                repo,
    ...
    pub fn entry() {
        if let Err(error) = paired_v5_real_main() {
            eprintln!("opensteamer paired-v5 update controller: {error}");
            std::process::exit(1);
        }
    }

    fn paired_v5_real_main() -> Result<()> {
        let arguments: Vec<String> = env::args().collect();
        verify_optimized_binary_scrub()?;
        match parse_v5_command(&arguments)? {
            V5Command::Preflight(repo) => {
                let repo = canonical_repo(&repo)?;
                verify_machine_contract()?;
                let _transaction_lock = acquire_update_transaction_lock()?;
                verify_committed_v4_baseline()?;
                let generation = verify_paired_v5_runtime()?;
                let provenance = verify_paired_v5_git_provenance(&repo, true)?;
                verify_isolated_pairing_items_present()?;
                require_path_absent(Path::new(V5_ACTIVE_UPDATE), "active paired-v5 pointer")?;
                require_v5_update_root_unused()?;
                println!(
                    "PAIRED_V5_UPDATE_PREFLIGHT_OK pid={} runs={} baseline=sole-ready pairing=preserved v1=immutable v2=immutable v3=immutable v4=immutable v5=absent commit={} tree={}",
            ...
            V5Command::Execute {
                ...
                authorized_commit,
                authorized_tree,
            } => execute_paired_v5_update(
                canonical_repo(&repo)?,
                &authorized_commit,
                &authorized_tree,
            ),
            V5Command::Rollback(repo) => rollback_existing_paired_v5_update(canonical_repo(&repo)?),
            V5Command::SelfTest => paired_v5_self_test(),
            V5Command::ProbeLock { runtime, lock, pid } => {
    ...
    fn parse_v5_command(arguments: &[String]) -> Result<V5Command> {
        match arguments {
            [_, mode, repo] if mode == V5_PREFLIGHT_MODE => {
                Ok(V5Command::Preflight(repo.clone()))
            }
            [_, mode, repo, authorized_commit, authorized_tree] if mode == V5_EXECUTE_MODE => {
                ...
                Ok(V5Command::Execute {
                    repo: repo.clone(),
                    authorized_commit: authorized_commit.clone(),
                    authorized_tree: authorized_tree.clone(),
                })
            }
            [_, mode, repo] if mode == V5_ROLLBACK_MODE => Ok(V5Command::Rollback(repo.clone())),
            [_, mode] if mode == V5_SELF_TEST_MODE => Ok(V5Command::SelfTest),
            [_, mode, runtime, lock, pid] if mode == PROBE_LOCK_MODE => {
                Ok(V5Command::ProbeLock {
            ...
            _ => Err(ControllerError(format!(
                "usage: {} {V5_PREFLIGHT_MODE} <canonical-repo>\n       {} {V5_EXECUTE_MODE} <canonical-repo> <authorized-commit> <authorized-tree>\n       {} {V5_ROLLBACK_MODE} <canonical-repo>\n       {} {V5_SELF_TEST_MODE}\n       {} {PROBE_LOCK_MODE} <runtime-dir> <lock-file> <pid>",
    ...
    fn verify_optimized_binary_scrub() -> Result<()> {
        ...
        let executable = env::current_exe().map_err(|error| {
            ControllerError(format!("cannot resolve the paired-v5 controller: {error}"))
        })?;
        let mut file = File::open(&executable).map_err(|error| {
            ControllerError(format!("cannot inspect the paired-v5 controller: {error}"))
        })?;
        ...
        {
            return Err(ControllerError(
                "paired-v5 controller binary has unsafe metadata".to_owned(),
            ));
        }
        ...
        {
            return Err(ControllerError(
                "paired-v5 controller changed while being scrub-verified".to_owned(),
            ));
        }
        for (index, encoded) in FORBIDDEN_MARKER_HEX.iter().enumerate() {
            ...
            {
                return Err(ControllerError(format!(
                    "optimized paired-v5 controller retained forbidden legacy marker {index}"
                )));
            }
        }
        Ok(())
    ...
    fn decode_marker_hex(encoded: &str) -> Result<Vec<u8>> {
        if encoded.len() % 2 != 0 {
            return Err(ControllerError(
                "paired-v5 binary scrub marker has odd length".to_owned(),
            ));
        }
        encoded
            .as_bytes()
    ...
    fn decode_hex_nibble(value: u8) -> Result<u8> {
        match value {
            ...
            _ => Err(ControllerError(
                "paired-v5 binary scrub marker is not lowercase hexadecimal".to_owned(),
            )),
        }
    }

    fn require_v5_update_root_unused() -> Result<()> {
        require_v5_update_root_unused_at(Path::new(V5_UPDATE_ROOT))
    }

    fn require_v5_update_root_unused_at(root: &Path) -> Result<()> {
        match fs::symlink_metadata(root) {
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(()),
            Err(error) => {
                return Err(ControllerError(format!(
                    "cannot inspect paired-v5 update root {}: {error}",
                    root.display()
                )))
            }
        ...
        let mut entries = fs::read_dir(root).map_err(|error| {
            ControllerError(format!(
                "cannot enumerate paired-v5 update root {}: {error}",
                root.display()
            ))
        })?;
        if entries.next().transpose()?.is_some() {
            return Err(ControllerError(
                "paired-v5 update root already contains retained attempt evidence; a second attempt is not authorized"
                    .to_owned(),
            ));
        }
        Ok(())
    ...
    fn prepare_v5_update_root_for_first_attempt() -> Result<()> {
        let root = Path::new(V5_UPDATE_ROOT);
        match fs::symlink_metadata(root) {
        ...
        require_v5_update_root_unused_at(root)
    }

    fn execute_paired_v5_update(
        repo: PathBuf,
        ...
        require_canonical_git_oid(authorized_tree, "authorized tree")?;
        verify_machine_contract()?;
        let transaction_lock = acquire_update_transaction_lock_at(Path::new(V5_UPDATE_LOCK))?;
        verify_committed_v4_baseline()?;
        let initial_generation = verify_paired_v5_runtime()?;
        verify_isolated_pairing_items_present()?;
        let provenance = verify_paired_v5_git_provenance(&repo, true)?;
        ...
        require_available_bytes(
            ...
            2 * 1_024 * 1_024 * 1_024,
            "before creating paired-v5 update evidence",
        )?;

        let nonce = new_nonce()?;
        let evidence = PathBuf::from(V5_UPDATE_ROOT).join(format!(
            "paired-v5-update-{}-{}-{}",
            unix_seconds()?,
            std::process::id(),
            nonce
        ...
        let layout = V5Layout::new(repo, evidence, &nonce);
        create_private_directory(&layout.rollback_dir)?;
        create_private_directory(&layout.failed_dir)?;
        let mut journal = V5Journal::create(&layout.journal)?;
        record_v5_install_hold_name(&layout)?;

        let result = perform_paired_v5_update(
            &layout,
            &mut journal,
            &provenance,
            &initial_generation,
        ...
        match result {
            Ok(()) => Ok(()),
            Err(primary) => {
                if journal.state == V5State::Committed {
                    let _ = write_result(
                        &layout.result,
                    ...
                    eprintln!(
                        "warning: paired-v5 update committed but final reporting failed: {primary}"
                    );
                    return Ok(());
                }
                let pointer_absent_before_stop = journal.state == V5State::StopInitiated
                    && matches!(
                        fs::symlink_metadata(V5_ACTIVE_UPDATE),
                        Err(error) if error.kind() == std::io::ErrorKind::NotFound
                    );
                let crossed_stop = journal.state >= V5State::StopInitiated
                    && journal.state < V5State::Committed
                    && !pointer_absent_before_stop;
                if !crossed_stop {
                    if layout.rollback_reserve.exists() {
                        let _ = release_rollback_reserve(&layout.rollback_reserve);
                    }
                    let _ = archive_v5_install_hold_root(&layout);
                    let _ = write_result(
                        &layout.result,
                        "failed-before-stop",
                        Some(&primary.to_string()),
                ...
                verify_v5_active_pointer(&layout.evidence)?;
                match rollback_to_current_baseline(&layout, &mut journal, &transaction_lock) {
                    Ok(()) => {
                        ...
                        retire_v5_active_pointer(&layout)?;
                    ...
                    Err(rollback) => {
                        ...
                        Err(ControllerError(format!(
                            "CRITICAL: paired-v5 update and rollback both failed; keep host offline; evidence={}: primary={primary}; rollback={rollback}",
                            layout.evidence.display()
                        )))
                    }
                }
    ...
    fn perform_paired_v5_update(
        layout: &V5Layout,
        journal: &mut V5Journal,
        provenance: &Provenance,
        initial_generation: &LaunchGeneration,
        authorized_commit: &str,
        authorized_tree: &str,
    ) -> Result<()> {
        export_v5_source(layout, provenance)?;
        journal.record(
            V5State::SourceExported,
            &[
        ...
        build_and_verify_v5_staged_app(layout)?;
        prepare_v5_deployment_reference(layout)?;
        journal.record(
            V5State::BuildVerified,
            &[(
                "executable_sha256",
        ...
        verify_v5_deployment_reference(layout)?;
        verify_isolated_pairing_items_present()?;
        verify_v4_pointer_unchanged()?;
        let revalidated = verify_paired_v5_runtime()?;
        ...
        let boundary_provenance = verify_paired_v5_git_provenance(&layout.repo, true)?;
        ...
        publish_v5_active_pointer(&layout.evidence)?;
        prepare_v5_install_hold(layout)?;
        journal.record(V5State::InstallHoldVerified, &[])?;

        verify_v5_active_pointer(&layout.evidence)?;
        verify_v5_deployment_reference(layout)?;
        verify_isolated_pairing_items_present()?;
        verify_v4_pointer_unchanged()?;
        let final_generation = verify_paired_v5_runtime()?;
        ...
        journal.record(V5State::CurrentStopped, &[])?;
        ...
        journal.record(V5State::CurrentHeld, &[])?;
        ...
        fs::remove_dir(&layout.install_hold_root).map_err(|error| {
            ControllerError(format!(
                "cannot remove empty paired-v5 install-hold root {}: {error}",
                layout.install_hold_root.display()
            ))
        })?;
        fsync_parent(&layout.install_hold_root)?;
        verify_v5_installed_matches_reference(layout)?;
        verify_isolated_pairing_items_present()?;
        verify_v4_pointer_unchanged()?;
        journal.record(V5State::NewPublished, &[])?;
        drop(lock);

        let checkpoint = capture_log_checkpoint()?;
        bootstrap_exact_new_job()?;
        journal.record(V5State::PersistentBootstrapped, &[])?;
        let generation = wait_for_paired_v5_launch_generation(Duration::from_secs(45))?;
        verify_paired_v5_deployment(layout, &checkpoint, &generation)?;
        verify_isolated_pairing_items_present()?;
        verify_v4_pointer_unchanged()?;
        journal.record(
        ...
        journal.record(V5State::Committed, &[])?;
        if let Err(error) = write_result(&layout.result, "success", None) {
            eprintln!("warning: paired-v5 update committed but result recording failed: {error}");
        }
        println!(
            "PAIRED_V5_HOST_UPDATE_COMMITTED evidence={} pid={} rollback=current-isolated-retained pairing=preserved",
            layout.evidence.display(),
            generation.pid
        );
        Ok(())
    ...
    fn rollback_existing_paired_v5_update(repo: PathBuf) -> Result<()> {
        verify_machine_contract()?;
        let transaction_lock = acquire_update_transaction_lock_at(Path::new(V5_UPDATE_LOCK))?;
        verify_committed_v4_baseline()?;
        let evidence =
            read_update_pointer_at(Path::new(V5_ACTIVE_UPDATE), Path::new(V5_UPDATE_ROOT))?;
        let layout = v5_layout_from_existing(repo, evidence)?;
        let mut journal = V5Journal::open(&layout.journal)?;
        if journal.state == V5State::RolledBack {
            verify_paired_v5_runtime()?;
            verify_isolated_pairing_items_present()?;
            ensure_rolled_back_result(&layout.result)?;
            retire_v5_active_pointer(&layout)?;
            println!("PAIRED_V5_HOST_UPDATE_ALREADY_ROLLED_BACK");
            return Ok(());
        }
        ...
        retire_v5_active_pointer(&layout)?;
        println!(
            "PAIRED_V5_HOST_UPDATE_ROLLED_BACK evidence={} pairing=preserved",
            layout.evidence.display()
        );
        Ok(())
    }
    ...
    fn rollback_to_current_baseline(
        ...
        let already_rolling_back = matches!(
            journal.state,
            V5State::RollbackStarted
                | V5State::FailedNewArchived
                | V5State::CurrentRestored
                | V5State::CurrentBootstrapped
        );
        if !already_rolling_back {
            journal.record(V5State::RollbackStarted, &[])?;
        }
        if layout.rollback_reserve.exists() {
            release_rollback_reserve(&layout.rollback_reserve)?;
        }

        archive_v5_install_hold_root(layout)?;
        verify_v5_active_pointer(&layout.evidence)?;
        verify_v4_pointer_unchanged()?;
        verify_isolated_pairing_items_present()?;
        bootout_paired_v5_job_if_loaded(layout)?;
        wait_for_no_capture_servers(Duration::from_secs(30))?;
        require_service_absent(NEW_LAUNCH_AGENT_LABEL)?;
        verify_protected_legacy_absent()?;
        let lock = acquire_unowned_shared_lock()?;
        ...
        if rollback_exists {
            ...
            if canonical_exists {
                ...
                if journal.state == V5State::RollbackStarted {
                    journal.record(V5State::FailedNewArchived, &[])?;
                }
            } else if failed_exists && journal.state == V5State::RollbackStarted {
                journal.record(V5State::FailedNewArchived, &[])?;
            }
            ...
            if matches!(
                journal.state,
                V5State::RollbackStarted | V5State::FailedNewArchived
            ) {
                journal.record(V5State::CurrentRestored, &[])?;
            }
        ...
        if !matches!(
            journal.state,
            V5State::CurrentRestored | V5State::CurrentBootstrapped
        ) {
            return Err(ControllerError(format!(
                "paired-v5 rollback topology is not resumable from {}",
                journal.state.token()
            )));
        }
        verify_isolated_pairing_items_present()?;
        ...
        let checkpoint = capture_log_checkpoint()?;
        bootstrap_exact_new_job()?;
        if journal.state == V5State::CurrentRestored {
            journal.record(V5State::CurrentBootstrapped, &[])?;
        }
        let generation = wait_for_paired_v5_launch_generation(Duration::from_secs(45))?;
        ...
        verify_v4_pointer_unchanged()?;
        verify_protected_legacy_absent()?;
        journal.record(V5State::RolledBack, &[])?;
        verify_paired_v5_runtime()?;
        Ok(())
    }

    fn v5_layout_from_existing(repo: PathBuf, evidence: PathBuf) -> Result<V5Layout> {
        require_descendant(Path::new(V5_UPDATE_ROOT), &evidence)?;
        ...
        let nonce = install_hold_root
            ...
            .ok_or_else(|| {
                ControllerError("paired-v5 install-hold name is malformed".to_owned())
            })?;
        let expected = V5Layout::new(repo, evidence, nonce);
        if expected.install_hold_root != install_hold_root {
            return Err(ControllerError(
                "paired-v5 install-hold path escaped its recorded layout".to_owned(),
            ));
        }
        require_v5_install_hold_layout(&expected.install_hold_root, &expected.install_hold)?;
        Ok(expected)
    }

    fn export_v5_source(layout: &V5Layout, provenance: &Provenance) -> Result<()> {
        require_path_absent(&layout.source_tar, "v5 source archive")?;
        require_path_absent(&layout.source_export, "v5 source export")?;
        ...
        let status = Command::new("/usr/bin/git")
            ...
            .stderr(Stdio::piped())
            .status()?;
        require_success(status, "git archive for paired-v5 update")?;
        ...
        require_output_success(&output, "extract paired-v5 source archive")?;
        require_regular(
            &layout
                .source_export
        ...
        require_regular(
            &layout
                .source_export
                .join("macOS/scripts/opensteamer-host-paired-v5-update-controller.rs"),
            0o600,
        )?;
    ...
    fn build_and_verify_v5_staged_app(layout: &V5Layout) -> Result<()> {
        require_path_absent(&layout.stage_output, "paired-v5 staged output")?;
        require_path_absent(&layout.scratch, "paired-v5 SwiftPM scratch")?;
        ...
        let status = Command::new(&build_script)
            ...
            .status()?;
        require_success(status, "fresh signed paired-v5 host build")?;
        ...
        if sha256(&executable)? == CURRENT_BASELINE_EXECUTABLE_SHA256 {
            return Err(ControllerError(
                "paired-v5 staged executable is byte-identical to the current isolated baseline"
                    .to_owned(),
            ));
        }
    ...
    fn verify_staged_pairing_namespace(executable: &Path) -> Result<()> {
        ...
        require_output_success(&strings, "inspect paired-v5 staged pairing namespace")?;
        let text = decode_utf8(&strings.stdout, "paired-v5 strings output")?;
        ...
        if isolated_count != 1 || protected_count != 0 {
            return Err(ControllerError(format!(
                "paired-v5 staged pairing namespace is not isolated: isolated_count={isolated_count} protected_count={protected_count}"
            )));
        }
        Ok(())
    }

    fn prepare_v5_deployment_reference(layout: &V5Layout) -> Result<()> {
        require_path_absent(
            &layout.deployment_reference_dir,
            "paired-v5 deployment-reference directory",
        )?;
        ...
        require_output_success(&output, "copy paired-v5 deployment reference")?;
        verify_v5_deployment_reference(layout)
    }

    fn verify_v5_deployment_reference(layout: &V5Layout) -> Result<()> {
        ...
        for app in [&layout.staged_app, &layout.deployment_reference_app] {
            ...
            if sha256(&executable)? == CURRENT_BASELINE_EXECUTABLE_SHA256 {
                return Err(ControllerError(
                    "paired-v5 deployment reference equals current isolated baseline".to_owned(),
                ));
            }
            verify_staged_pairing_namespace(&executable)?;
        }
    ...
    fn prepare_v5_install_hold(layout: &V5Layout) -> Result<()> {
        require_v5_install_hold_layout(&layout.install_hold_root, &layout.install_hold)?;
        require_path_absent(&layout.install_hold_root, "paired-v5 hidden install hold")?;
        ...
        require_output_success(&output, "copy paired-v5 install hold")?;
        verify_bundle(
            &layout.source_export,
            &layout.install_hold,
            false,
    ...
    fn record_v5_install_hold_name(layout: &V5Layout) -> Result<()> {
        require_v5_install_hold_layout(&layout.install_hold_root, &layout.install_hold)?;
    ...
    fn archive_v5_install_hold_root(layout: &V5Layout) -> Result<()> {
        require_v5_install_hold_layout(&layout.install_hold_root, &layout.install_hold)?;
        ...
        archive_v5_install_hold_root_at(&layout.install_hold_root, &archive)
    }

    fn archive_v5_install_hold_root_at(install_hold_root: &Path, archive: &Path) -> Result<()> {
    ...
    fn verify_v5_installed_matches_reference(layout: &V5Layout) -> Result<()> {
        verify_bundle(
            &layout.source_export,
            Path::new(NEW_APP),
            true,
    ...
    fn verify_paired_v5_deployment(
        layout: &V5Layout,
    ...
    fn verify_v4_pointer_unchanged() -> Result<()> {
        verify_committed_v4_baseline()
    }
    ...
    fn verify_reviewed_launch_agent_unchanged() -> Result<()> {
        ...
        if sha256(&source)? != REVIEWED_LAUNCH_AGENT_SHA256 {
            return Err(ControllerError(
                "committed v4 LaunchAgent source changed".to_owned(),
            ));
        }
        Ok(())
    }
    ...
    fn verify_paired_v5_runtime() -> Result<LaunchGeneration> {
        verify_committed_v4_baseline()?;
        verify_protected_legacy_absent()?;
        verify_reviewed_launch_agent_unchanged()?;
        verify_current_baseline_app_at(Path::new(NEW_APP), true)?;
    ...
    fn verify_paired_v5_git_provenance(repo: &Path, require_remote: bool) -> Result<Provenance> {
        let status = command_output(
            "/usr/bin/git",
            &["status", "--porcelain=v1", "--untracked-files=all"],
            Some(repo),
        )?;
        require_output_success(&status, "inspect paired-v5 git worktree")?;
        if !status.stdout.is_empty() {
            return Err(ControllerError(
                "repository must be completely clean before a paired-v5 host update".to_owned(),
            ));
        }
        ...
        require_output_success(&ancestry, "verify required microphone telemetry ancestry")?;
        if require_remote {
            let output = command_output(
                "/usr/bin/git",
                &[
            ...
            require_output_success(&output, "verify pushed paired-v5 source commit")?;
            let text = decode_utf8(&output.stdout, "paired-v5 git ls-remote output")?;
            let records: Vec<&str> = text.lines().collect();
            let expected_ref = format!("refs/heads/{EXPECTED_SOURCE_BRANCH}");
            if records.len() != 1 {
                return Err(ControllerError(
                    "paired-v5 source branch is absent or ambiguous on origin".to_owned(),
                ));
            }
            ...
            {
                return Err(ControllerError(
                    "origin paired-v5 source branch does not resolve to local HEAD".to_owned(),
                ));
            }
        }
        Ok(Provenance {
    ...
    fn wait_for_paired_v5_launch_generation(timeout: Duration) -> Result<LaunchGeneration> {
        wait_for_launch_generation(timeout)
    }

    fn bootout_paired_v5_job_if_loaded(layout: &V5Layout) -> Result<()> {
        ...
        if state.status.success() {
            let loaded = parse_loaded_launch_job(decode_utf8(
                &state.stdout,
                "paired-v5 rollback launchctl state",
            )?)?;
            ...
            require_output_success(&output, "boot out paired-v5 LaunchAgent during rollback")?;
    ...
    fn verify_live_canonical_process(
        ...
        require_output_success(&output, "verify canonical paired-v5 host mapped code")
    }

    fn require_v5_install_hold_layout(root: &Path, app: &Path) -> Result<()> {
        ...
        {
            return Err(ControllerError(format!(
                "paired-v5 install hold escaped its reviewed layout: {}",
                app.display()
            )));
        }
        Ok(())
    ...
    fn publish_v5_active_pointer(evidence: &Path) -> Result<()> {
        require_descendant(Path::new(V5_UPDATE_ROOT), evidence)?;
        let pending = PathBuf::from(format!("{V5_ACTIVE_UPDATE}.pending-{}", std::process::id()));
        require_path_absent(&pending, "pending paired-v5 pointer")?;
        require_path_absent(Path::new(V5_ACTIVE_UPDATE), "active paired-v5 pointer")?;
        let mut file = create_new_private(&pending)?;
        writeln!(file, "{}", evidence.display())?;
        file.sync_all()?;
        rename_exclusive(&pending, Path::new(V5_ACTIVE_UPDATE))?;
        fsync_parent(Path::new(V5_ACTIVE_UPDATE))
    }

    fn verify_v5_active_pointer(expected_evidence: &Path) -> Result<()> {
        verify_update_pointer_at(
            Path::new(V5_ACTIVE_UPDATE),
            expected_evidence,
            Path::new(V5_UPDATE_ROOT),
        )
    }

    fn retire_v5_active_pointer(layout: &V5Layout) -> Result<()> {
    ...
    fn parse_v5_journal(text: &str) -> Result<V5State> {
        let mut lines = text.lines();
        if lines.next() != Some(V5_JOURNAL_HEADER) {
            return Err(ControllerError(
                "paired-v5 journal header is malformed".to_owned(),
            ));
        }
        let mut state = None;
        for line in lines {
            ...
            if fields.next() != Some("STATE") {
                return Err(ControllerError(
                    "paired-v5 journal record is malformed".to_owned(),
                ));
            }
            let next = fields
                .next()
                .and_then(V5State::parse)
                .ok_or_else(|| ControllerError("paired-v5 journal state is unknown".to_owned()))?;
            if let Some(previous) = state {
                validate_v5_transition(previous, next)?;
            } else if next != V5State::Begun {
                return Err(ControllerError(
                    "paired-v5 journal does not begin at BEGUN".to_owned(),
                ));
            }
            let fields: Vec<&str> = fields.collect();
            let expected = v5_field_schema(next);
            if fields.len() != expected.len() {
                return Err(ControllerError(
                    "paired-v5 journal field count is invalid".to_owned(),
                ));
            }
            for (field, expected_key) in fields.into_iter().zip(expected) {
                let (key, value) = field.split_once('=').ok_or_else(|| {
                    ControllerError("paired-v5 journal field is malformed".to_owned())
                })?;
                if key != *expected_key || !is_safe_journal_value(value) {
                    return Err(ControllerError(
                        "paired-v5 journal field is unsafe".to_owned(),
                    ));
                }
            }
            state = Some(next);
        }
        state.ok_or_else(|| ControllerError("paired-v5 journal has no state".to_owned()))
    }

    fn v5_field_schema(state: V5State) -> &'static [&'static str] {
        match state {
            V5State::Begun
            | V5State::InstallHoldVerified
            | V5State::CurrentStopped
            | V5State::CurrentHeld
            ...
            | V5State::CurrentBootstrapped
            | V5State::RolledBack => &[],
            V5State::SourceExported => &["commit", "tree", "initial_pid"],
            V5State::BuildVerified => &["executable_sha256"],
            V5State::StopInitiated => &["reserve_device", "reserve_inode", "reserve_bytes"],
            V5State::ReadyVerified => &["pid", "runs", "nonce"],
            V5State::CriticalFailure => &["phase"],
        }
    }

    fn validate_v5_fields(state: V5State, fields: &[(&str, String)]) -> Result<()> {
        let expected = v5_field_schema(state);
        if fields.len() != expected.len() {
            return Err(ControllerError(
                "paired-v5 journal record has wrong field count".to_owned(),
            ));
        }
        for ((key, value), expected_key) in fields.iter().zip(expected) {
            if *key != *expected_key || !is_safe_journal_value(value) {
                return Err(ControllerError("unsafe paired-v5 journal field".to_owned()));
            }
        }
        Ok(())
    }

    fn validate_v5_transition(previous: V5State, next: V5State) -> Result<()> {
        if previous == V5State::Begun && next == V5State::Begun {
            return Ok(());
        }
        let forward = matches!(
            (previous, next),
            (V5State::Begun, V5State::SourceExported)
                | (V5State::SourceExported, V5State::BuildVerified)
                | (V5State::BuildVerified, V5State::StopInitiated)
                | (V5State::StopInitiated, V5State::InstallHoldVerified)
                | (V5State::InstallHoldVerified, V5State::CurrentStopped)
                | (V5State::CurrentStopped, V5State::CurrentHeld)
                | (V5State::CurrentHeld, V5State::NewPublished)
                | (V5State::NewPublished, V5State::PersistentBootstrapped)
                | (V5State::PersistentBootstrapped, V5State::ReadyVerified)
                | (V5State::ReadyVerified, V5State::Committed)
        );
        let rollback_entry = next == V5State::RollbackStarted
            && ((previous >= V5State::StopInitiated && previous <= V5State::Committed)
                || previous == V5State::CriticalFailure)
            && previous != V5State::RollbackStarted;
        let rollback = matches!(
            (previous, next),
            (V5State::RollbackStarted, V5State::FailedNewArchived)
                | (V5State::RollbackStarted, V5State::CurrentRestored)
                | (V5State::FailedNewArchived, V5State::CurrentRestored)
                | (V5State::CurrentRestored, V5State::CurrentBootstrapped)
                | (V5State::CurrentBootstrapped, V5State::RolledBack)
        );
        let critical = next == V5State::CriticalFailure
            && previous >= V5State::RollbackStarted
            && previous < V5State::RolledBack;
        if forward || rollback_entry || rollback || critical {
            Ok(())
        } else {
            Err(ControllerError(format!(
                "invalid paired-v5 journal transition: {} -> {}",
                previous.token(),
                next.token()
            )))
        }
    ...
    const ALL_V5_STATES: [V5State; 17] = [
        V5State::Begun,
        V5State::SourceExported,
        V5State::BuildVerified,
        V5State::StopInitiated,
    ...
    fn is_plausible_v5_torn_tail(tail: &[u8], previous: V5State) -> bool {
        ...
        ALL_V5_STATES
            .iter()
            .copied()
            .filter(|next| validate_v5_transition(previous, *next).is_ok())
            .any(|next| is_plausible_v5_record_prefix(tail, next))
    }

    fn is_plausible_v5_record_prefix(tail: &str, state: V5State) -> bool {
    ...
    fn paired_v5_self_test() -> Result<()> {
        verify_v5_cli_surface()?;
        let valid = format!(
        ...
        if parse_v5_journal(&valid)? != V5State::Committed {
            return Err(ControllerError(
                "paired-v5 committed journal parser self-test failed".to_owned(),
            ));
        }
        ...
        if parse_v5_journal(&rolled_back)? != V5State::RolledBack {
            return Err(ControllerError(
                "paired-v5 rollback journal parser self-test failed".to_owned(),
            ));
        }
        if parse_v5_journal(&format!(
            "{V5_JOURNAL_HEADER}\nSTATE BEGUN\nSTATE NEW_PUBLISHED\n"
        ))
        .is_ok()
        {
            return Err(ControllerError(
                "paired-v5 journal parser accepted a skipped transition".to_owned(),
            ));
        }
        let hold_root = Path::new("/Applications/.opensteamer-paired-v5-install-selftest");
        let hold_app = hold_root.join("opensteamer Host.app");
        require_v5_install_hold_layout(hold_root, &hold_app)?;
        if require_v5_install_hold_layout(
            Path::new("/Applications/opensteamer Host.app"),
            &hold_app,
        )
        .is_ok()
            || require_v5_install_hold_layout(hold_root, &hold_root.join("unreviewed Host.app"))
                .is_ok()
        {
            return Err(ControllerError(
                "paired-v5 install-hold layout self-test failed".to_owned(),
            ));
        }
        paired_v5_dynamic_self_test()?;
        println!("SELF_TEST_OK paired-v5-host-update-controller");
        Ok(())
    }

    fn verify_v5_cli_surface() -> Result<()> {
        let executable = "controller".to_owned();
        let repo = V5_EXPECTED_REPO.to_owned();
        let authorized_commit = "a".repeat(40);
        let authorized_tree = "b".repeat(40);
        let allowed = [
            vec![
                executable.clone(),
                V5_PREFLIGHT_MODE.to_owned(),
                repo.clone(),
            ],
            vec![
                executable.clone(),
                V5_EXECUTE_MODE.to_owned(),
                repo.clone(),
            ...
            vec![
                executable.clone(),
                V5_ROLLBACK_MODE.to_owned(),
                repo.clone(),
            ],
            vec![executable.clone(), V5_SELF_TEST_MODE.to_owned()],
            vec![
                executable.clone(),
        ...
        if !matches!(parse_v5_command(&allowed[0]), Ok(V5Command::Preflight(_)))
            || !matches!(parse_v5_command(&allowed[1]), Ok(V5Command::Execute { .. }))
            || !matches!(parse_v5_command(&allowed[2]), Ok(V5Command::Rollback(_)))
            || !matches!(parse_v5_command(&allowed[3]), Ok(V5Command::SelfTest))
            || !matches!(
                parse_v5_command(&allowed[4]),
                Ok(V5Command::ProbeLock { .. })
            )
        {
            return Err(ControllerError(
                "paired-v5 CLI rejected a reviewed command shape".to_owned(),
            ));
        }

        let malformed = [
            vec![executable.clone(), V5_PREFLIGHT_MODE.to_owned()],
            vec![executable.clone(), V5_EXECUTE_MODE.to_owned(), repo.clone()],
            vec![
                executable.clone(),
                V5_EXECUTE_MODE.to_owned(),
                repo.clone(),
        ...
        if malformed
            .iter()
            .any(|arguments| parse_v5_command(arguments).is_ok())
        {
            return Err(ControllerError(
                "paired-v5 CLI accepted an unreviewed command shape".to_owned(),
            ));
        }
        ...
        {
            return Err(ControllerError(
                "paired-v5 authorization binding accepted a mismatched commit or tree".to_owned(),
            ));
        }
        ...
        for encoded_mode in [
            ...
            let mode = String::from_utf8(decode_marker_hex(encoded_mode)?).map_err(|_| {
                ControllerError("paired-v5 CLI test mode is not UTF-8".to_owned())
            })?;
            let arguments = vec![executable.clone(), mode, repo.clone()];
            if parse_v5_command(&arguments).is_ok() {
                return Err(ControllerError(
                    "paired-v5 CLI exposed an inherited update mode".to_owned(),
                ));
            }
        }
        Ok(())
    ...
    fn paired_v5_dynamic_self_test() -> Result<()> {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map_err(|_| ControllerError("system clock predates Unix epoch".to_owned()))?
            .as_nanos();
        let directory = PathBuf::from(format!(
            "/private/tmp/opensteamer-paired-v5-selftest-{}-{unique}",
            std::process::id()
        ));
        require_path_absent(&directory, "paired-v5 self-test directory")?;
        fs::create_dir(&directory)?;
        fs::set_permissions(&directory, fs::Permissions::from_mode(0o700))?;
        let result = paired_v5_dynamic_self_test_in(&directory);
        let cleanup = fs::remove_dir_all(&directory);
        match (result, cleanup) {
            (Err(error), _) => Err(error),
            (Ok(()), Err(error)) => Err(ControllerError(format!(
                "cannot remove paired-v5 self-test directory: {error}"
            ))),
            (Ok(()), Ok(())) => Ok(()),
        }
    }

    fn paired_v5_dynamic_self_test_in(directory: &Path) -> Result<()> {
        let recoverable = directory.join("recoverable.log");
        let mut journal = V5Journal::create(&recoverable)?;
        journal.record(
            V5State::SourceExported,
            &[
                ("commit", "a".repeat(40)),
        ...
        {
            return Err(ControllerError(
                "paired-v5 journal validation failure changed durable bytes".to_owned(),
            ));
        }
        drop(journal);
        ...
        partial.sync_all()?;
        drop(partial);
        let mut reopened = V5Journal::open(&recoverable)?;
        if reopened.state != V5State::SourceExported {
            return Err(ControllerError(
                "paired-v5 journal recovery accepted an incomplete final record".to_owned(),
            ));
        }
        reopened.record(
            V5State::BuildVerified,
        ...
        let mut corrupt_journal = V5Journal::create(&corrupt)?;
        ...
        drop(corrupt_append);
        if V5Journal::open(&corrupt).is_ok() {
            return Err(ControllerError(
                "paired-v5 journal recovery accepted a malformed complete record".to_owned(),
            ));
        }
        ...
        if acquire_update_transaction_lock_at(&transaction_lock).is_ok() {
            return Err(ControllerError(
                "paired-v5 transaction lock allowed concurrent ownership".to_owned(),
            ));
        }
        drop(first);
        ...
        require_v5_update_root_unused_at(&failed_attempt_root)?;
        ...
        if require_v5_update_root_unused_at(&failed_attempt_root).is_ok() {
            return Err(ControllerError(
                "paired-v5 single-attempt gate accepted failed-before-stop evidence".to_owned(),
            ));
        }
        ...
        require_v5_update_root_unused_at(&rolled_back_root)?;
        ...
        if require_v5_update_root_unused_at(&rolled_back_root).is_ok() {
            return Err(ControllerError(
                "paired-v5 single-attempt gate accepted rolled-back evidence".to_owned(),
            ));
        }

        let pointer_fixture = directory.join("v4-pointer-fixture");
        let mut pointer = create_new_private(&pointer_fixture)?;
        pointer.write_all(COMMITTED_V4_EVIDENCE.as_bytes())?;
        pointer.write_all(b"\n")?;
        pointer.sync_all()?;
        drop(pointer);
        ...
        if verify_self_test_pinned_file(&pointer_fixture, &pointer_hash).is_ok() {
            return Err(ControllerError(
                "v4 pointer corruption was accepted".to_owned(),
            ));
        }

        let evidence_fixture = directory.join("v4-evidence-fixture");
        ...
        if verify_self_test_pinned_file(&evidence_fixture, &evidence_hash).is_ok() {
            return Err(ControllerError(
                "v4 evidence corruption was accepted".to_owned(),
            ));
        }

        self_test_v4_oracle_pin_mutation(directory)?;
    ...
    fn self_test_partial_install_hold_recovery(directory: &Path) -> Result<()> {
        ...
        archive_v5_install_hold_root_at(&install_hold_root, &archive)?;
        require_path_absent(&install_hold_root, "quarantined partial install-hold root")?;
        require_directory(&archive, 0o700)?;
        ...
        fs::create_dir(&install_hold_root)?;
        fs::set_permissions(&install_hold_root, fs::Permissions::from_mode(0o700))?;
        if archive_v5_install_hold_root_at(&install_hold_root, &archive).is_ok() {
            return Err(ControllerError(
    ...
    fn self_test_v4_oracle_pin_mutation(directory: &Path) -> Result<()> {
        for (name, mode) in [
            ...
            writeln!(oracle, "immutable-v4-oracle={name}")?;
            oracle.sync_all()?;
            drop(oracle);
            ...
            if verify_self_test_pinned_file_with_mode(&path, mode, &expected_sha256).is_ok() {
                return Err(ControllerError(format!(
                    "v4 rollback oracle corruption was accepted: {name}"
                )));
            }
        }
        Ok(())
    ...
    fn self_test_publication_boundary_case(
        ...
        const REPLACEMENT_BYTES: &str = "paired-v5-replacement";
        ...
        let pointer = case_root.join("pinned-v4-pointer-fixture");

        let mut pointer_file = create_new_private(&pointer)?;
        writeln!(pointer_file, "{COMMITTED_V4_EVIDENCE}")?;
        pointer_file.sync_all()?;
        drop(pointer_file);
        if sha256(&pointer)? != COMMITTED_V4_POINTER_SHA256 {
            return Err(ControllerError(
                "publication matrix v4 pointer does not match the committed bytes".to_owned(),
            ));
        }
        verify_self_test_pinned_file(&pointer, COMMITTED_V4_POINTER_SHA256)?;
        ...
        if case.current_held {
            self_test_checked_rename(&canonical, &rollback, &pointer, COMMITTED_V4_POINTER_SHA256)?;
        }
        if case.new_published {
            self_test_checked_rename(&pending, &canonical, &pointer, COMMITTED_V4_POINTER_SHA256)?;
        }
        ...
        if path_exists_without_follow(&rollback)? {
            ...
            self_test_checked_rename(&rollback, &canonical, &pointer, COMMITTED_V4_POINTER_SHA256)?;
        }
