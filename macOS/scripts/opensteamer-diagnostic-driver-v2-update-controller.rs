...
const LOCK_PROBE_MODE: &str = "--probe-diagnostic-driver-v2-host-lock";
const LOCK_FREE_PROBE_MODE: &str = "--probe-diagnostic-driver-v2-host-lock-free";
const LOCK_RECORD_PROBE_MODE: &str = "--read-diagnostic-driver-v2-host-lock-record";
...
fn require_retained_v1_descriptor(
    file: &File,
    is_directory: bool,
    inode: u64,
    links: u64,
    ...
    {
        return Err(ControllerError(format!(
            "retained v1 descriptor identity changed: {label}"
        )));
    }
...
fn require_retained_v1_support_descriptor(file: &File) -> Result<fs::Metadata> {
    let metadata = file.metadata()?;
    if !metadata.file_type().is_dir()
        || metadata.file_type().is_symlink()
        || metadata.uid() != USER_ID
    ...
    {
        return Err(ControllerError(
            "retained v1 support descriptor identity changed".to_owned(),
        ));
    }
...
fn require_retained_v1_descriptor_children(
    directory: &File,
    expected: &[&[u8]],
    label: &str,
) -> Result<()> {
...
fn read_retained_v1_descriptor(file: &File, maximum: u64, label: &str) -> Result<Vec<u8>> {
    let before = file.metadata()?;
...
fn retained_v1_descriptor_graph_identities(
    ...
    let support = require_retained_v1_support_descriptor(&graph.support)?;
    let update_root = require_retained_v1_descriptor(
        &graph.update_root,
        true,
    ...
    let evidence = require_retained_v1_descriptor(
        &graph.evidence,
        true,
    ...
    let probes = require_retained_v1_descriptor(
        &graph.probes,
        true,
    ...
    let pointer = require_retained_v1_descriptor(
        &graph.pointer,
        false,
    ...
    let journal = require_retained_v1_descriptor(
        &graph.journal,
        false,
    ...
    let request = require_retained_v1_descriptor(
        &graph.request,
        false,
    ...
    let reader = require_retained_v1_descriptor(
        &graph.reader,
        false,
    ...
    let named_lock = require_retained_v1_descriptor(
        &graph.named_lock,
        false,
    ...
    let held_lock_metadata = require_retained_v1_descriptor(
        held_lock,
        false,
        RETAINED_V1_USER_UPDATE_LOCK_INODE,
        1,
...
fn verify_retained_v1_descriptor_graph_payload(graph: &RetainedV1DescriptorGraph) -> Result<()> {
    ...
    require_retained_v1_descriptor_children(&graph.probes, &[], "retained v1 probes")?;

    let pointer_bytes = read_retained_v1_descriptor(&graph.pointer, 256, "retained v1 pointer")?;
    let journal_bytes = read_retained_v1_descriptor(&graph.journal, 512, "retained v1 journal")?;
    let request_bytes = read_retained_v1_descriptor(&graph.request, 1_024, "retained v1 request")?;
    let reader_bytes =
        read_retained_v1_descriptor(&graph.reader, DIAGNOSTIC_READER_SIZE, "retained v1 reader")?;
...
) -> Result<(String, String, HostGeneration, RetainedV1DescriptorGraph)> {
    if unsafe { getuid() } != USER_ID || unsafe { geteuid() } != USER_ID {
        return Err(ControllerError(
...
fn preflight(repo: &Path) -> Result<()> {
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V2_PREFLIGHT_OK host_pid={} host_runs={} coreaudiod_pid={} coreaudiod_runs={} installed_driver={}:{} candidate_commit={} candidate_tree={} release_commit={} release_tree={} pairing=metadata-only legacy=protected reader=passive both_order=no-default-mutation namespaces=fresh",
        host.pid,
        host.runs,
        coreaudio.pid,
        coreaudio.runs,
...
fn validate_journal_fields(
    ...
    for (key, value) in fields {
        match key.as_str() {
            ...
            | "retained_v1_locator_sha256"
            | "retained_v1_request_sha256" => {
                require_lower_hex(value, 64, "retained v1 journal attestation")?
            }
...
fn create_user_layout(nonce: &str) -> Result<UserLayout> {
    ...
    let evidence = Path::new(USER_UPDATE_ROOT).join(format!(
        "diagnostic-driver-v2-{seconds}-{}-{nonce}",
        std::process::id()
    ));
...
fn parse_root_request_text(text: &str) -> Result<RootRequest> {
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V2") {
    ...
    if request.reader_sha256 != DIAGNOSTIC_READER_SHA256
        ...
        || !request
            ...
            .and_then(OsStr::to_str)
            .is_some_and(|leaf| {
                leaf.starts_with("diagnostic-driver-v2-") && leaf.ends_with(&request.nonce)
            })
...
fn parse_root_state_text(
    ...
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_STATE_V2") || !text.ends_with('\n')
    {
        return Err(ControllerError(
            ...
            .is_some_and(|evidence| {
                evidence.parent() == Some(Path::new(USER_UPDATE_ROOT))
                    && evidence
                        .file_name()
                        .and_then(OsStr::to_str)
                        .is_some_and(|leaf| leaf.starts_with("diagnostic-driver-v2-"))
...
fn run_passive_driver_validation(
    ...
    let nonce = format!("diagnostic-driver-v2-{}", request.nonce);
...
fn write_root_result(layout: &RootLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RESULT_V2\nstatus={status}\ndetail={detail}\n");
...
fn write_root_recovery_result(layout: &RootLayout, outcome: &str, detail: &str) -> Result<()> {
    ...
    match fs::symlink_metadata(&layout.recovery_result) {
        ...
        Ok(_) => {
            ...
            if !existing.starts_with("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V2\n")
                || !existing.ends_with('\n')
...
fn perform_root_transaction(request_path: &Path) -> Result<HostGeneration> {
    ...
    let reserve_lease = match allocate_rollback_reserve(&layout) {
        Ok(lease) => lease,
        Err(error) => {
            let finalization = match parse_optional_root_state(&layout) {
                Ok(durable) => {
                    finalize_prestop_preserving_host(&layout, &mut journal, durable).map(|_| ())
                }
...
fn root_authorized_update(request_path: &Path) -> Result<()> {
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V2_UPDATE_COMMITTED host_pid={} host_runs={} pairing=preserved routes=unchanged legacy=protected",
        host.pid, host.runs
    );
    Ok(())
}
...
fn write_user_result(layout: &UserLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_RESULT_V2\nstatus={status}\ndetail={detail}\n");
...
fn execute_authorized_update(
    ...
    {
        return Err(ControllerError(
            "retained v1 descriptor guard changed immediately before root dispatch".to_owned(),
        ));
    }
    ...
    if stdout.lines().count() != 1 || !stdout.starts_with("DIAGNOSTIC_DRIVER_V2_UPDATE_COMMITTED ")
    {
        return Err(ControllerError(
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V2_UPDATE_COMPLETE evidence={} host_pid={} pairing=preserved routes=unchanged legacy=protected",
...
fn rollback_authorized_update(repo: &Path) -> Result<()> {
    ...
    let recovery_kind = if line.starts_with("DIAGNOSTIC_DRIVER_V2_ROOT_ROLLBACK_COMPLETE ") {
        "rolled-back"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V2_ROOT_PRESTOP_ABORTED ") {
        "prestop-aborted"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V2_ROOT_RECOVERY_COMMITTED ") {
        "committed"
    } else {
        return Err(ControllerError(
    ...
    verify_pairing_metadata_only()?;
    println!(
        "DIAGNOSTIC_DRIVER_V2_RECOVERY_COMPLETE outcome={recovery_kind} host_pid={} pairing=preserved routes={route_status} legacy=protected",
        host.pid
    );
    Ok(())
}
...
fn complete_root_recovery(_request: RootRequest, layout: RootLayout) -> Result<()> {
    ...
    let (outcome, marker) = match plan {
        ...
        RootRecoveryPlan::ResumeRollback => {
            ...
            (outcome, "DIAGNOSTIC_DRIVER_V2_ROOT_ROLLBACK_COMPLETE")
        }
    ...
    let recovery_outcome = match marker {
        "DIAGNOSTIC_DRIVER_V2_ROOT_RECOVERY_COMMITTED" => "committed",
        "DIAGNOSTIC_DRIVER_V2_ROOT_PRESTOP_ABORTED" => "prestop-aborted",
        "DIAGNOSTIC_DRIVER_V2_ROOT_ROLLBACK_COMPLETE" => "rolled-back",
        _ => {
            return Err(ControllerError(
...
fn write_or_verify_prestop_abort_witness(layout: &RootLayout, journal: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_PRESTOP_ABORT_JOURNAL_V2\noutcome=prestop-aborted\njournal={journal}\n"
    );
    match fs::symlink_metadata(&path) {
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
...
fn finalize_sealed_bootstrap_without_root_pointer(fixed_digest: &str) -> Result<()> {
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V2_ROOT_PRESTOP_ABORTED host_pid={} routes=unchanged pairing=preserved",
        host.pid
    );
    Ok(())
}
...
fn self_test() -> Result<()> {
    ...
    let root_hash_path = Path::new(
        "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v2/controller-0123456789abcdef0123456789abcdef/controller",
    );
    ...
    let synthetic = RootRequest {
        ...
        evidence: PathBuf::from(
            "/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v2/diagnostic-driver-v2-1-1-0123456789abcdef0123456789abcdef",
        ),
    ...
    for (fixture, label) in [
        ...
        (
            request_text.replace(
                ...
                "evidence=/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v2/../escape\n",
            ),
        ...
        (
            request_text.replace(
                ...
                "root_controller=/Library/Application Support/opensteamer/diagnostic-driver-controllers-v2/controller-0123456789abcdef0123456789abcdef/../controller\n",
            ),
    ...
    let pointer_bytes = b"/Library/Application Support/opensteamer/diagnostic-driver-updates-v2/transaction-0123456789abcdef0123456789abcdef\n";
    ...
    if !uid501_source_path_is_allowed(Path::new(BOTH_ORDER_PROBE), false)
        ...
        || uid501_source_path_is_allowed(
            Path::new("/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v2/../escape/probes/both-order.json"),
            true,
        )
    {
        return Err(ControllerError(
    ...
    let source = include_str!("opensteamer-diagnostic-driver-v2-update-controller.rs");
    let forbidden = [
    ...
    println!("DIAGNOSTIC_DRIVER_V2_SELF_TEST_OK tests=106");
    Ok(())
}
