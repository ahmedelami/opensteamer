...
//! This controller owns a disjoint v6 journal/pointer namespace, never starts an interactive
//! host, never resets pairing, and rolls back to the exact committed pairing-preserving v5 host.

#[allow(dead_code)]
mod paired_v6 {
    include!(env!("OPENSTEAMER_PAIRED_V6_INCLUDED_SOURCE"));

    const V6_PREFLIGHT_MODE: &str = "--verify-paired-v6-host-update-preflight";
    const V6_EXECUTE_MODE: &str = "--execute-authorized-paired-v6-host-update";
    const V6_ROLLBACK_MODE: &str = "--rollback-authorized-paired-v6-host-update";
    const V6_SELF_TEST_MODE: &str = "--self-test-paired-v6-host-update";
    const V6_EXPECTED_REPO: &str = "/Users/ahmed/Documents/Codex/opensteamer";

    const V6_UPDATE_ROOT: &str =
        "/Users/ahmed/Library/Application Support/opensteamer/paired-host-updates-v6";
    const V6_ACTIVE_UPDATE: &str =
        "/Users/ahmed/Library/Application Support/opensteamer/active-paired-host-update-v6";
    const V6_UPDATE_LOCK: &str = UPDATE_LOCK;
    const V6_JOURNAL_HEADER: &str = "OPENSTEAMER_PAIRED_HOST_UPDATE_V6";
    const HIDDEN_INSTALL_PREFIX: &str = ".opensteamer-paired-v6-install-";
    ...
    const CURRENT_BASELINE_APP: &str = "/Users/ahmed/Library/Application Support/opensteamer/paired-host-updates-v5/paired-v5-update-1786316959-19979-3f7de8a9-473f-4abf-b15d-9790c827765e/deployment-reference/opensteamer Host.app";
    const CURRENT_BASELINE_SOURCE_EXPORT: &str = "/Users/ahmed/Library/Application Support/opensteamer/paired-host-updates-v5/paired-v5-update-1786316959-19979-3f7de8a9-473f-4abf-b15d-9790c827765e/source-export";
    const CURRENT_BASELINE_EXECUTABLE_SHA256: &str =
        "2cb98599725f1a8c658b9a8afc38b50fabe252168292e27505af88cbecf2d205";
    const CURRENT_BASELINE_CDHASH: &str = "92ad981f78d75d63d7a857c677bc73fdfc004da6";
    ...
    const REQUIRED_HIDDEN_WRITER_PATCH_COMMIT: &str = "c9502b38ccd278eb03aa913851a643b08c44d5c6";
    ...
    impl V6Journal {
        fn create(path: &Path) -> Result<Self> {
            let mut file = OpenOptions::new()
                ...
                .open(path)
                .map_err(|error| ControllerError(format!("cannot create v6 journal: {error}")))?;
            validate_open_journal_file(path, &file)?;
            writeln!(file, "{V6_JOURNAL_HEADER}")?;
            file.sync_all()?;
            fsync_parent(path)?;
        ...
        fn open(path: &Path) -> Result<Self> {
            ...
            let complete_text = std::str::from_utf8(&bytes[..complete_length])
                .map_err(|_| ControllerError("v6 journal is not UTF-8".to_owned()))?;
            let state = parse_v6_journal(complete_text)?;
            if complete_length != bytes.len() {
                if !is_plausible_v6_torn_tail(&bytes[complete_length..], state) {
                    return Err(ControllerError(
                        "v6 journal has an implausible incomplete final record".to_owned(),
                    ));
                }
                validate_open_journal_file(path, &file)?;
        ...
        fn record(&mut self, state: V6State, fields: &[(&str, String)]) -> Result<()> {
            self.require_healthy()?;
            validate_v6_transition(self.state, state)?;
            validate_v6_fields(state, fields)?;
                ...
                .and_then(|_| self.file.sync_all())
            {
                ...
                return match recovery {
                    Ok(()) => Err(ControllerError(format!(
                        "cannot durably append v6 journal record: {error}"
                    ))),
                    Err(recovery_error) => Err(ControllerError(format!(
                        "cannot durably append v6 journal record ({error}) or restore prior length ({recovery_error})"
                    ))),
                };
            }
        ...
        fn require_healthy(&mut self) -> Result<()> {
            if !self.healthy {
                return Err(ControllerError(
                    "v6 journal is poisoned after an unrecovered append failure".to_owned(),
                ));
            }
        ...
        fn restore_after_failed_append(&mut self, prior_length: u64) -> Result<()> {
            ...
            if bytes.len() as u64 != prior_length || !bytes.ends_with(b"\n") {
                return Err(ControllerError(
                    "v6 journal append recovery did not restore prior durable length".to_owned(),
                ));
            }
            let text = std::str::from_utf8(&bytes)
                .map_err(|_| ControllerError("recovered v6 journal is not UTF-8".to_owned()))?;
            if parse_v6_journal(text)? != self.state {
                return Err(ControllerError(
                    "v6 journal append recovery did not restore prior state".to_owned(),
                ));
            }
    ...
    impl V6Layout {
        fn new(repo: PathBuf, evidence: PathBuf, nonce: &str) -> Self {
            ...
            let install_hold_root = PathBuf::from(format!(
                "/Applications/.opensteamer-paired-v6-install-{nonce}"
            ));
            Self {
                repo,
    ...
    pub fn entry() {
        if let Err(error) = paired_v6_real_main() {
            eprintln!("opensteamer paired-v6 update controller: {error}");
            std::process::exit(1);
        }
    }

    fn paired_v6_real_main() -> Result<()> {
        let arguments: Vec<String> = env::args().collect();
        verify_optimized_binary_scrub()?;
        match parse_v6_command(&arguments)? {
            V6Command::Preflight(repo) => {
                let repo = canonical_repo(&repo)?;
                verify_machine_contract()?;
                let _transaction_lock = acquire_update_transaction_lock()?;
                verify_committed_v5_baseline()?;
                let generation = verify_paired_v6_runtime()?;
                let provenance = verify_paired_v6_git_provenance(&repo, true)?;
                verify_isolated_pairing_items_present()?;
                require_path_absent(Path::new(V6_ACTIVE_UPDATE), "active paired-v6 pointer")?;
                require_v6_update_root_unused()?;
                println!(
                    "PAIRED_V6_UPDATE_PREFLIGHT_OK pid={} runs={} baseline=sole-ready pairing=preserved v1=immutable v2=immutable v3=immutable v4=immutable v5=immutable v6=absent commit={} tree={}",
            ...
            V6Command::Execute {
                ...
                authorized_commit,
                authorized_tree,
            } => execute_paired_v6_update(
                canonical_repo(&repo)?,
                &authorized_commit,
                &authorized_tree,
            ),
            V6Command::Rollback(repo) => rollback_existing_paired_v6_update(canonical_repo(&repo)?),
            V6Command::SelfTest => paired_v6_self_test(),
            V6Command::ProbeLock { runtime, lock, pid } => {
    ...
    fn parse_v6_command(arguments: &[String]) -> Result<V6Command> {
        match arguments {
            [_, mode, repo] if mode == V6_PREFLIGHT_MODE => {
                Ok(V6Command::Preflight(repo.clone()))
            }
            [_, mode, repo, authorized_commit, authorized_tree] if mode == V6_EXECUTE_MODE => {
                ...
                Ok(V6Command::Execute {
                    repo: repo.clone(),
                    authorized_commit: authorized_commit.clone(),
                    authorized_tree: authorized_tree.clone(),
                })
            }
            [_, mode, repo] if mode == V6_ROLLBACK_MODE => Ok(V6Command::Rollback(repo.clone())),
            [_, mode] if mode == V6_SELF_TEST_MODE => Ok(V6Command::SelfTest),
            [_, mode, runtime, lock, pid] if mode == PROBE_LOCK_MODE => {
                Ok(V6Command::ProbeLock {
            ...
            _ => Err(ControllerError(format!(
                "usage: {} {V6_PREFLIGHT_MODE} <canonical-repo>\n       {} {V6_EXECUTE_MODE} <canonical-repo> <authorized-commit> <authorized-tree>\n       {} {V6_ROLLBACK_MODE} <canonical-repo>\n       {} {V6_SELF_TEST_MODE}\n       {} {PROBE_LOCK_MODE} <runtime-dir> <lock-file> <pid>",
    ...
    fn verify_optimized_binary_scrub() -> Result<()> {
        ...
        let executable = env::current_exe().map_err(|error| {
            ControllerError(format!("cannot resolve the paired-v6 controller: {error}"))
        })?;
        let mut file = File::open(&executable).map_err(|error| {
            ControllerError(format!("cannot inspect the paired-v6 controller: {error}"))
        })?;
        ...
        {
            return Err(ControllerError(
                "paired-v6 controller binary has unsafe metadata".to_owned(),
            ));
        }
        ...
        {
            return Err(ControllerError(
                "paired-v6 controller changed while being scrub-verified".to_owned(),
            ));
        }
        for (index, encoded) in FORBIDDEN_MARKER_HEX.iter().enumerate() {
            ...
            {
                return Err(ControllerError(format!(
                    "optimized paired-v6 controller retained forbidden legacy marker {index}"
                )));
            }
        }
        Ok(())
    ...
    fn decode_marker_hex(encoded: &str) -> Result<Vec<u8>> {
        if encoded.len() % 2 != 0 {
            return Err(ControllerError(
                "paired-v6 binary scrub marker has odd length".to_owned(),
            ));
        }
        encoded
            .as_bytes()
    ...
    fn decode_hex_nibble(value: u8) -> Result<u8> {
        match value {
            ...
            _ => Err(ControllerError(
                "paired-v6 binary scrub marker is not lowercase hexadecimal".to_owned(),
            )),
        }
    }

    fn require_v6_update_root_unused() -> Result<()> {
        require_v6_update_root_unused_at(Path::new(V6_UPDATE_ROOT))
    }

    fn require_v6_update_root_unused_at(root: &Path) -> Result<()> {
        match fs::symlink_metadata(root) {
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(()),
            Err(error) => {
                return Err(ControllerError(format!(
                    "cannot inspect paired-v6 update root {}: {error}",
                    root.display()
                )))
            }
        ...
        let mut entries = fs::read_dir(root).map_err(|error| {
            ControllerError(format!(
                "cannot enumerate paired-v6 update root {}: {error}",
                root.display()
            ))
        })?;
        if entries.next().transpose()?.is_some() {
            return Err(ControllerError(
                "paired-v6 update root already contains retained attempt evidence; a second attempt is not authorized"
                    .to_owned(),
            ));
        }
        Ok(())
    ...
    fn prepare_v6_update_root_for_first_attempt() -> Result<()> {
        let root = Path::new(V6_UPDATE_ROOT);
        match fs::symlink_metadata(root) {
        ...
        require_v6_update_root_unused_at(root)
    }

    fn execute_paired_v6_update(
        repo: PathBuf,
        ...
        require_canonical_git_oid(authorized_tree, "authorized tree")?;
        verify_machine_contract()?;
        let transaction_lock = acquire_update_transaction_lock_at(Path::new(V6_UPDATE_LOCK))?;
        verify_committed_v5_baseline()?;
        let initial_generation = verify_paired_v6_runtime()?;
        verify_isolated_pairing_items_present()?;
        let provenance = verify_paired_v6_git_provenance(&repo, true)?;
        ...
        require_available_bytes(
            ...
            2 * 1_024 * 1_024 * 1_024,
            "before creating paired-v6 update evidence",
        )?;

        let nonce = new_nonce()?;
        let evidence = PathBuf::from(V6_UPDATE_ROOT).join(format!(
            "paired-v6-update-{}-{}-{}",
            unix_seconds()?,
            std::process::id(),
            nonce
        ...
        let layout = V6Layout::new(repo, evidence, &nonce);
        create_private_directory(&layout.rollback_dir)?;
        create_private_directory(&layout.failed_dir)?;
        let mut journal = V6Journal::create(&layout.journal)?;
        record_v6_install_hold_name(&layout)?;

        let result = perform_paired_v6_update(
            &layout,
            &mut journal,
            &provenance,
            &initial_generation,
        ...
        match result {
            Ok(()) => Ok(()),
            Err(primary) => {
                if journal.state == V6State::Committed {
                    let _ = write_result(
                        &layout.result,
                    ...
                    eprintln!(
                        "warning: paired-v6 update committed but final reporting failed: {primary}"
                    );
                    return Ok(());
                }
                let pointer_absent_before_stop = journal.state == V6State::StopInitiated
                    && matches!(
                        fs::symlink_metadata(V6_ACTIVE_UPDATE),
                        Err(error) if error.kind() == std::io::ErrorKind::NotFound
                    );
                let crossed_stop = journal.state >= V6State::StopInitiated
                    && journal.state < V6State::Committed
                    && !pointer_absent_before_stop;
                if !crossed_stop {
                    if layout.rollback_reserve.exists() {
                        let _ = release_rollback_reserve(&layout.rollback_reserve);
                    }
                    let _ = archive_v6_install_hold_root(&layout);
                    let _ = write_result(
                        &layout.result,
                        "failed-before-stop",
                        Some(&primary.to_string()),
                ...
                verify_v6_active_pointer(&layout.evidence)?;
                match rollback_to_current_baseline(&layout, &mut journal, &transaction_lock) {
                    Ok(()) => {
                        ...
                        retire_v6_active_pointer(&layout)?;
                    ...
                    Err(rollback) => {
                        ...
                        Err(ControllerError(format!(
                            "CRITICAL: paired-v6 update and rollback both failed; keep host offline; evidence={}: primary={primary}; rollback={rollback}",
                            layout.evidence.display()
                        )))
                    }
                }
    ...
    fn perform_paired_v6_update(
        layout: &V6Layout,
        journal: &mut V6Journal,
        provenance: &Provenance,
        initial_generation: &LaunchGeneration,
        authorized_commit: &str,
        authorized_tree: &str,
    ) -> Result<()> {
        export_v6_source(layout, provenance)?;
        journal.record(
            V6State::SourceExported,
            &[
        ...
        build_and_verify_v6_staged_app(layout)?;
        prepare_v6_deployment_reference(layout)?;
        journal.record(
            V6State::BuildVerified,
            &[(
                "executable_sha256",
        ...
        verify_v6_deployment_reference(layout)?;
        verify_isolated_pairing_items_present()?;
        verify_v5_pointer_unchanged()?;
        let revalidated = verify_paired_v6_runtime()?;
        ...
        let boundary_provenance = verify_paired_v6_git_provenance(&layout.repo, true)?;
        ...
        publish_v6_active_pointer(&layout.evidence)?;
        prepare_v6_install_hold(layout)?;
        journal.record(V6State::InstallHoldVerified, &[])?;

        verify_v6_active_pointer(&layout.evidence)?;
        verify_v6_deployment_reference(layout)?;
        verify_isolated_pairing_items_present()?;
        verify_v5_pointer_unchanged()?;
        let final_generation = verify_paired_v6_runtime()?;
        ...
        journal.record(V6State::CurrentStopped, &[])?;
        ...
        journal.record(V6State::CurrentHeld, &[])?;
        ...
        fs::remove_dir(&layout.install_hold_root).map_err(|error| {
            ControllerError(format!(
                "cannot remove empty paired-v6 install-hold root {}: {error}",
                layout.install_hold_root.display()
            ))
        })?;
        fsync_parent(&layout.install_hold_root)?;
        verify_v6_installed_matches_reference(layout)?;
        verify_isolated_pairing_items_present()?;
        verify_v5_pointer_unchanged()?;
        journal.record(V6State::NewPublished, &[])?;
        drop(lock);

        let checkpoint = capture_log_checkpoint()?;
        bootstrap_exact_new_job()?;
        journal.record(V6State::PersistentBootstrapped, &[])?;
        let generation = wait_for_paired_v6_launch_generation(Duration::from_secs(45))?;
        verify_paired_v6_deployment(layout, &checkpoint, &generation)?;
        verify_isolated_pairing_items_present()?;
        verify_v5_pointer_unchanged()?;
        journal.record(
        ...
        journal.record(V6State::Committed, &[])?;
        if let Err(error) = write_result(&layout.result, "success", None) {
            eprintln!("warning: paired-v6 update committed but result recording failed: {error}");
        }
        println!(
            "PAIRED_V6_HOST_UPDATE_COMMITTED evidence={} pid={} rollback=current-isolated-retained pairing=preserved",
            layout.evidence.display(),
            generation.pid
        );
        Ok(())
    ...
    fn rollback_existing_paired_v6_update(repo: PathBuf) -> Result<()> {
        verify_machine_contract()?;
        let transaction_lock = acquire_update_transaction_lock_at(Path::new(V6_UPDATE_LOCK))?;
        verify_committed_v5_baseline()?;
        let evidence =
            read_update_pointer_at(Path::new(V6_ACTIVE_UPDATE), Path::new(V6_UPDATE_ROOT))?;
        let layout = v6_layout_from_existing(repo, evidence)?;
        let mut journal = V6Journal::open(&layout.journal)?;
        if journal.state == V6State::RolledBack {
            verify_paired_v6_runtime()?;
            verify_isolated_pairing_items_present()?;
            ensure_rolled_back_result(&layout.result)?;
            retire_v6_active_pointer(&layout)?;
            println!("PAIRED_V6_HOST_UPDATE_ALREADY_ROLLED_BACK");
            return Ok(());
        }
        ...
        retire_v6_active_pointer(&layout)?;
        println!(
            "PAIRED_V6_HOST_UPDATE_ROLLED_BACK evidence={} pairing=preserved",
            layout.evidence.display()
        );
        Ok(())
    }
    ...
    fn rollback_to_current_baseline(
        ...
        let already_rolling_back = matches!(
            journal.state,
            V6State::RollbackStarted
                | V6State::FailedNewArchived
                | V6State::CurrentRestored
                | V6State::CurrentBootstrapped
        );
        if !already_rolling_back {
            journal.record(V6State::RollbackStarted, &[])?;
        }
        if layout.rollback_reserve.exists() {
            release_rollback_reserve(&layout.rollback_reserve)?;
        }

        archive_v6_install_hold_root(layout)?;
        verify_v6_active_pointer(&layout.evidence)?;
        verify_v5_pointer_unchanged()?;
        verify_isolated_pairing_items_present()?;
        bootout_paired_v6_job_if_loaded(layout)?;
        wait_for_no_capture_servers(Duration::from_secs(30))?;
        require_service_absent(NEW_LAUNCH_AGENT_LABEL)?;
        verify_protected_legacy_absent()?;
        let lock = acquire_unowned_shared_lock()?;
        ...
        if rollback_exists {
            ...
            if canonical_exists {
                ...
                if journal.state == V6State::RollbackStarted {
                    journal.record(V6State::FailedNewArchived, &[])?;
                }
            } else if failed_exists && journal.state == V6State::RollbackStarted {
                journal.record(V6State::FailedNewArchived, &[])?;
            }
            ...
            if matches!(
                journal.state,
                V6State::RollbackStarted | V6State::FailedNewArchived
            ) {
                journal.record(V6State::CurrentRestored, &[])?;
            }
        ...
        if !matches!(
            journal.state,
            V6State::CurrentRestored | V6State::CurrentBootstrapped
        ) {
            return Err(ControllerError(format!(
                "paired-v6 rollback topology is not resumable from {}",
                journal.state.token()
            )));
        }
        verify_isolated_pairing_items_present()?;
        ...
        let checkpoint = capture_log_checkpoint()?;
        bootstrap_exact_new_job()?;
        if journal.state == V6State::CurrentRestored {
            journal.record(V6State::CurrentBootstrapped, &[])?;
        }
        let generation = wait_for_paired_v6_launch_generation(Duration::from_secs(45))?;
        ...
        verify_v5_pointer_unchanged()?;
        verify_protected_legacy_absent()?;
        journal.record(V6State::RolledBack, &[])?;
        verify_paired_v6_runtime()?;
        Ok(())
    }

    fn v6_layout_from_existing(repo: PathBuf, evidence: PathBuf) -> Result<V6Layout> {
        require_descendant(Path::new(V6_UPDATE_ROOT), &evidence)?;
        ...
        let nonce = install_hold_root
            ...
            .ok_or_else(|| {
                ControllerError("paired-v6 install-hold name is malformed".to_owned())
            })?;
        let expected = V6Layout::new(repo, evidence, nonce);
        if expected.install_hold_root != install_hold_root {
            return Err(ControllerError(
                "paired-v6 install-hold path escaped its recorded layout".to_owned(),
            ));
        }
        require_v6_install_hold_layout(&expected.install_hold_root, &expected.install_hold)?;
        Ok(expected)
    }

    fn export_v6_source(layout: &V6Layout, provenance: &Provenance) -> Result<()> {
        require_path_absent(&layout.source_tar, "v6 source archive")?;
        require_path_absent(&layout.source_export, "v6 source export")?;
        ...
        let status = Command::new("/usr/bin/git")
            ...
            .stderr(Stdio::piped())
            .status()?;
        require_success(status, "git archive for paired-v6 update")?;
        ...
        require_output_success(&output, "extract paired-v6 source archive")?;
        require_regular(
            &layout
                .source_export
        ...
        require_regular(
            &layout
                .source_export
                .join("macOS/scripts/opensteamer-host-paired-v6-update-controller.rs"),
            0o600,
        )?;
    ...
    fn build_and_verify_v6_staged_app(layout: &V6Layout) -> Result<()> {
        require_path_absent(&layout.stage_output, "paired-v6 staged output")?;
        require_path_absent(&layout.scratch, "paired-v6 SwiftPM scratch")?;
        ...
        let status = Command::new(&build_script)
            ...
            .status()?;
        require_success(status, "fresh signed paired-v6 host build")?;
        ...
        if sha256(&executable)? == CURRENT_BASELINE_EXECUTABLE_SHA256 {
            return Err(ControllerError(
                "paired-v6 staged executable is byte-identical to the current isolated baseline"
                    .to_owned(),
            ));
        }
    ...
    fn verify_staged_pairing_namespace(executable: &Path) -> Result<()> {
        ...
        require_output_success(&strings, "inspect paired-v6 staged pairing namespace")?;
        let text = decode_utf8(&strings.stdout, "paired-v6 strings output")?;
        ...
        if isolated_count != 1 || protected_count != 0 {
            return Err(ControllerError(format!(
                "paired-v6 staged pairing namespace is not isolated: isolated_count={isolated_count} protected_count={protected_count}"
            )));
        }
        Ok(())
    }

    fn prepare_v6_deployment_reference(layout: &V6Layout) -> Result<()> {
        require_path_absent(
            &layout.deployment_reference_dir,
            "paired-v6 deployment-reference directory",
        )?;
        ...
        require_output_success(&output, "copy paired-v6 deployment reference")?;
        verify_v6_deployment_reference(layout)
    }

    fn verify_v6_deployment_reference(layout: &V6Layout) -> Result<()> {
        ...
        for app in [&layout.staged_app, &layout.deployment_reference_app] {
            ...
            if sha256(&executable)? == CURRENT_BASELINE_EXECUTABLE_SHA256 {
                return Err(ControllerError(
                    "paired-v6 deployment reference equals current isolated baseline".to_owned(),
                ));
            }
            verify_staged_pairing_namespace(&executable)?;
        }
    ...
    fn prepare_v6_install_hold(layout: &V6Layout) -> Result<()> {
        require_v6_install_hold_layout(&layout.install_hold_root, &layout.install_hold)?;
        require_path_absent(&layout.install_hold_root, "paired-v6 hidden install hold")?;
        ...
        require_output_success(&output, "copy paired-v6 install hold")?;
        verify_bundle(
            &layout.source_export,
            &layout.install_hold,
            false,
    ...
    fn record_v6_install_hold_name(layout: &V6Layout) -> Result<()> {
        require_v6_install_hold_layout(&layout.install_hold_root, &layout.install_hold)?;
    ...
    fn archive_v6_install_hold_root(layout: &V6Layout) -> Result<()> {
        require_v6_install_hold_layout(&layout.install_hold_root, &layout.install_hold)?;
        ...
        archive_v6_install_hold_root_at(&layout.install_hold_root, &archive)
    }

    fn archive_v6_install_hold_root_at(install_hold_root: &Path, archive: &Path) -> Result<()> {
    ...
    fn verify_v6_installed_matches_reference(layout: &V6Layout) -> Result<()> {
        verify_bundle(
            &layout.source_export,
            Path::new(NEW_APP),
            true,
    ...
    fn verify_paired_v6_deployment(
        layout: &V6Layout,
    ...
    fn verify_paired_v6_runtime() -> Result<LaunchGeneration> {
        verify_committed_v5_baseline()?;
        verify_protected_legacy_absent()?;
        verify_reviewed_launch_agent_unchanged()?;
        verify_current_baseline_app_at(Path::new(NEW_APP), true)?;
    ...
    fn verify_paired_v6_git_provenance(repo: &Path, require_remote: bool) -> Result<Provenance> {
        let status = command_output(
            "/usr/bin/git",
            &["status", "--porcelain=v1", "--untracked-files=all"],
            Some(repo),
        )?;
        require_output_success(&status, "inspect paired-v6 git worktree")?;
        if !status.stdout.is_empty() {
            return Err(ControllerError(
                "repository must be completely clean before a paired-v6 host update".to_owned(),
            ));
        }
        ...
        require_output_success(&ancestry, "verify required hidden-writer patch ancestry")?;
        if require_remote {
            let output = command_output(
                "/usr/bin/git",
                &[
            ...
            require_output_success(&output, "verify pushed paired-v6 source commit")?;
            let text = decode_utf8(&output.stdout, "paired-v6 git ls-remote output")?;
            let records: Vec<&str> = text.lines().collect();
            let expected_ref = format!("refs/heads/{EXPECTED_SOURCE_BRANCH}");
            if records.len() != 1 {
                return Err(ControllerError(
                    "paired-v6 source branch is absent or ambiguous on origin".to_owned(),
                ));
            }
            ...
            {
                return Err(ControllerError(
                    "origin paired-v6 source branch does not resolve to local HEAD".to_owned(),
                ));
            }
        }
        Ok(Provenance {
    ...
    fn wait_for_paired_v6_launch_generation(timeout: Duration) -> Result<LaunchGeneration> {
        wait_for_launch_generation(timeout)
    }

    fn bootout_paired_v6_job_if_loaded(layout: &V6Layout) -> Result<()> {
        ...
        if state.status.success() {
            let loaded = parse_loaded_launch_job(decode_utf8(
                &state.stdout,
                "paired-v6 rollback launchctl state",
            )?)?;
            ...
            require_output_success(&output, "boot out paired-v6 LaunchAgent during rollback")?;
    ...
    fn verify_live_canonical_process(
        ...
        require_output_success(&output, "verify canonical paired-v6 host mapped code")
    }

    fn require_v6_install_hold_layout(root: &Path, app: &Path) -> Result<()> {
        ...
        {
            return Err(ControllerError(format!(
                "paired-v6 install hold escaped its reviewed layout: {}",
                app.display()
            )));
        }
        Ok(())
    ...
    fn publish_v6_active_pointer(evidence: &Path) -> Result<()> {
        require_descendant(Path::new(V6_UPDATE_ROOT), evidence)?;
        let pending = PathBuf::from(format!("{V6_ACTIVE_UPDATE}.pending-{}", std::process::id()));
        require_path_absent(&pending, "pending paired-v6 pointer")?;
        require_path_absent(Path::new(V6_ACTIVE_UPDATE), "active paired-v6 pointer")?;
        let mut file = create_new_private(&pending)?;
        writeln!(file, "{}", evidence.display())?;
        file.sync_all()?;
        rename_exclusive(&pending, Path::new(V6_ACTIVE_UPDATE))?;
        fsync_parent(Path::new(V6_ACTIVE_UPDATE))
    }

    fn verify_v6_active_pointer(expected_evidence: &Path) -> Result<()> {
        verify_update_pointer_at(
            Path::new(V6_ACTIVE_UPDATE),
            expected_evidence,
            Path::new(V6_UPDATE_ROOT),
        )
    }

    fn retire_v6_active_pointer(layout: &V6Layout) -> Result<()> {
    ...
    fn parse_v6_journal(text: &str) -> Result<V6State> {
        let mut lines = text.lines();
        if lines.next() != Some(V6_JOURNAL_HEADER) {
            return Err(ControllerError(
                "paired-v6 journal header is malformed".to_owned(),
            ));
        }
        let mut state = None;
        for line in lines {
            ...
            if fields.next() != Some("STATE") {
                return Err(ControllerError(
                    "paired-v6 journal record is malformed".to_owned(),
                ));
            }
            let next = fields
                .next()
                .and_then(V6State::parse)
                .ok_or_else(|| ControllerError("paired-v6 journal state is unknown".to_owned()))?;
            if let Some(previous) = state {
                validate_v6_transition(previous, next)?;
            } else if next != V6State::Begun {
                return Err(ControllerError(
                    "paired-v6 journal does not begin at BEGUN".to_owned(),
                ));
            }
            let fields: Vec<&str> = fields.collect();
            let expected = v6_field_schema(next);
            if fields.len() != expected.len() {
                return Err(ControllerError(
                    "paired-v6 journal field count is invalid".to_owned(),
                ));
            }
            for (field, expected_key) in fields.into_iter().zip(expected) {
                let (key, value) = field.split_once('=').ok_or_else(|| {
                    ControllerError("paired-v6 journal field is malformed".to_owned())
                })?;
                if key != *expected_key || !is_safe_journal_value(value) {
                    return Err(ControllerError(
                        "paired-v6 journal field is unsafe".to_owned(),
                    ));
                }
            }
            state = Some(next);
        }
        state.ok_or_else(|| ControllerError("paired-v6 journal has no state".to_owned()))
    }

    fn v6_field_schema(state: V6State) -> &'static [&'static str] {
        match state {
            V6State::Begun
            | V6State::InstallHoldVerified
            ...
            | V6State::CurrentBootstrapped
            | V6State::RolledBack => &[],
            V6State::SourceExported => &["commit", "tree", "initial_pid"],
            V6State::BuildVerified => &["executable_sha256"],
            V6State::StopInitiated => &["reserve_device", "reserve_inode", "reserve_bytes"],
            V6State::ReadyVerified => &["pid", "runs", "nonce"],
            V6State::CriticalFailure => &["phase"],
        }
    }

    fn validate_v6_fields(state: V6State, fields: &[(&str, String)]) -> Result<()> {
        let expected = v6_field_schema(state);
        if fields.len() != expected.len() {
            return Err(ControllerError(
                "paired-v6 journal record has wrong field count".to_owned(),
            ));
        }
        for ((key, value), expected_key) in fields.iter().zip(expected) {
            if *key != *expected_key || !is_safe_journal_value(value) {
                return Err(ControllerError("unsafe paired-v6 journal field".to_owned()));
            }
        }
        Ok(())
    }

    fn validate_v6_transition(previous: V6State, next: V6State) -> Result<()> {
        if previous == V6State::Begun && next == V6State::Begun {
            return Ok(());
        }
        let forward = matches!(
            (previous, next),
            (V6State::Begun, V6State::SourceExported)
                | (V6State::SourceExported, V6State::BuildVerified)
                | (V6State::BuildVerified, V6State::StopInitiated)
                | (V6State::StopInitiated, V6State::InstallHoldVerified)
                | (V6State::InstallHoldVerified, V6State::CurrentStopped)
                | (V6State::CurrentStopped, V6State::CurrentHeld)
                | (V6State::CurrentHeld, V6State::NewPublished)
                | (V6State::NewPublished, V6State::PersistentBootstrapped)
                | (V6State::PersistentBootstrapped, V6State::ReadyVerified)
                | (V6State::ReadyVerified, V6State::Committed)
        );
        let rollback_entry = next == V6State::RollbackStarted
            && ((previous >= V6State::StopInitiated && previous <= V6State::Committed)
                || previous == V6State::CriticalFailure)
            && previous != V6State::RollbackStarted;
        let rollback = matches!(
            (previous, next),
            (V6State::RollbackStarted, V6State::FailedNewArchived)
                | (V6State::RollbackStarted, V6State::CurrentRestored)
                | (V6State::FailedNewArchived, V6State::CurrentRestored)
                | (V6State::CurrentRestored, V6State::CurrentBootstrapped)
                | (V6State::CurrentBootstrapped, V6State::RolledBack)
        );
        let critical = next == V6State::CriticalFailure
            && previous >= V6State::RollbackStarted
            && previous < V6State::RolledBack;
        if forward || rollback_entry || rollback || critical {
            Ok(())
        } else {
            Err(ControllerError(format!(
                "invalid paired-v6 journal transition: {} -> {}",
                previous.token(),
                next.token()
            )))
        }
    ...
    const ALL_V6_STATES: [V6State; 17] = [
        V6State::Begun,
        V6State::SourceExported,
        V6State::BuildVerified,
        V6State::StopInitiated,
    ...
    fn is_plausible_v6_torn_tail(tail: &[u8], previous: V6State) -> bool {
        ...
        ALL_V6_STATES
            .iter()
            .copied()
            .filter(|next| validate_v6_transition(previous, *next).is_ok())
            .any(|next| is_plausible_v6_record_prefix(tail, next))
    }

    fn is_plausible_v6_record_prefix(tail: &str, state: V6State) -> bool {
    ...
    fn paired_v6_self_test() -> Result<()> {
        verify_v6_cli_surface()?;
        let valid = format!(
        ...
        if parse_v6_journal(&valid)? != V6State::Committed {
            return Err(ControllerError(
                "paired-v6 committed journal parser self-test failed".to_owned(),
            ));
        }
        ...
        if parse_v6_journal(&rolled_back)? != V6State::RolledBack {
            return Err(ControllerError(
                "paired-v6 rollback journal parser self-test failed".to_owned(),
            ));
        }
        if parse_v6_journal(&format!(
            "{V6_JOURNAL_HEADER}\nSTATE BEGUN\nSTATE NEW_PUBLISHED\n"
        ))
        .is_ok()
        {
            return Err(ControllerError(
                "paired-v6 journal parser accepted a skipped transition".to_owned(),
            ));
        }
        let hold_root = Path::new("/Applications/.opensteamer-paired-v6-install-selftest");
        let hold_app = hold_root.join("opensteamer Host.app");
        require_v6_install_hold_layout(hold_root, &hold_app)?;
        if require_v6_install_hold_layout(
            Path::new("/Applications/opensteamer Host.app"),
            &hold_app,
        )
        .is_ok()
            || require_v6_install_hold_layout(hold_root, &hold_root.join("unreviewed Host.app"))
                .is_ok()
        {
            return Err(ControllerError(
                "paired-v6 install-hold layout self-test failed".to_owned(),
            ));
        }
        paired_v6_dynamic_self_test()?;
        println!("SELF_TEST_OK paired-v6-host-update-controller");
        Ok(())
    }

    fn verify_v6_cli_surface() -> Result<()> {
        let executable = "controller".to_owned();
        let repo = V6_EXPECTED_REPO.to_owned();
        let authorized_commit = "a".repeat(40);
        let authorized_tree = "b".repeat(40);
        let allowed = [
            vec![
                executable.clone(),
                V6_PREFLIGHT_MODE.to_owned(),
                repo.clone(),
            ],
            vec![
                executable.clone(),
                V6_EXECUTE_MODE.to_owned(),
                repo.clone(),
            ...
            vec![
                executable.clone(),
                V6_ROLLBACK_MODE.to_owned(),
                repo.clone(),
            ],
            vec![executable.clone(), V6_SELF_TEST_MODE.to_owned()],
            vec![
                executable.clone(),
        ...
        if !matches!(parse_v6_command(&allowed[0]), Ok(V6Command::Preflight(_)))
            || !matches!(parse_v6_command(&allowed[1]), Ok(V6Command::Execute { .. }))
            || !matches!(parse_v6_command(&allowed[2]), Ok(V6Command::Rollback(_)))
            || !matches!(parse_v6_command(&allowed[3]), Ok(V6Command::SelfTest))
            || !matches!(
                parse_v6_command(&allowed[4]),
                Ok(V6Command::ProbeLock { .. })
            )
        {
            return Err(ControllerError(
                "paired-v6 CLI rejected a reviewed command shape".to_owned(),
            ));
        }

        let malformed = [
            vec![executable.clone(), V6_PREFLIGHT_MODE.to_owned()],
            vec![executable.clone(), V6_EXECUTE_MODE.to_owned(), repo.clone()],
            vec![
                executable.clone(),
                V6_EXECUTE_MODE.to_owned(),
                repo.clone(),
        ...
        if malformed
            .iter()
            .any(|arguments| parse_v6_command(arguments).is_ok())
        {
            return Err(ControllerError(
                "paired-v6 CLI accepted an unreviewed command shape".to_owned(),
            ));
        }
        ...
        {
            return Err(ControllerError(
                "paired-v6 authorization binding accepted a mismatched commit or tree".to_owned(),
            ));
        }
        ...
        for encoded_mode in [
            ...
            let mode = String::from_utf8(decode_marker_hex(encoded_mode)?).map_err(|_| {
                ControllerError("paired-v6 CLI test mode is not UTF-8".to_owned())
            })?;
            let arguments = vec![executable.clone(), mode, repo.clone()];
            if parse_v6_command(&arguments).is_ok() {
                return Err(ControllerError(
                    "paired-v6 CLI exposed an inherited update mode".to_owned(),
                ));
            }
        }
        Ok(())
    ...
    fn paired_v6_dynamic_self_test() -> Result<()> {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map_err(|_| ControllerError("system clock predates Unix epoch".to_owned()))?
            .as_nanos();
        let directory = PathBuf::from(format!(
            "/private/tmp/opensteamer-paired-v6-selftest-{}-{unique}",
            std::process::id()
        ));
        require_path_absent(&directory, "paired-v6 self-test directory")?;
        fs::create_dir(&directory)?;
        fs::set_permissions(&directory, fs::Permissions::from_mode(0o700))?;
        let result = paired_v6_dynamic_self_test_in(&directory);
        let cleanup = fs::remove_dir_all(&directory);
        match (result, cleanup) {
            (Err(error), _) => Err(error),
            (Ok(()), Err(error)) => Err(ControllerError(format!(
                "cannot remove paired-v6 self-test directory: {error}"
            ))),
            (Ok(()), Ok(())) => Ok(()),
        }
    }

    fn paired_v6_dynamic_self_test_in(directory: &Path) -> Result<()> {
        let recoverable = directory.join("recoverable.log");
        let mut journal = V6Journal::create(&recoverable)?;
        journal.record(
            V6State::SourceExported,
            &[
                ("commit", "a".repeat(40)),
        ...
        {
            return Err(ControllerError(
                "paired-v6 journal validation failure changed durable bytes".to_owned(),
            ));
        }
        drop(journal);
        ...
        partial.sync_all()?;
        drop(partial);
        let mut reopened = V6Journal::open(&recoverable)?;
        if reopened.state != V6State::SourceExported {
            return Err(ControllerError(
                "paired-v6 journal recovery accepted an incomplete final record".to_owned(),
            ));
        }
        reopened.record(
            V6State::BuildVerified,
        ...
        let mut corrupt_journal = V6Journal::create(&corrupt)?;
        ...
        drop(corrupt_append);
        if V6Journal::open(&corrupt).is_ok() {
            return Err(ControllerError(
                "paired-v6 journal recovery accepted a malformed complete record".to_owned(),
            ));
        }
        ...
        if acquire_update_transaction_lock_at(&transaction_lock).is_ok() {
            return Err(ControllerError(
                "paired-v6 transaction lock allowed concurrent ownership".to_owned(),
            ));
        }
        drop(first);
        ...
        require_v6_update_root_unused_at(&failed_attempt_root)?;
        ...
        if require_v6_update_root_unused_at(&failed_attempt_root).is_ok() {
            return Err(ControllerError(
                "paired-v6 single-attempt gate accepted failed-before-stop evidence".to_owned(),
            ));
        }
        ...
        require_v6_update_root_unused_at(&rolled_back_root)?;
        ...
        if require_v6_update_root_unused_at(&rolled_back_root).is_ok() {
            return Err(ControllerError(
                "paired-v6 single-attempt gate accepted rolled-back evidence".to_owned(),
            ));
        }
    ...
    fn self_test_partial_install_hold_recovery(directory: &Path) -> Result<()> {
        ...
        archive_v6_install_hold_root_at(&install_hold_root, &archive)?;
        require_path_absent(&install_hold_root, "quarantined partial install-hold root")?;
        require_directory(&archive, 0o700)?;
        ...
        fs::create_dir(&install_hold_root)?;
        fs::set_permissions(&install_hold_root, fs::Permissions::from_mode(0o700))?;
        if archive_v6_install_hold_root_at(&install_hold_root, &archive).is_ok() {
            return Err(ControllerError(
    ...
    fn self_test_publication_boundary_case(
        ...
        const REPLACEMENT_BYTES: &str = "paired-v6-replacement";
