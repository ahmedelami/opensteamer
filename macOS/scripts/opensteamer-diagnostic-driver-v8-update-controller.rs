...
const PREFLIGHT_MODE: &str = "--verify-diagnostic-driver-v8-update-preflight";
const EXECUTE_MODE: &str = "--execute-authorized-diagnostic-driver-v8-update";
const ROLLBACK_MODE: &str = "--rollback-authorized-diagnostic-driver-v8-update";
const SELF_TEST_MODE: &str = "--self-test-diagnostic-driver-v8-update";
const ROOT_MODE: &str = "--root-authorized-diagnostic-driver-v8-update";
const ROOT_ROLLBACK_MODE: &str = "--root-rollback-diagnostic-driver-v8-update";
const ROOT_SEALED_ROLLBACK_MODE: &str = "--root-sealed-rollback-diagnostic-driver-v8-update";
const LOCK_PROBE_MODE: &str = "--probe-diagnostic-driver-v8-host-lock";
const LOCK_FREE_PROBE_MODE: &str = "--probe-diagnostic-driver-v8-host-lock-free";
const LOCK_RECORD_PROBE_MODE: &str = "--read-diagnostic-driver-v8-host-lock-record";
...
const EXPECTED_RELEASE_BRANCH: &str = "fix/diagnostic-driver-v8-current-host";
const EXPECTED_UPDATER_BASE_COMMIT: &str = "5f1160f2d4e70dc4e76a2b2bf9006659858e7bdd";
const EXPECTED_UPDATER_BASE_TREE: &str = "074ced33592b71e7a1744749cb07122301a567e8";
...
const ROOT_RECOVERY_CONTROLLER: &str =
    "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v8/recovery-controller";
const ROOT_RECOVERY_CONTROLLER_PIN: &str = "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v8/recovery-controller.sha256";
...
const JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_UPDATE_V8";
const ROOT_JOURNAL_HEADER: &str = "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_UPDATE_V8";
...
fn parse_fresh_route_helper_record(text: &str) -> Result<PostReloadRouteObservation> {
    ...
    let lines = text
        ...
        .split('\n')
        .collect::<Vec<_>>();
    if lines.first().copied() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_FRESH_ROUTE_V8") {
        return Err(ControllerError(
...
fn run_uid501_fresh_route_helper(timeout: Duration) -> Result<Output> {
    let executable = env::current_exe()?;
    bounded_output(
        path_text(&executable)?,
...
fn fresh_route_observation_for_generation(
    ...
    let capture = run_uid501_fresh_route_helper(COMMAND_TIMEOUT);
    ...
    classify_fresh_route_helper_execution(capture)
}
...
fn prove_post_reload_routes(
    ...
    for attempt in 1..=POST_RELOAD_ROUTE_MAX_ATTEMPTS {
        ...
        let before = read_coreaudio_generation_before(deadline)?;
        ...
        let capture =
            run_uid501_fresh_route_helper(remaining_before_post_reload_deadline(deadline)?);
        let after = read_coreaudio_generation_before(deadline)?;
        ...
        let observation = classify_fresh_route_helper_execution(capture)?;
        let progress = advance_post_reload_route_proof(
            &mut previous_complete,
            authenticated_baseline,
            observation,
        )?;
...
fn verify_git_provenance(repo: &Path) -> Result<(String, String)> {
    ...
    if branch != EXPECTED_RELEASE_BRANCH {
        return Err(ControllerError(
            "live release checkout is not the exact dedicated V8 branch".to_owned(),
        ));
    }
...
fn preflight(repo: &Path) -> Result<()> {
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V8_PREFLIGHT_OK host_pid={} host_runs={} coreaudiod_pid={} coreaudiod_runs={} installed_driver={}:{} candidate_commit={} candidate_tree={} release_commit={} release_tree={} pairing=metadata-only legacy=protected retained_v1_v2_v3_v4_v5_v6_v7=immutable reader=passive both_order=no-default-mutation namespaces=fresh",
        host.pid,
        host.runs,
        coreaudio.pid,
        coreaudio.runs,
...
fn create_user_layout(nonce: &str) -> Result<UserLayout> {
    ...
    let evidence = Path::new(USER_UPDATE_ROOT).join(format!(
        "diagnostic-driver-v8-{seconds}-{}-{nonce}",
        std::process::id()
    ));
...
fn parse_root_request_text(text: &str) -> Result<RootRequest> {
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V8") {
    ...
    if request.reader_sha256 != DIAGNOSTIC_READER_SHA256
        ...
        || !request
            ...
            .and_then(OsStr::to_str)
            .is_some_and(|leaf| {
                leaf.starts_with("diagnostic-driver-v8-") && leaf.ends_with(&request.nonce)
            })
...
fn parse_root_state_text(
    ...
    let mut lines = text.lines();
    if lines.next() != Some("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_STATE_V8") || !text.ends_with('\n')
    {
        return Err(ControllerError(
            ...
            .is_some_and(|evidence| {
                evidence.parent() == Some(Path::new(USER_UPDATE_ROOT))
                    && evidence
                        .file_name()
                        .and_then(OsStr::to_str)
                        .is_some_and(|leaf| leaf.starts_with("diagnostic-driver-v8-"))
...
fn run_passive_driver_validation(
    ...
    let nonce = format!("diagnostic-driver-v8-{}", request.nonce);
...
fn write_root_result(layout: &RootLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RESULT_V8\nstatus={status}\ndetail={detail}\n");
...
fn write_root_recovery_result(layout: &RootLayout, outcome: &str, detail: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V8\noutcome={outcome}\ndetail={detail}\n"
    );
    match fs::symlink_metadata(&layout.recovery_result) {
        ...
        Ok(_) => {
            ...
            if !existing.starts_with("OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_RECOVERY_RESULT_V8\n")
                || !existing.ends_with('\n')
...
fn perform_root_transaction(request_path: &Path) -> Result<HostGeneration> {
        ...
        Err(update_error) => {
            match rollback_root_transaction(&layout, &mut journal, &initial, Some(&baseline_route))
            {
                Ok(rollback) => {
                    ...
                    Err(ControllerError(format!(
                    "candidate deployment failed and exact v8 rollback completed with host pid {} routes={route_status}: {}",
    ...
    if verify_retained_v5_root_rolled_back(&retained_v5_root_lock)? != retained_v5 {
        return Err(ControllerError(
            "retained v5 root boundary changed across the V8 cutover/rollback".to_owned(),
        ));
    }
    if verify_retained_v6_root_rolled_back(&retained_v6_root_lock)? != retained_v6 {
        return Err(ControllerError(
            "retained v6 root boundary changed across the V8 cutover/rollback".to_owned(),
        ));
    }
    if verify_retained_v7_root_rolled_back(&retained_v7_root_lock)? != retained_v7 {
        return Err(ControllerError(
            "retained v7 root boundary changed across the V8 cutover/rollback".to_owned(),
        ));
    }
    outcome
}
...
fn root_authorized_update(request_path: &Path) -> Result<()> {
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V8_UPDATE_COMMITTED host_pid={} host_runs={} pairing=preserved routes=unchanged legacy=protected",
        host.pid, host.runs
    );
    Ok(())
}
...
fn write_user_result(layout: &UserLayout, status: &str, detail: &str) -> Result<()> {
    ...
    let bytes =
        format!("OPENSTEAMER_DIAGNOSTIC_DRIVER_RESULT_V8\nstatus={status}\ndetail={detail}\n");
...
fn execute_authorized_update(
    ...
    if commit != authorized_commit || tree != authorized_tree {
        return Err(ControllerError(
            "explicit authorization differs from the clean pushed dedicated V8 branch HEAD/tree"
                .to_owned(),
        ));
    }
    ...
    {
        return Err(ControllerError(
            "retained v1/v2/v3/v4/v5/v6/v7 descriptor guard changed immediately before root dispatch"
                .to_owned(),
        ));
    }
    ...
    if stdout.lines().count() != 1 || !stdout.starts_with("DIAGNOSTIC_DRIVER_V8_UPDATE_COMMITTED ")
    {
        return Err(ControllerError(
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V8_UPDATE_COMPLETE evidence={} host_pid={} pairing=preserved routes=unchanged legacy=protected",
...
fn rollback_authorized_update(repo: &Path) -> Result<()> {
    ...
    let recovery_kind = if line.starts_with("DIAGNOSTIC_DRIVER_V8_ROOT_ROLLBACK_COMPLETE ") {
        "rolled-back"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V8_ROOT_PRESTOP_ABORTED ") {
        "prestop-aborted"
    } else if line.starts_with("DIAGNOSTIC_DRIVER_V8_ROOT_RECOVERY_COMMITTED ") {
        "committed"
    } else {
        return Err(ControllerError(
    ...
    verify_pairing_metadata_only()?;
    println!(
        "DIAGNOSTIC_DRIVER_V8_RECOVERY_COMPLETE outcome={recovery_kind} host_pid={} pairing=preserved routes={route_status} legacy=protected",
        host.pid
    );
    Ok(())
}
...
fn complete_root_recovery(_request: RootRequest, layout: RootLayout) -> Result<()> {
    ...
    let (outcome, marker) = match plan {
        RootRecoveryPlan::PrestopPreserveHost => (
            ...
            "DIAGNOSTIC_DRIVER_V8_ROOT_PRESTOP_ABORTED",
        ),
        RootRecoveryPlan::RepairCommittedState | RootRecoveryPlan::ReportCommitted => (
            ...
            "DIAGNOSTIC_DRIVER_V8_ROOT_RECOVERY_COMMITTED",
        ),
        RootRecoveryPlan::RepairRolledBackState | RootRecoveryPlan::ReportRolledBack => (
            ...
            "DIAGNOSTIC_DRIVER_V8_ROOT_ROLLBACK_COMPLETE",
        ),
        ...
        RootRecoveryPlan::ResumeRollback => {
            ...
            (outcome, "DIAGNOSTIC_DRIVER_V8_ROOT_ROLLBACK_COMPLETE")
        }
    ...
    let recovery_outcome = match marker {
        "DIAGNOSTIC_DRIVER_V8_ROOT_RECOVERY_COMMITTED" => "committed",
        "DIAGNOSTIC_DRIVER_V8_ROOT_PRESTOP_ABORTED" => "prestop-aborted",
        "DIAGNOSTIC_DRIVER_V8_ROOT_ROLLBACK_COMPLETE" => "rolled-back",
        _ => {
            return Err(ControllerError(
...
fn write_or_verify_prestop_abort_witness(layout: &RootLayout, journal: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_PRESTOP_ABORT_JOURNAL_V8\noutcome=prestop-aborted\njournal={journal}\n"
    );
    match fs::symlink_metadata(&path) {
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
...
fn finalize_sealed_bootstrap_without_root_pointer(fixed_digest: &str) -> Result<()> {
    ...
    let bytes = format!(
        "OPENSTEAMER_DIAGNOSTIC_DRIVER_BOOTSTRAP_ABORT_V8\nnonce={}\nhost_pid={}\noutcome=prestop-aborted\n",
    ...
    println!(
        "DIAGNOSTIC_DRIVER_V8_ROOT_PRESTOP_ABORTED host_pid={} routes=unproved pairing=preserved",
        host.pid
    );
    Ok(())
}
...
fn root_sealed_rollback_authorized_update() -> Result<()> {
    ...
    let Some(layout) = reconcile_root_pointer_for_recovery(&locator_request)? else {
        ...
        {
            return Err(ControllerError(
                "retained v4/v5/v6/v7 root boundary changed before pointerless recovery finalization"
                    .to_owned(),
            ));
        }
    ...
    {
        return Err(ControllerError(
            "retained v4/v5/v6/v7 root boundary changed before sealed recovery".to_owned(),
        ));
    }
...
fn self_test() -> Result<()> {
    ...
    let root_hash_path = Path::new(
        "/Library/Application Support/opensteamer/diagnostic-driver-controllers-v8/controller-0123456789abcdef0123456789abcdef/controller",
    );
    ...
    let synthetic = RootRequest {
        ...
        evidence: PathBuf::from(
            "/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v8/diagnostic-driver-v8-1-1-0123456789abcdef0123456789abcdef",
        ),
    ...
    for (fixture, label) in [
        ...
        (
            request_text.replace(
                ...
                "evidence=/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v8/../escape\n",
            ),
        ...
        (
            request_text.replace(
                ...
                "root_controller=/Library/Application Support/opensteamer/diagnostic-driver-controllers-v8/controller-0123456789abcdef0123456789abcdef/../controller\n",
            ),
    ...
    let pointer_bytes = b"/Library/Application Support/opensteamer/diagnostic-driver-updates-v8/transaction-0123456789abcdef0123456789abcdef\n";
    ...
    for (fixture, label) in [
        (
            complete_route_record.replacen(
                "OPENSTEAMER_DIAGNOSTIC_DRIVER_FRESH_ROUTE_V8",
                "OPENSTEAMER_DIAGNOSTIC_DRIVER_FRESH_ROUTE_V7",
                1,
            ),
    ...
    if !uid501_source_path_is_allowed(Path::new(BOTH_ORDER_PROBE), false)
        ...
        || uid501_source_path_is_allowed(
            Path::new("/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v8/../escape/probes/both-order.json"),
            true,
        )
    {
        return Err(ControllerError(
    ...
    let source = include_str!("opensteamer-diagnostic-driver-v8-update-controller.rs");
    let forbidden = [
    ...
    println!("DIAGNOSTIC_DRIVER_V8_SELF_TEST_OK tests=112");
    Ok(())
}
