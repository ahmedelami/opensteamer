...
PREFLIGHT_MODE='--verify-diagnostic-driver-v16-update-preflight'
EXECUTE_MODE='--execute-authorized-diagnostic-driver-v16-update'
ROLLBACK_MODE='--rollback-authorized-diagnostic-driver-v16-update'
SELF_TEST_MODE='--self-test-diagnostic-driver-v16-update'
EXPECTED_REPO='/Users/ahmed/Documents/Codex/opensteamer-diagnostic-v3'
USER_UPDATE_ROOT='/Users/ahmed/Library/Application Support/opensteamer/diagnostic-driver-updates-v16'
USER_ACTIVE_POINTER='/Users/ahmed/Library/Application Support/opensteamer/active-diagnostic-driver-update-v16'
ROOT_SUPPORT='/Library/Application Support/opensteamer'
ROOT_CONTROLLER_PARENT='/Library/Application Support/opensteamer/diagnostic-driver-controllers-v16'
...
ROOT_ACTIVE_POINTER='/Library/Application Support/opensteamer/active-diagnostic-driver-update-v16'
ROOT_BOOTSTRAP_LOCATOR='/Library/Application Support/opensteamer/diagnostic-driver-bootstrap-v16.txt'
DIAGNOSTIC_READER_SHA256='6d3f5d71b399ada1244b84177d351c2406d426055da8f54e8f689af1c22a8ded'
SOURCE="$EXPECTED_REPO/macOS/scripts/opensteamer-diagnostic-driver-v16-update-controller.rs"
...
EXPECTED_SOURCE_SHA256='a61a0ff8edd3c6aa034020718ea4a972cc5c1eea5a66b61372e799177ef68b32'
EXPECTED_BINARY_SHA256='a210e748ddd61afba4c97b17d0b0d65eff52783e1d3faf227becdf29895c72f6'
...
if [ "$MODE" = "$ROLLBACK_MODE" ]; then
    ...
    if [ ! -e "$ROOT_ACTIVE_POINTER" ] && [ ! -L "$ROOT_ACTIVE_POINTER" ] \
        ...
        if [ -f "$ROOT_BOOTSTRAP_LOCATOR" ] && [ ! -L "$ROOT_BOOTSTRAP_LOCATOR" ] \
            ...
            && [ "$(/usr/bin/sed -n '1p' "$ROOT_BOOTSTRAP_LOCATOR")" = 'OPENSTEAMER_DIAGNOSTIC_DRIVER_ROOT_REQUEST_V16' ]; then
                        ...
                        case "$LOCATOR_DISPLAY_SELECTED" in
                            1080:1920:1080:1920:60000|603:1311:1206:2622:60000|540:1170:1080:2340:60000|540:960:1080:1920:60000|414:896:828:1792:60000|750:1334:750:1334:60000|720:1280:720:1280:60000)
                                case "$LOCATOR_EVIDENCE" in
                                    "$USER_UPDATE_ROOT"/diagnostic-driver-v16-*"$LOCATOR_NONCE") LOCATOR_READY=1 ;;
                                esac
                                ;;
                        esac
                    fi
        ...
        if [ "$FIXED_READY" -eq 0 ]; then
            ...
            echo 'DIAGNOSTIC_DRIVER_V16_BOOTSTRAP_INCOMPLETE_HOST_PRESERVED root_pointer=absent' >&2
            exit 75
        fi
    fi
    [ -d "$ROOT_SUPPORT" ] && [ ! -L "$ROOT_SUPPORT" ] \
    ...
    case "$RECOVERY_OUTPUT" in
        DIAGNOSTIC_DRIVER_V16_RECOVERY_COMPLETE\ outcome=committed\ *) RECOVERY_OUTCOME='committed' ;;
        DIAGNOSTIC_DRIVER_V16_RECOVERY_COMPLETE\ outcome=rolled-back\ *) RECOVERY_OUTCOME='rolled-back' ;;
        DIAGNOSTIC_DRIVER_V16_RECOVERY_COMPLETE\ outcome=prestop-aborted\ *) RECOVERY_OUTCOME='prestop-aborted' ;;
        *)
    ...
    if [ -f "$USER_ACTIVE_POINTER" ] && [ ! -L "$USER_ACTIVE_POINTER" ] \
        ...
        case "$RECOVERY_EVIDENCE" in
            "$USER_UPDATE_ROOT"/diagnostic-driver-v16-*)
...
compile_controller() {
    ...
    "$RUSTC" --edition=2021 -D warnings -C opt-level=2 \
        ...
        --remap-path-prefix "$EXPECTED_REPO=/reviewed/opensteamer-diagnostic-driver-v16" \
        --remap-path-prefix "$build_root=/reviewed/opensteamer-diagnostic-driver-v16-build" \
        "$SOURCE" -o "$controller"
