...
script_dir="${0:A:h}"
parser="$script_dir/parse-installer-signature-v7.sh"
test_root="$(/usr/bin/mktemp -d /private/tmp/opensteamer-installer-parser-v7.XXXXXX)"
case "$test_root" in
    /private/tmp/opensteamer-installer-parser-v7.*) ;;
    *) exit 73 ;;
esac
