...
| iPhone microphone forwarding into BlackHole | A connection-level unit lease registers the exact default-input listener before writing, saves and freshly resolves the prior UID, requires bounded listener/readback proof, is fenced by monitor/device/peer/connection generations, and conditionally restores only while it still owns BlackHole. Its acquisition occurs at peer+ICE+control health before system-audio startup and has no track/PCM dependency. The separate forwarding driver still requires exact track admission, a running queue, one successful post-start pull, and two advancing progress snapshots. The physical gate records the original input, proves BlackHole at the authenticated healthy boundary before PCM proof, keeps output and system output unchanged, restores the original input after disconnect, opens BlackHole input by stable UID, and recognizes a known remote challenge. | Delay selection until track or PCM, omit restoration, restore the wrong UID, accept a stale generation, treat `noErr` as proof, register the listener after writing, overwrite a newer external input choice, mutate output or system output, count priming as progress, treat silence or callback count alone as readiness, omit the second advancing snapshot, or replace physical UID-pinned capture with forwarding counters. |
...
- For worldwide-only microphone forwarding, capture the original input plus the
  output and system-output UIDs before connection. Require BlackHole as the input at
  the authenticated peer/ICE/control boundary before remote-track or PCM proof,
  exact output and system-output equality throughout, and restoration of the
